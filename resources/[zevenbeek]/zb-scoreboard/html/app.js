...
QBScoreboard.Open = function(data) {
    ...
    $("#uptime").html("Uptime: "+data.uptimeHours+" uur en "+data.uptimeMinutes+" minuten.");
    ...
    $.each(data.requiredCops, function(i, category){
        ...
        if (data.currentMakelaar > 0) {
            var Abeam = $(".scoreboard-info").find('[data-type="makelaar"]');
            var Astatus = $(Abeam).find(".info-beam-status");
            $(Astatus).html('<i class="fas fa-check"></i>');
        } else {
            var Abeam = $(".scoreboard-info").find('[data-type="makelaar"]');
        ...
        if (data.currentANWB > 0) {
            var Abeam = $(".scoreboard-info").find('[data-type="anwb"]');
            var Astatus = $(Abeam).find(".info-beam-status");
            $(Astatus).html('<i class="fas fa-check"></i>');
        } else {
            var Abeam = $(".scoreboard-info").find('[data-type="anwb"]');

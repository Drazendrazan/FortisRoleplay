...
Citizen.CreateThread(function()
	AddTextEntry("Tattoos", "Tattoo Winkel")
...
function RemoveTattoo(name, label)
	...
	QBCore.Functions.Notify("Je hebt " .. GetLabelText(label) .. " weg gelaserd", 'success')
end

function CreateScale(sType)
	if scaleString ~= sType and sType == "OpenShop" then
		scaleType = setupScaleform("instructional_buttons", "Open Tattoo Winkel", 38)
		scaleString = sType
	elseif scaleString ~= sType and sType == "Control" then
		scaleType = setupScaleform2("instructional_buttons", "Verander Camera Positie", 21, "Verander Tattoo zichtbaarheid", {90, 89}, "Koop/Verwijder Tattoo", 191)
		scaleString = sType
	end
end

Citizen.CreateThread(function()
	TattooMenu.CreateMenu("tattoo", "Tattoo Winkel", function()
        return CloseTattooShop()
    end)
    TattooMenu.SetSubTitle('tattoo', "Type Tattoo")
						...
						if GetEntityModel(PlayerPedId()) == `mp_m_freemode_01` then
							if tattoo.HashNameMale ~= '' then
								...
								if found then
									...
									local clicked, hovered = TattooMenu.Button(GetLabelText(tattoo.Name), "~HUD_COLOUR_WHITE~€ 800")
									if clicked then

...
QBCore.Commands.Add("verkoopauto", "Verkoop een auto aan een speler.", {{name="id", help="ID"},{name="autotier", help="(1-8)"}}, true, function(source, args)
	...
	if Player ~= nil then
        TriggerClientEvent("zb-luxury:client:verkoopAuto", tonumber(args[1]), tier, verkoperID)
	else
...
QBCore.Commands.Add("verkoopheli", "Verkoop een helikopter aan een speler.", {{name="id", help="ID"},{name="helitier", help="(10-14)"}}, true, function(source, args)
    ...
    local verkoperID = source
	if Player ~= nil then
        TriggerClientEvent("zb-luxury:client:verkoopHeli", tonumber(args[1]), tier, verkoperID)
	else
		TriggerClientEvent('chatMessage', source, "SYSTEEM", "error", "Speler is niet online!")
	end
end, "admin")

QBCore.Functions.CreateCallback("zb-luxury:server:koopVoertuig", function(source, cb, tier)
    local src = source
...
RegisterNetEvent("zb-luxury:server:notifyVerkoperSucces")
AddEventHandler("zb-luxury:server:notifyVerkoperSucces", function(verkoperID)
    local verkoperID = verkoperID
    TriggerClientEvent("QBCore:Notify", verkoperID, "Je client heeft het voertuig kunnen betalen!")
end)

RegisterNetEvent("zb-luxury:server:notifyVerkoperFout")
AddEventHandler("zb-luxury:server:notifyVerkoperFout", function(verkoperID2)

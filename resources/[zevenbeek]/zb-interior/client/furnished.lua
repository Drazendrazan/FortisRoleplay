...
function CreateApartmentFurnished(spawn)
    ...
    local POIOffsets = {}
	POIOffsets.exit = json.decode('{"z":1.2,"y":4,"x":3.572736328125,"h":2.2633972168}')
    ...
    DoScreenFadeOut(500)
	local hash = GetHashKey('gabz_pinkcage')
	...
	local house = CreateObject(hash, spawn.x, spawn.y, spawn.z, false, false, false)
    FreezeEntityPosition(house, true)
	table.insert(objects, house)
	

	if IsNew then
		TeleportToInterior(spawn.x - 4.5, spawn.y - 2.10, spawn.z + 1.5, POIOffsets.exit.h)
		SetTimeout(750, function()
			TriggerEvent('qb-clothes:client:CreateFirstCharacter')
			IsNew = false
		end)
	else
		TeleportToInterior(spawn.x - 4.0, spawn.y - 3.00144140625, spawn.z + 1.5, POIOffsets.exit.h)
	end
    return { objects, POIOffsets }
end

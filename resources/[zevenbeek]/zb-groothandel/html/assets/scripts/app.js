...
const app = {
    ...
    methods: {
        ...
        closeLaptop() {
            ...
            $.post("https://zb-groothandel/sluitLaptop", JSON.stringify({}));
        },
        ...
        werknemerAannemen() {
            if (this.programmas.magazijn.appData.bsn && this.programmas.magazijn.appData.bsn.length > 0) {
                if (this.groothandelData.data.werknemerspots > Object.keys(this.werknemersData).length) {
                    // Stuur post request
                    $.post("https://zb-groothandel/werknemerAannemen", JSON.stringify({
                        bsn: this.programmas.magazijn.appData.bsn
                    }), function(resultaat) {
                        if (resultaat.resultaat) {
                            $.post("https://zb-groothandel/NuiNotify", JSON.stringify({
        ...
        werknemerOntslaan(citizenid) {
            $.post("https://zb-groothandel/werknemerOntslaan", JSON.stringify({
                cid: citizenid
            }))
        },
        ...
        koopWerknemerSlot() {
            if (this.groothandelData.data.werknemerspots < 5) {
                $.post("https://zb-groothandel/koopWerknemerSlot", JSON.stringify({}));
            }
        },

        // Start handel ronde (verkopen en inkopen in 1 functie)
        handel_start(type, index) {
            $.post("https://zb-groothandel/startRonde", JSON.stringify({
                id: index,
                type: type
            }));
        },
        ...
        balansOpnemen() {
            $.post("https://zb-groothandel/balansOpnemen", JSON.stringify({}));
        },

        koopVoertuig(voertuig) {
            $.post("https://zb-groothandel/koopVoertuig", JSON.stringify({
                voertuig: voertuig,
            }));
        },
        ...
        veranderSpecialisatie() {
            if (this.groothandelData.data.specialisatie_aanpassen_kan) {
                ...
                $.post("https://zb-groothandel/veranderSpecialisaties", JSON.stringify({}));
            } else {

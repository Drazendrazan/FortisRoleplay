        ...
        if IsPedInAnyVehicle(GetPlayerPed(-1)) then
            ...
            if IsControlJustReleased(0, Keys["B"]) and GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1)), -1) == GetPlayerPed(-1) then 
                if IsPedInAnyVehicle(GetPlayerPed(-1)) then
                    cruiseSpeed = Speed
                    if cruiseOn then
                        QBCore.Functions.Notify("Begrenzer uitgeschakeld!", "error")
                    else
                        QBCore.Functions.Notify("Begrenzer ingeschakeld op "..tostring(math.floor(cruiseSpeed * 3.6)).."km/h", "success")
                    end

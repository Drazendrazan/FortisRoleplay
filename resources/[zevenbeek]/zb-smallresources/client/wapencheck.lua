    ...
    while true do
        ...
        for w, wapenLijst in pairs(wapens["wapens"]) do
            if playerWapen == GetHashKey(wapenLijst) and playerWapen ~= laatsteWapen then
                ...
                TriggerServerEvent("zb-smallresources:server:wapencheck", wapenLijst, playerWapen)
            end
        end
        Wait(2000)
    end
...
AddEventHandler("zb-smallresources:client:wapenGeefCheck", function()
	local playerWapen = GetSelectedPedWeapon(GetPlayerPed(-1))
    local playerWapenHash = GetHashKey(playerWapen)
...
RegisterNetEvent('zb-smallresources:client:leegHandWapen')
AddEventHandler('zb-smallresources:client:leegHandWapen', function(wapenHash)

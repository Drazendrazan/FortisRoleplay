...
AddEventHandler('inventory:server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount)
	...
	if fromInventory == "player" or fromInventory == "hotbar" then
		...
		if fromItemData ~= nil and fromItemData.amount >= fromAmount then
			...
			elseif QBCore.Shared.SplitStr(toInventory, "-")[1] == "traphouse" then
				...
				local toItemData = exports['zb-traphouses']:GetInventoryData(traphouseId, toSlot)
				local IsItemValid = exports['zb-traphouses']:CanItemBeSaled(fromItemData.name:lower())
				if IsItemValid then
					Player.Functions.RemoveItem(fromItemData.name, fromAmount, fromSlot)
					TriggerClientEvent("inventory:client:CheckWeapon", src, fromItemData.name)
					if toItemData ~= nil then
						...
						local toAmount = tonumber(toAmount) ~= nil and tonumber(toAmount) or toItemData.amount
						if toItemData.name ~= fromItemData.name then
							exports['zb-traphouses']:RemoveHouseItem(traphouseId, fromSlot, itemInfo["name"], toAmount)
							Player.Functions.AddItem(toItemData.name, toAmount, fromSlot, toItemData.info)
	...
	elseif QBCore.Shared.SplitStr(fromInventory, "-")[1] == "traphouse" then
		...
		if fromItemData ~= nil and fromItemData.amount >= fromAmount then
			...
			if toInventory == "player" or toInventory == "hotbar" then
				local toItemData = Player.Functions.GetItemBySlot(toSlot)
				exports['zb-traphouses']:RemoveHouseItem(traphouseId, fromSlot, itemInfo["name"], fromAmount)
				if toItemData ~= nil then
					local itemInfo = QBCore.Shared.Items[toItemData.name:lower()]
					local toAmount = tonumber(toAmount) ~= nil and tonumber(toAmount) or toItemData.amount
					if toItemData.name ~= fromItemData.name then
						Player.Functions.RemoveItem(toItemData.name, toAmount, toSlot)
						exports['zb--traphouses']:AddHouseItem(traphouseId, fromSlot, itemInfo["name"], toAmount, toItemData.info, src)
...
RegisterServerEvent("zb-inventory:server:geefItem")
AddEventHandler("zb-inventory:server:geefItem", function(item, aantal, naarSpeler)
	local src = source
	local item = item
	local aantal = aantal
...
-- Contant geld check
QBCore.Functions.CreateCallback("zb-inventory:server:vraagContantGeld", function(source, cb)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
...
-- test dingejte
QBCore.Functions.CreateCallback("zb-inventory:server:combineerTestCheckDing", function(source, cb, item1, item2)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent("zb-inventory:server:verwijderKnuppelItem")
AddEventHandler("zb-inventory:server:verwijderKnuppelItem", function()
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent("zb-inventory:server:banCombineHacker")
AddEventHandler("zb-inventory:server:banCombineHacker", function()
    local src = source

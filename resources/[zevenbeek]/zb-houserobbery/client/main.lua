...
function enterRobberyHouse(house)
    ...
    if Config.Houses[house]["tier"] == 1 then
        data = exports['zb-interior']:CreateTier1HouseFurnished(coords)
    end
    Citizen.Wait(100)
    houseObj = data[1]
    POIOffsets = data[2]
...
RegisterNetEvent('zb-huisoverval:client:ResetOssoStatus')
AddEventHandler('zb-huisoverval:client:ResetOssoStatus', function(house)
...
RegisterNetEvent('zb-huisoverval:client:betreedOsso')
AddEventHandler('zb-huisoverval:client:betreedOsso', function(house)
...
function lockpickFinish(success)
    if success then
        TriggerServerEvent('zb-huisoverval:server:betreedHuis', closestHouse)
    else
        if usingAdvanced then
...
RegisterNetEvent('zb-huisoverval:client:zetHuisStatus')
AddEventHandler('zb-huisoverval:client:zetHuisStatus', function(house, state)
...
function searchCabin(cabin)
    ...
    TriggerServerEvent('zb-huisoverval:server:zetKauloDrukStatus', cabin, currentHouse, true)
    ...
    Skillbar.Start({
        ...
        if SucceededAttempts + 1 >= NeededAttempts then
            ...
            openingDoor = false
            ClearPedTasks(GetPlayerPed(-1))
            TriggerServerEvent('zb-huisoverval:server:zoekKasjeZebai', cabin, currentHouse)
            Config.Houses[currentHouse]["furniture"][cabin]["searched"] = true
            TriggerServerEvent('zb-huisoverval:server:zetKauloDrukStatus', cabin, currentHouse, false)
            SucceededAttempts = 0
...
RegisterNetEvent('zb-huisoverval:client:zetKasjeStatusOulleh')
AddEventHandler('zb-huisoverval:client:zetKasjeStatusOulleh', function(house, cabin, state)
    Config.Houses[house]["furniture"][cabin]["searched"] = state
end)

RegisterNetEvent('zb-huisoverval:client:zetDrukkeOsso')
AddEventHandler('zb-huisoverval:client:zetDrukkeOsso', function(cabin, house, bool)

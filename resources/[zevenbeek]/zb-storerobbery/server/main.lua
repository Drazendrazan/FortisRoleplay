...
AddEventHandler('qb-storerobbery:server:callCops', function(type, safe, streetLabel, coords)
    ...
    local alertData = {
        ...
        description = "Er is een winkel overval gaande te "..streetLabel.." (CAMERA ID: "..cameraId..")"
    }
    ...
    while true do
        for k, v in pairs(Config.Registers) do
            if Config.Registers[k].time > 0 and (Config.Registers[k].time - Config.tickInterval) >= 0 then
                ...
                Config.Registers[k].sync = nil
            else
                if Config.Registers[k].sync == nil then
                    Config.Registers[k].sync = true
...
-- Finns code
local robberyTimeout = 0

Citizen.CreateThread(function()
    while true do
        Wait(1000)
        if robberyTimeout > 0 then
            robberyTimeout = robberyTimeout - 1
        end
    end
end)

RegisterServerEvent('qb-storerobbery:server:setRobberyTimeout')
AddEventHandler('qb-storerobbery:server:setRobberyTimeout', function(time)
    robberyTimeout = time
end)

QBCore.Functions.CreateCallback('qb-storerobbery:server:getRobberyTimeout', function(source, cb)
    cb(robberyTimeout)
end)
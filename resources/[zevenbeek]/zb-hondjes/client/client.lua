            ...
            if not vrij then
                ...
                if timer == 0 then
                    TriggerServerEvent("qb-phone:server:sendNewMail", {
                        ...
                        button = {
                            enabled = true,
                            buttonEvent = "zb-hondjes:client:krijglocatie",
                        }
                    })
                end
            else
        ...
        while true do
            ...
            if not klaar then
                ...
                if GetDistanceBetweenCoords(pos, deurLocatie["deur"].x, deurLocatie["deur"].y, deurLocatie["deur"].z) < 3 then
                    ...
                    if IsControlJustPressed (0, 38) then
                        TriggerServerEvent("zb-hondjes:server:krijggeld")
                        RemoveBlip(DeurBlip)
                        DeleteEntity(dierntie)
                        SetPedAsNoLongerNeeded(pendtie)
                        dierSpawn = false

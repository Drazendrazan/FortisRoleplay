...
RegisterNUICallback('setFoodWarning', function(data)
    ...
    QBCore.Functions.Notify('Fitbit: Eten waarschuwing gezet op '..foodValue..'%')
end)
...
RegisterNUICallback('setThirstWarning', function(data)
    ...
    QBCore.Functions.Notify('Fitbit: Dorst waarschuwing gezet op '..thirstValue..'%')
end)
                ...
                if hasItem then
                    ...
                    if PlayerData.metadata["fitbit"].food ~= nil then
                        if PlayerData.metadata["hunger"] < PlayerData.metadata["fitbit"].food then
                            TriggerEvent("chatMessage", "FITBIT ", "warning", "Je eten is "..round(PlayerData.metadata["hunger"], 2).."%")

...
AddEventHandler("QBCore:Client:OnPlayerLoaded", function()
	QBCore.Functions.TriggerCallback("zb-playtime:server:ontvangPlaytime", function(playtimeTabel)
    ...
    while true do
        ...
        if ingelogd then
            ...
            if type(db_id) == "boolean" then
			    TriggerServerEvent("zb-playtime:server:updatePlaytime", minuten, 0)
            else
                TriggerServerEvent("zb-playtime:server:updatePlaytime", minuten, db_id)
            end
			Citizen.Wait(60000)
        else
            Citizen.Wait(3000)

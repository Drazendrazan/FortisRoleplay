...
QBCore.Functions.CreateCallback("zb-playtime:server:ontvangPlaytime", function(source, cb)
    if source ~= nil then
...
RegisterNetEvent("zb-playtime:server:updatePlaytime")
AddEventHandler("zb-playtime:server:updatePlaytime", function(minuten, db_id)
    if source ~= nil then
        local db_id = db_id

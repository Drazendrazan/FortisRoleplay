                        ...
                        if dist < 30 then
                            ...
                            if dist < 0.6 then
                                if not Config.Locations[case]["isBusy"] and not Config.Locations[case]["isOpened"] then
                                    DrawText3Ds(Config.Locations[case]["coords"]["x"], Config.Locations[case]["coords"]["y"], Config.Locations[case]["coords"]["z"], '~g~[E]~w~ - Inslaan')
...
function smashVitrine(k)
    ...
    elseif math.random(1, 100) <= 5 and IsWearingHandshoes() then
        TriggerServerEvent("evidence:server:CreateFingerDrop", plyCoords)
        QBCore.Functions.Notify("Je hebt je vingers in het glas gesneden!", "error")
    end

    smashing = true

    QBCore.Functions.Progressbar("smash_vitrine", "Vitrine inslaan..", Config.WhitelistedWeapons[pedWeapon]["timeOut"], false, true, {
        ...
        TriggerServerEvent('qb-jewellery:server:PoliceAlertMessage', "Overval Juwelier", plyCoords, false)
        smashing = false

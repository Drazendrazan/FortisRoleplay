		...
		if closestScrapyard ~= 0 then
				...
				if IsPedInAnyVehicle(GetPlayerPed(-1)) then
					...
					if vehicle ~= 0 and vehicle ~= nil then 
						...
						if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 2.5 and not isBusy then
							DrawText3Ds(vehpos.x, vehpos.y, vehpos.z, "~g~E~w~ - Scrappen")
							if IsControlJustReleased(0, Keys["E"]) then
								if GetPedInVehicleSeat(vehicle, -1) == GetPlayerPed(-1) then
									if IsVehicleValid(GetEntityModel(vehicle)) then 
										ScrapVehicle(vehicle)
									else
										QBCore.Functions.Notify("Dit  voertuig kan niet gescrapt worden..", "error")
									end
								else
									QBCore.Functions.Notify("Je bent niet de bestuurder!", "error")
								end
							end
						end
					end
			...
			if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.Locations[closestScrapyard]["list"].x, Config.Locations[closestScrapyard]["list"].y, Config.Locations[closestScrapyard]["list"].z, true) < 1.5 then
				if not IsPedInAnyVehicle(GetPlayerPed(-1)) and not emailSend then
					DrawText3Ds(Config.Locations[closestScrapyard]["list"].x, Config.Locations[closestScrapyard]["list"].y, Config.Locations[closestScrapyard]["list"].z, "~g~E~w~ - Aankloppen")
					if IsControlJustReleased(0, Keys["E"]) then
	...
	if Config.CurrentVehicles ~= nil and next(Config.CurrentVehicles) ~= nil then 
		...
		SetTimeout(math.random(15000, 20000), function()
			...
			TriggerServerEvent('qb-phone:server:sendNewMail', {
				sender = "Pietje & Co B.V.",
				subject = "Voertuiglijst",
				message = "Er is werk aan de winkel! <br />Je mag de resten houden, rot voor de rest op.<br /><br /><strong>Voertuiglijst:</strong><br />".. vehicleList,
				button = {}
			})
		end)
	else
		QBCore.Functions.Notify("Je kan momenteel geen voertuigen scrappen..", "error")
	end
end
...
function ScrapVehicle(vehicle)
	...
	QBCore.Functions.Progressbar("scrap_vehicle", "Voertuig scrappen..", scrapTime, false, true, {

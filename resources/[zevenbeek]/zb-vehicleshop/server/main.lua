...
AddEventHandler('qb-vehicleshop:server:buyVehicle', function(vehicleData, garage)
    ...
    if (balance - vData["price"]) >= 0 then
        ...
        TriggerClientEvent("QBCore:Notify", src, "Gefeliciteerd! Je voertuig is afgeleverd bij "..QB.GarageLabel[garage], "success", 5000)
        pData.Functions.RemoveMoney('bank', vData["price"], "Voertuig gekocht")
        ...
        TriggerEvent("qb-log:server:CreateLog", "vehicleshop", "Cardealer", "green", "**" ..GetPlayerName(src) .. "** heeft succesvol een " .. vData["name"] .. " gekocht voor €" .. vData["price"])
    else
		TriggerClientEvent("QBCore:Notify", src, "Je hebt onvoldoende saldo, je mist €"..format_thousand(vData["price"] - balance), "error", 5000)
    end
end)
...
AddEventHandler('qb-vehicleshop:server:buyShowroomVehicle', function(vehicle, class)
    ...
    if (balance - vehiclePrice) >= 0 then
        ...
        TriggerClientEvent("QBCore:Notify", src, "Gefeliciteerd! Je voertuig wacht buiten op je.", "success", 5000)
        ...
        TriggerEvent("qb-log:server:CreateLog", "vehicleshop", "Cardealer", "green", "**"..GetPlayerName(src) .. "** heeft succesvol een " .. QBCore.Shared.Vehicles[vehicle]["name"] .. " gekocht voor €" .. QBCore.Shared.Vehicles[vehicle]["price"])
    else
        TriggerClientEvent("QBCore:Notify", src, "Je hebt onvoldoende saldo, je mist €"..format_thousand(vehiclePrice - balance), "error", 5000)
    end
end)
...
QBCore.Commands.Add("verkoop", "Verkoopt een voertuig aan het opgegeven ID.", {}, false, function(source, args)
    ...
    if Player.PlayerData.job.name == "cardealer" then
        ...
        TriggerClientEvent('QBCore:Notify', source, 'Je bent geen cardealer!', 'error')
    end
end)

QBCore.Commands.Add("testdrive", "Maakt een testrit aan.", {}, false, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
    local TargetId = args[1]
...
AddEventHandler('qb-vehicleshop:server:ConfirmVehicle', function(ShowroomVehicle)
    ...
    elseif Player.PlayerData.money.bank >= VehPrice then
        ...
        if Player.PlayerData.money.cash > Player.PlayerData.money.bank then
            TriggerClientEvent('QBCore:Notify', src, 'Je hebt onvoldoende saldo, je mist €('..(Player.PlayerData.money.cash - VehPrice)..',-)')
        else
            TriggerClientEvent('QBCore:Notify', src, 'Je hebt onvoldoende saldo, je mist €('..(Player.PlayerData.money.bank - VehPrice)..',-)')
        end
    end
end)
...
QBCore.Functions.CreateCallback('qb-vehicleshop:server:SellVehicle', function(source, cb, vehicle, plate)
    ...
    QBCore.Functions.ExecuteSql(false, "SELECT * FROM `player_vehicles` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."' AND `plate` = '"..plate.."'", function(result)
        if result[1] ~= nil then
            Player.Functions.AddMoney('bank', math.ceil(VehicleData["price"] / 100 * 50), "Voertuig verkocht")

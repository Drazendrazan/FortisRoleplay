...
function ButtonSelected(button)
    ...
    elseif this == "vehicles" then
		...
		elseif btn == "SUVs" then
			OpenMenu('suvs')
		elseif btn == "Bike's" then
			OpenMenu('motorcycles')
		elseif btn == "Vans" then
            OpenMenu('vans')
        elseif btn == "Modded" then
            OpenMenu('modded')
		end
	end
end
...
Citizen.CreateThread(function()
    ...
    for i = 1, #QB.ShowroomVehicles, 1 do
		...
		SetVehicleNumberPlateText(veh, i .. "VERKOOP")
    end
end)
                ...
                if VehicleData["shop"] == "pdm" then
                    ...
                    if SellDistance < 1 then
                        if not SellStarted then
                            DrawText3Ds(QB.QuickSell.x, QB.QuickSell.y, QB.QuickSell.z, '~g~E~w~ - Verkopen voor ~g~€'..math.ceil(VehicleData["price"] / 100 * 50))
                            if IsControlJustPressed(0, Keys["E"]) then
                                SellStarted = true
                            end
                        else
                            DrawText3Ds(QB.QuickSell.x, QB.QuickSell.y, QB.QuickSell.z, '~g~7~w~ - Bevestigen / ~r~8~w~ - Annuleren')
                            if IsControlJustPressed(0, Keys["7"]) or IsDisabledControlJustPressed(0, Keys["7"]) then
                                ...
                                QBCore.Functions.TriggerCallback('qb-vehicleshop:server:SellVehicle', function(SoldVehicle)
                                    if SoldVehicle then
                                        TriggerEvent("qb-phone-new:client:BankNotify", "Je hebt €"..math.ceil(VehicleData["price"] / 100 * 50).." ontvangen van Premium Deluxe Motorsports B.V")
        ...
        if ClosestVehicle ~= nil then
            if dist < 1.5 then
                if not QB.ShowroomVehicles[ClosestVehicle].inUse then
                    ...
                    if not QB.ShowroomVehicles[ClosestVehicle].inUse then
                        if not vehshop.opened then
                            if not buySure then
                                DrawText3Ds(QB.ShowroomVehicles[ClosestVehicle].coords.x, QB.ShowroomVehicles[ClosestVehicle].coords.y, QB.ShowroomVehicles[ClosestVehicle].coords.z + 1.8, '~g~G~w~ - Veranderen')
                            end
							if not buySure then
								DrawText3Ds(QB.ShowroomVehicles[ClosestVehicle].coords.x, QB.ShowroomVehicles[ClosestVehicle].coords.y, QB.ShowroomVehicles[ClosestVehicle].coords.z + 1.60, '~g~H~w~ - Proefrit')
							end
                            if not buySure then
                                DrawText3Ds(QB.ShowroomVehicles[ClosestVehicle].coords.x, QB.ShowroomVehicles[ClosestVehicle].coords.y, QB.ShowroomVehicles[ClosestVehicle].coords.z + 1.70, '~g~E~w~ - Kopen (~g~€'..vehPrice..'~w~)')
                            elseif buySure then
                        ...
                        elseif vehshop.opened then
                            if modelLoaded then
                                DrawText3Ds(QB.ShowroomVehicles[ClosestVehicle].coords.x, QB.ShowroomVehicles[ClosestVehicle].coords.y, QB.ShowroomVehicles[ClosestVehicle].coords.z + 1.65, 'Uitzoeken...')
                            else
                                DrawText3Ds(QB.ShowroomVehicles[ClosestVehicle].coords.x, QB.ShowroomVehicles[ClosestVehicle].coords.y, QB.ShowroomVehicles[ClosestVehicle].coords.z + 1.65, 'Voertuig wordt veranderd, een ogenblik geduld.')
                            end
                        end
                    else
                        DrawText3Ds(QB.ShowroomVehicles[ClosestVehicle].coords.x, QB.ShowroomVehicles[ClosestVehicle].coords.y, QB.ShowroomVehicles[ClosestVehicle].coords.z + 1.65, 'Voertuig wordt gebruikt door een andere klant.')
                    end
					...
					if IsControlJustPressed(0, Keys['H']) then
						if not vehshop.opened and not testrit then
							if not buySure then
								testrit = true
								startProefrit(QB.ShowroomVehicles[ClosestVehicle].chosenVehicle)
							end
						end
					end
                ...
                elseif QB.ShowroomVehicles[ClosestVehicle].inUse then
                    DrawText3Ds(QB.ShowroomVehicles[ClosestVehicle].coords.x, QB.ShowroomVehicles[ClosestVehicle].coords.y, QB.ShowroomVehicles[ClosestVehicle].coords.z + 0.5, 'Plek wordt gebruikt...')
                end
...
startProefrit = function(vehicle)
	local timer = 120
	local vehicletestrit = nil
	QBCore.Functions.SpawnVehicle(vehicle, function(veh)
		QBCore.Functions.Notify('Je hebt een testrit gestart met een ' .. vehicle .. ', je krijgt 2 minuten om hem in te leveren, anders zal hij automatisch worden verwijdert!', 'primary', 7500)
	...
	while timer > 0 do
		...
		if not IsPedInAnyVehicle(GetPlayerPed(-1), false) then
			...
			QBCore.Functions.Notify('Testrit geannuleerd!', 'error', 3500)
            QBCore.Functions.DeleteVehicle(vehicletestrit)
            testrit = false
			return
		end
		if timer < 60 then
			QBCore.Functions.Notify('Je hebt nog ' .. timer .. ' seconden!', 'primary', 1000)
		end
		timer = timer - 1
		if timer == 0 then
			QBCore.Functions.Notify('Testrit afgelopen!', 'error', 3500)
            QBCore.Functions.DeleteVehicle(vehicletestrit)
            testrit = false
			return
		end
...
AddEventHandler('qb-vehicleshop:client:setShowroomVehicle', function(showroomVehicle, k)
    if QB.ShowroomVehicles[k].chosenVehicle ~= showroomVehicle then
        ...
        SetVehicleDoorsLocked(veh, 3)
        FreezeEntityPosition(veh, true)
        SetVehicleNumberPlateText(veh, k .. "VERKOOP")
        modelLoaded = true

...
RegisterNetEvent("zb-security:server:triggeredDevTools")
AddEventHandler("zb-security:server:triggeredDevTools", function()
    local steam = GetPlayerName(source)
    TriggerEvent("qb-log:server:CreateLog", "inventoryGeef", "Dev Tools Blocker", "red", "**Steam:** "..steam)

    DropPlayer(source, "Gebruik van devtools is verboden op Zevenbeek!")
end)
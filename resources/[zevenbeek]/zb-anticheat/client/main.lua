        ...
        if group == Config.Group and isLoggedIn then 
            if not IsDecorating then 
                if not IsEntityVisible(ped) then
                    ...
                    TriggerEvent('QBCore:Notify', "QB-ANTICHEAT: You were invisble but now visible again!")
                    TriggerServerEvent("qb-log:server:CreateLog", "anticheat", "Made player visible", "green", "** @everyone " ..GetPlayerName(player).. "** was invisible and is visible again by the QB-Anticheat")            
                end 
            end
        end
    end

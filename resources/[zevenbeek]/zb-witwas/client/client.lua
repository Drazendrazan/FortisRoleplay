            ...
            if not gestart then
                if GetDistanceBetweenCoords(pedPositie, 1136.08, -988.23, 46.11, true) < 1 then
                    ...
                    if IsControlJustPressed(0, 38) then
                        QBCore.Functions.TriggerCallback("zb-witwas:server:checkAantal", function(aantal)
                            local aantal = aantal
                            ...
                            if aantal >= 1000 then
                                ...
                                TriggerServerEvent("zb-witwas:server:verwijderViesgeld", aantal)
                                ...
                                QBCore.Functions.Progressbar("witwassen", "Geld schoonmaken...", tijdsDuur, false, true, {
                                    ...
                                    TriggerServerEvent("zb-witwas:server:geefWitgeld", witgeld)
                                    QBCore.Functions.Notify("Je hebt je viesgeld schoongemaakt!", "success")
                                    gestart = false
                                end, function() -- Cancel
                                    TriggerServerEvent("zb-witwas:server:cancelWassen", aantal)
...
RegisterNetEvent("zb-witwas:client:gepaktZemmel")
AddEventHandler("zb-witwas:client:gepaktZemmel", function()
    -- Run wanneer de zemmel gecatched wordt :)
    TriggerServerEvent("zb-witwas:server:kauloHacker", "#ccc")
end)

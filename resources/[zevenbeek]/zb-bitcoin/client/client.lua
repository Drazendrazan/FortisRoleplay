...
RegisterNetEvent("zb-bitcoin:client:betreedHuis")
AddEventHandler("zb-bitcoin:client:betreedHuis", function(osso)
    TriggerEvent("zb-bitcoin:client:kastcoords")
    spawnServerKasten(osso)
    huidigHuis = osso
    inside = true
end)

RegisterNetEvent("zb-bitcoin:client:verlaatHuis")
AddEventHandler("zb-bitcoin:client:verlaatHuis", function(osso)
...
function spawnServerKasten(huidigHuis)
    QBCore.Functions.TriggerCallback('zb-bitcoin:server:vraagKastenOp', function(cb)
...
-- Kast plaatsen
RegisterNetEvent("zb-bitcoin:client:gebruikServerkast")
AddEventHandler("zb-bitcoin:client:gebruikServerkast", function()
    if inside then
        QBCore.Functions.TriggerCallback('zb-bitcoin:server:vraagHoeveelheidKastenOp', function(resultaat)
            if resultaat < 10 then
                ...
                QBCore.Functions.Progressbar("zb-bitcoin", "Serverkast aan het aansluiten...", 6500, false, true, {
                    ...
                    TriggerServerEvent("zb-bitcoin:server:slaKastOp", huidigHuis, json.encode(serverData["serverData"]))
                    refreshServerKasten(huidigHuis)

                end, function() -- Cancel
                    ExecuteCommand("e c")
                    TriggerServerEvent("zb-bitcoin:server:serverkast")
        ...
        TriggerServerEvent("zb-bitcoin:server:itemterug", "serverkast")
    end
end)
...
RegisterNUICallback("verwijderKast", function(data, cb)
    ...
    TriggerEvent("zb-bitcoin:client:verwijderServerKast", verwijderRackId)
end)

RegisterNetEvent("zb-bitcoin:client:verwijderServerKast")
AddEventHandler("zb-bitcoin:client:verwijderServerKast", function(verwijderRackId)
    local serverkast = {}
    ...
    QBCore.Functions.TriggerCallback('zb-bitcoin:server:zoekRackID', function(serverkast)
            ...
            if GetDistanceBetweenCoords(pedPos, coords.x, coords.y, coords.z, true) < 7 then
                ...
                if GetDistanceBetweenCoords(pedPos, coords.x, coords.y, coords.z, true) < 2 then
                    ...
                    if IsControlJustPressed(0, 38) then
                        QBCore.Functions.Progressbar("zb-bitcoin", "Serverkast aan het afsluiten...", 5000, false, true, {
                            disableMovement = true,
                            disableCarMovement = true,
                            disableMouse = false,
                            disableCombat = true,
                            ...
                            ClearPedTasks(PlayerPedId())
                            ExecuteCommand("e c")
                            TriggerServerEvent("zb-bitcoin:server:verwijderKast", verwijderRackId, huidigHuis)
                            refreshServerKasten(huidigHuis)
                            serverkast = nil
                        end, function() -- Cancel
                            ExecuteCommand("e c")
...
RegisterNetEvent("zb-bitcoin:client:repair")
AddEventHandler("zb-bitcoin:client:repair", function(type)
    if not aanHetRepairen then
        if inside and #serverKasten[huidigHuis] > 0 then        
            ...
            if closestKastID ~= 0 and closestKastBetween < 3 then
                if closestKastID ~= 0 then
                    aanHetRepairen = true
                    QBCore.Functions.Progressbar("zb-bitcoin", type.." vervangen...", 5000, false, false, {
                        disableMovement = true,
                        disableCarMovement = true,
                        disableMouse = false,
                        disableCombat = true,
                        ...
                        TriggerServerEvent("zb-bitcoin:server:vervangKastDone", closestKastID, type)
                        ClearPedTasks(PlayerPedId())
                ...
                TriggerServerEvent("zb-bitcoin:server:retourOnderdeel", type)
            end
        else
            TriggerServerEvent("zb-bitcoin:server:retourOnderdeel", type)
        end
        ...
        TriggerServerEvent("zb-bitcoin:server:itemterug", type)
    end
end)
...
RegisterNetEvent("zb-bitcoin:client:openlaptop")
AddEventHandler("zb-bitcoin:client:openlaptop", function()
    if inside then
        SendNUIMessage({
            type = "open",
        })
...
RegisterNUICallback("minenUitbetalenCrypto", function(data, cb)
    TriggerServerEvent("zb-bitcoin:server:coinsBetalen", huidigHuis)
end)
...
function betaalVoorProducten(producten)
    ...
    QBCore.Functions.TriggerCallback("zb-bitcoin:sever:betaalProducten", function(heeftBetaald)
            ...
            elseif leverBlipGeplaatst then
                ...
                if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), -661.80, -861.62, 24.49, true) < 10 then
                    ...
                    if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), -661.80, -861.62, 24.49, true) < 1.3 then
                        ...
                        if IsControlJustPressed(0, 38) then
                            TriggerServerEvent("zb-bitcoin:server:ontvangLevering", bestelling)

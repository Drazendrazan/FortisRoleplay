...
Citizen.CreateThread(function()
    for k, v in pairs(QBCustoms.Locations) do
        ...
        AddTextComponentString("Voertuigentuning")
...
RegisterNUICallback('CanRepairVehicle', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('qb-customs:server:CanPurchase', function(CanBuy)
        if CanBuy then
            ...
            QBCore.Functions.Notify('Je hebt onvoldoende saldo!', 'error')
            cb(false)
        end
    end, data.price)
end)
...
function GetAvailableMods()
    ...
    if QBCore.Shared.Vehicles[GetEntityModel(veh)] ~= nil then
        model = QBCore.Shared.Vehicles[GetEntityModel(veh)].model
        ...
        if CanBuy then
            ...
            if ShoppingCart.buttons ~= nil and next(ShoppingCart.buttons) ~= nil then
                for k, v in pairs(ShoppingCart.buttons) do
                    ...
                    if v.modtype ~= nil then
                        if v.modtype ~= "respray" and v.modtype ~= "wheels" and v.modtype ~= "wheelcolor" and v.modtype ~= "wheelaccessories" and v.modtype ~= "frontwheels" and v.modtype ~= "backwheels" and v.modtype ~= "liveries" and v.modtype ~= 11 then
                        ...
                        elseif v.modtype == 11 then -- Engine
                            SetVehicleMod(veh, 11, v.modid)
                        end
                    end
                end
                ...
                TriggerServerEvent("qb-log:server:CreateLog", "bennys", "Upgrades gekocht", "green", "**" .. PlayerData.name .. "** (citizenid: *" .. PlayerData.citizenid .. "* | id: *(" .. PlayerData.source .. ")* heeft **€" .. TotalPrice .. "** betaald voor de volgende upgrades: \n" .. String)
            end
            ...
            TriggerServerEvent('qb-customs:server:SaveVehicleProps', CurrentVehicleData, plate)
        else
            QBCore.Functions.Notify('Je hebt onvoldoende saldo!', 'error')
        end
    end, TotalPrice)

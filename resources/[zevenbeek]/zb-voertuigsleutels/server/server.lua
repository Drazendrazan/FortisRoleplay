...
RegisterServerEvent("zb-voertuigsleutels:server:setVoertuigOwner")
AddEventHandler("zb-voertuigsleutels:server:setVoertuigOwner", function(kenteken)
    local src = source
...
QBCore.Functions.CreateCallback("zb-voertuigsleutels:server:checkOwner",function(source, cb, kenteken)
...
RegisterServerEvent("zb-voertuigsleutels:server:verwijderSleutel")
AddEventHandler("zb-voertuigsleutels:server:verwijderSleutel", function(kenteken)
...
RegisterServerEvent("zb-voertuigsleutels:server:geefSleutels")
AddEventHandler("zb-voertuigsleutels:server:geefSleutels", function(kenteken, target)
    local src = source
    local kenteken = kenteken
    local target = target
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if QBCore.Functions.GetPlayer(target) ~= nil then
        ...
        if resultaat then
            -- TriggerEvent("zb-voertuigsleutels:server:setVoertuigOwner", target, kenteken)
...
QBCore.Commands.Add("geefsleutels", "Geeft de sleutels van het voertuig.", {{name = "id", help = "ID"}}, true, function(source, args)
    ...
    TriggerClientEvent('zb-voertuigsleutels:client:geefSleutels', src, target)
end)

QBCore.Commands.Add("motor", "Start/stopt de motor van de auto.", {}, true, function(source, args)
	local src = source
    TriggerClientEvent('zb-voertuigsleutels:client:startStopMotor', src)
    dump(sleutels)
end)

QBCore.Commands.Add("adminsleutel", "Krijg sleutel van het voertuig naast je (staff)", {}, false, function(source, args)
    local src = source
    TriggerClientEvent("zb-voertuigsleutels:server:maakSleutelMogelijk", src)
    TriggerClientEvent("zb-voertuigsleutels:client:adminSleutel", src)
end, "admin")

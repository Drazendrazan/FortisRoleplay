    ...
    while true do
        ...
        if IsControlJustPressed(1, 182) then
            ...
            if GetDistanceBetweenCoords(pedPos, voertuigPos) < 4 then
                ...
                QBCore.Functions.TriggerCallback("zb-voertuigsleutels:server:checkOwner", function(callback)
                    if callback then
...
AddEventHandler("vehiclekeys:client:SetOwner", function(kenteken)
    ...
    TriggerServerEvent("zb-voertuigsleutels:server:setVoertuigOwner", kenteken)
end)

RegisterNetEvent("zb-voertuigsleutels:server:maakSleutelMogelijk")
AddEventHandler("zb-voertuigsleutels:server:maakSleutelMogelijk", function()
    laatsteVoertuig = nil
end)

RegisterNetEvent('zb-voertuigsleutels:client:geefSleutels')
AddEventHandler('zb-voertuigsleutels:client:geefSleutels', function(target)
    if IsPedInAnyVehicle(GetPlayerPed(-1)) then
        local kenteken = GetVehicleNumberPlateText(GetVehiclePedIsIn(GetPlayerPed(-1), false))
        TriggerServerEvent('zb-voertuigsleutels:server:geefSleutels', kenteken, target)
    else
...
RegisterNetEvent("zb-voertuigsleutels:client:adminSleutel")
AddEventHandler("zb-voertuigsleutels:client:adminSleutel", function()
    local pedPos = GetEntityCoords(GetPlayerPed(-1))
    local voertuig = QBCore.Functions.GetClosestVehicle()
    local voertuigPos = GetEntityCoords(voertuig)
...
RegisterNetEvent("zb-voertuigsleutels:client:startStopMotor")
AddEventHandler("zb-voertuigsleutels:client:startStopMotor", function()

...
QBCore.Functions.CreateCallback('zb-lapraces:server:GetRacingLeaderboards', function(source, cb)
    cb(Races)
end)
...
RegisterServerEvent('zb-lapraces:server:FinishPlayer')
AddEventHandler('zb-lapraces:server:FinishPlayer', function(RaceData, TotalTime, TotalLaps, BestLap)
    local src = source
    ...
    TriggerClientEvent('zb-lapraces:client:PlayerFinishs', -1, RaceData.RaceId, PlayersFinished, Player)
...
RegisterServerEvent('zb-lapraces:server:CreateLapRace')
AddEventHandler('zb-lapraces:server:CreateLapRace', function(RaceName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    
    if IsWhitelisted(Player.PlayerData.citizenid) then
        if IsNameAvailable(RaceName) then
            TriggerClientEvent('zb-lapraces:client:StartRaceEditor', source, RaceName)
        else
...
QBCore.Functions.CreateCallback('zb-lapraces:server:GetRaces', function(source, cb)
    cb(AvailableRaces)
end)

QBCore.Functions.CreateCallback('zb-lapraces:server:GetListedRaces', function(source, cb)
    cb(Races)
end)

QBCore.Functions.CreateCallback('zb-lapraces:server:GetRacingData', function(source, cb, RaceId)
    cb(Races[RaceId])
end)

QBCore.Functions.CreateCallback('zb-lapraces:server:HasCreatedRace', function(source, cb)
    cb(HasOpenedRace(QBCore.Functions.GetPlayer(source).PlayerData.citizenid))
end)

QBCore.Functions.CreateCallback('zb-lapraces:server:IsAuthorizedToCreateRaces', function(source, cb, TrackName)
...
QBCore.Functions.CreateCallback('zb-lapraces:server:GetTrackData', function(source, cb, RaceId)
...
RegisterServerEvent('zb-lapraces:server:JoinRace')
AddEventHandler('zb-lapraces:server:JoinRace', function(RaceData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if CurrentRace ~= nil then
        ...
        if (AmountOfRacers - 1) == 0 then
            ...
            TriggerClientEvent('zb-lapraces:client:LeaveRace', src, Races[CurrentRace])
        else
    ...
    TriggerClientEvent('zb-lapraces:client:JoinRace', src, Races[RaceId], RaceData.Laps)
    TriggerClientEvent('qb-phone_new:client:UpdateLapraces', -1)
...
RegisterServerEvent('zb-lapraces:server:LeaveRace')
AddEventHandler('zb-lapraces:server:LeaveRace', function(RaceData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local RaceName
    ...
    if (AmountOfRacers - 1) == 0 then
        ...
        TriggerClientEvent('zb-lapraces:client:LeaveRace', src, Races[RaceId])
...
RegisterServerEvent('zb-lapraces:server:SetupRace')
AddEventHandler('zb-lapraces:server:SetupRace', function(RaceId, Laps)
                    ...
                    if Races[RaceId].Waiting then
                        ...
                        for cid,_ in pairs(Races[RaceId].Racers) do
                            ...
                            if RacerData ~= nil then
                                TriggerClientEvent('zb-lapraces:client:LeaveRace', RacerData.PlayerData.source, Races[RaceId])
                            end
                        end
                        table.remove(AvailableRaces, AvailableKey)
...
RegisterServerEvent('zb-lapraces:server:UpdateRaceState')
AddEventHandler('zb-lapraces:server:UpdateRaceState', function(RaceId, Started, Waiting)
    Races[RaceId].Waiting = Waiting
    Races[RaceId].Started = Started
end)

RegisterServerEvent('zb-lapraces:server:UpdateRacerData')
AddEventHandler('zb-lapraces:server:UpdateRacerData', function(RaceId, Checkpoint, Lap, Finished)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('zb-lapraces:client:UpdateRaceRacerData', -1, RaceId, Races[RaceId])
end)

RegisterServerEvent('zb-lapraces:server:StartRace')
AddEventHandler('zb-lapraces:server:StartRace', function(RaceId)
    local src = source
    ...
    if RaceId ~= nil then
        if AvailableRaces[AvailableKey].SetupCitizenId == MyPlayer.PlayerData.citizenid then
            ...
            for CitizenId,_ in pairs(Races[RaceId].Racers) do
                local Player = QBCore.Functions.GetPlayerByCitizenId(CitizenId)
                if Player ~= nil then
                    TriggerClientEvent('zb-lapraces:client:RaceCountdown', Player.PlayerData.source)
                end
            end
            TriggerClientEvent('qb-phone_new:client:UpdateLapraces', -1)
        else
...
RegisterServerEvent('zb-lapraces:server:SaveRace')
AddEventHandler('zb-lapraces:server:SaveRace', function(RaceData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('zb-lapraces:server:CanRaceSetup', function(source, cb)

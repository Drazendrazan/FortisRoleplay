...
RegisterNetEvent("zb-radio:client:openRadio")
AddEventHandler("zb-radio:client:openRadio", function()
    SendNUIMessage({
        type = "open"
    })
...
RegisterNUICallback("verbind", function(data)
    ...
    if frequentie >= 1 and frequentie <= 1000 and frequentie ~= portofoonFrequentie then
        QBCore.Functions.TriggerCallback("zb-radio:server:checkRadioFrequentie", function(resultaat)
            if resultaat then
                exports["pma-voice"]:setVoiceProperty("radioEnabled", true)
                exports["pma-voice"]:setRadioChannel(frequentie)
                TriggerServerEvent("zb-radio:server:verbindFrequentie", frequentie, data.naam, portofoonOudeFrequentie)
...
RegisterNUICallback("verbreek", function() 
    ...
    TriggerServerEvent("zb-radio:server:verbreekFrequentie", portofoonFrequentie)
    SendNUIMessage({
        type = "verbreek"
    })
    portofoonFrequentie = 0
...
RegisterNetEvent("zb-radio:client:refreshLijst")
AddEventHandler("zb-radio:client:refreshLijst", function(frequentie, lijst)
                    ...
                    if frequentie >= 1 and frequentie <= 1000 then
                        QBCore.Functions.TriggerCallback("zb-radio:server:checkRadioFrequentie", function(resultaat)
                            if resultaat then
                                exports["pma-voice"]:setVoiceProperty("radioEnabled", true)
                                exports["pma-voice"]:setRadioChannel(frequentie)
                                TriggerServerEvent("zb-radio:server:verbindFrequentie", frequentie, portofoonNaam, portofoonOudeFrequentie)
                                portofoonFrequentie = frequentie
                                portofoonOudeFrequentie = frequentie
                            else
                                QBCore.Functions.Notify("Je bent niet toegestaan deze frequentie te joinen!", "error")

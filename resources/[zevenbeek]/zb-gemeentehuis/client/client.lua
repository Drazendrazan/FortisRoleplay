...
RegisterNUICallback("krijgJob", function(data, cb)
    if inComputer then
        if data.baan ~= "police" and data.baan ~= "ambulance" and data.baan ~= "mechanic" then
            ...
            TriggerServerEvent("zb-gemeentehuis:server:setJob", baan)
        else
            -- Chappie probeert overheids baan
            TriggerServerEvent("zb-gemeentehuis:server:kauloHacker", "#aaa")
        end
    else
        -- Chappie zit niet eens in lappie
        TriggerServerEvent("zb-gemeentehuis:server:kauloHacker", "#bbb")
    end
end)

RegisterNetEvent("zb-gemeentehuis:client:gepaktZemmel")
AddEventHandler("zb-gemeentehuis:client:gepaktZemmel", function()
    -- Run wanneer de zemmel gecatched wordt :)
    TriggerServerEvent("zb-gemeentehuis:server:kauloHacker", "#ccc")
end)

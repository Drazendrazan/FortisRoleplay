...
QBCore.Functions.CreateCallback("zb-shisha:server:reserveerPlek", function(source, cb, plek)
    local src = source
...
RegisterNetEvent("zb-shisha:server:maakPlekVrij")
AddEventHandler("zb-shisha:server:maakPlekVrij", function(plek)
    local src = source
    Config.loungeLocaties[plek].actief = false
end)

RegisterNetEvent("zb-shisha:server:betaal")
AddEventHandler("zb-shisha:server:betaal", function(prijs)
    local src = source

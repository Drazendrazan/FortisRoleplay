...
QBCore.Functions.CreateUseableItem("tunerlaptop", function(source, item)
    TriggerClientEvent('zb-tunerchip:client:openChip', source)
end)

RegisterServerEvent('zb-tunerchip:server:TuneStatus')
AddEventHandler('zb-tunerchip:server:TuneStatus', function(plate, bool)
    if bool then
...
RegisterServerEvent("zb-tunerchip:server:foei")
AddEventHandler("zb-tunerchip:server:foei", function()
    local src = source
    DropPlayer(src, 'Foei zo makkelijk gaat het abusen van de tunerchip niet, groetjes Damian aka fortis staff!')
end)

QBCore.Functions.CreateCallback("zb-tunerchip:server:HasChip", function(source, cb)
    local src = source
...
QBCore.Functions.CreateCallback('zb-tunerchip:server:GetStatus', function(source, cb, plate)

                            ...
                            if not Config.SmallBanks[closestBank]["lockers"][k]["isOpened"] then
                                if lockerDist < 5 then
                                    ...
                                    if lockerDist < 0.5 then
                                        DrawText3Ds(Config.SmallBanks[closestBank]["lockers"][k].x, Config.SmallBanks[closestBank]["lockers"][k].y, Config.SmallBanks[closestBank]["lockers"][k].z + 0.3, '~g~E~w~ - Kraken')
    ...
    if closestBank ~= nil then
        QBCore.Functions.TriggerCallback('qb-bankrobbery:server:isRobberyActive', function(isBusy)
            if not isBusy then
                ...
                QBCore.Functions.Notify("Het tijdslot zit er nog op, probeer het later nog eens!", "error", 5500)
            end
        end)
    end
end)
...
function openLocker(bankId, lockerId)
    ...
    if bankId == "paleto" then
        QBCore.Functions.TriggerCallback('qb-bankrobbery:server:GetItem', function(hasItem)
            if hasItem then
                ...
                IsDrilling = true
                QBCore.Functions.Progressbar("open_locker_drill", "Kluis kraken...", 60000, false, true, {
                    disableMovement = true,
                    disableCarMovement = true,
                    disableMouse = false,
                    disableCombat = true,
                    ...
                    QBCore.Functions.Notify("Er lag iets in de kluis!", "success")
                    IsDrilling = false
                end, function() -- Cancel
                    StopAnimTask(GetPlayerPed(-1), "anim@heists@fleeca_bank@drilling", "drill_straight_idle", 1.0)
                    TriggerServerEvent('qb-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
                ...
                QBCore.Functions.Notify("Je hebt hier een drillboor voor nodig!", "error")
                TriggerServerEvent('qb-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
            end
        end, "drill")
    else
        IsDrilling = true
        QBCore.Functions.Progressbar("open_locker", "Kluis kraken...", math.random(24000, 48000), false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
...
AddEventHandler('qb-bankrobbery:client:robberyCall', function(type, key, streetLabel, coords)
    if PlayerJob.name == "police" and onDuty then 
        ...
        AddTextComponentString("112 - Poging bankoverval")

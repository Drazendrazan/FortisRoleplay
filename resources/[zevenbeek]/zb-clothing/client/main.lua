                    ...
                    if not creatingCharacter then
                        ...
                        if dist < 5 then
                            ...
                            elseif Config.Stores[k].shopType == "barber" then
                                DrawText3Ds(Config.Stores[k].x, Config.Stores[k].y, Config.Stores[k].z + 1.25, '~g~E~w~ - Kapper')
                            end
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    ...
                                    QBCore.Functions.TriggerCallback('qb-clothing:server:getOutfits', function(result)
                                        openMenu({
                                            ...
                                            {menu = "accessoires", label = "Accesoires", selected = false}
                                        })
...
RegisterNUICallback('TrackerError', function()
    TriggerEvent('chatMessage', "SYSTEM", "error", "Je kan je enkelband niet afdoen!")
end)
...
AddEventHandler('qb-clothes:client:CreateFirstCharacter', function()
    QBCore.Functions.GetPlayerData(function(PlayerData)
		...
		if PlayerData.charinfo.gender == "vrouw" then 
...
checkBodyArmor = function(r)
	for k,v in pairs(whitelistedVest) do
		if k == r then
			SetPedArmour(GetPlayerPed(-1), v.armor)
			return
		end
	end
	return
...
AddEventHandler('qb-clothing:client:loadOutfit', function(oData)
    ...
    if oData.outfitName ~= nil then
        TriggerEvent('chatMessage', "SYSTEM", "warning", "Je hebt "..oData.outfitName.." gekozen! Druk bevestig om om te kleden.")
    end
end)

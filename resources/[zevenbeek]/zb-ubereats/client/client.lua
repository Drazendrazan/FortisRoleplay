                    ...
                    if IsControlJustPressed(0, 38) then
                        if ophalenMogelijk == false then
                            QBCore.Functions.Progressbar("verzamel", "Bestelling ophalen...", 7000, false, true, {
                                ...
                                TriggerServerEvent("zb-ubereats:server:addBestelling")
                                TriggerEvent("zb-ubereats:client:krijgLocatie")
...
AddEventHandler("zb-ubereats:client:krijgLocatie", function()
                    ...
                    if missieDistance < 2 then
                        ...
                        if IsControlJustPressed(0, 38) then
                            if IsPedInAnyVehicle(missiePed, false) then
                                ...
                                QBCore.Functions.TriggerCallback("zb-ubereats:server:requestBestelling", function(aantal)
                                    if aantal < 1 then
                                        QBCore.Functions.Notify("Je hebt de bestelling niet bij je!", "error")
                                    else
                                        TriggerServerEvent("zb-ubereats:server:removeBestelling")

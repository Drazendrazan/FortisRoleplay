            ...
            if (Coins > 1000) {
                ...
                if (QB.Phone.Data.PlayerData.money.crypto >= 1000) {
                    ...
                    if ((parseInt(QB.Phone.Data.PlayerData.money.crypto) + parseInt(Coins)) > 1000) {
                        ...
                        $.post('https://zb-phone/BuyCrypto', JSON.stringify({
                            Coins: Coins,
                            Price: Price,
                        }), function(CryptoData){
                            if (CryptoData !== false) {
                                ...
                                QB.Phone.Notifications.Add("fas fa-university", "zb-bank", "&euro; "+Price+",- is er van je bank afgeschreven!", "#badc58", 2500);
                                setTimeout(() => {
                                    document.getElementById("buy-crypto").disabled = false;
                                }, 1000);
                            } else {
...
$(document).on('click', '#sell-crypto', function(e){
    ...
    if ((Coins !== "") && (Price !== "")) {
        if (CryptoData.Portfolio >= parseInt(Coins)) {
            $.post('https://zb-phone/SellCrypto', JSON.stringify({
                Coins: Coins,
                Price: Price,
            }), function(CryptoData){
                if (CryptoData !== false) {
                    ...
                    CloseCryptoPage()
                    QB.Phone.Data.PlayerData.money.bank = parseInt(QB.Phone.Data.PlayerData.money.bank) + parseInt(Price);
                    QB.Phone.Notifications.Add("fas fa-bitcoin", "zb-bank", "&euro; "+Price+",- is toegevoegd aan de bank!", "#badc58", 2500);
                } else {
    ...
    if ((Coins !== "") && (WalletId !== "")) {
        if (CryptoData.Portfolio >= Coins) {
            if (WalletId !== CryptoData.WalletId) {
                $.post('https://zb-phone/TransferCrypto', JSON.stringify({
                    Coins: Coins,
                    WalletId: WalletId,
                }), function(CryptoData){

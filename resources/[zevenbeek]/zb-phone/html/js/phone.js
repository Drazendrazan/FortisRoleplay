...
var CurrentFooterTab = "contacten";
var CallData = {};
...
$(document).on('click', '.phone-app-footer-button', function(e){
    ...
    if (PressedFooterTab !== CurrentFooterTab) {
        ...
        if (PressedFooterTab == "recent") {
            $.post('https://zb-phone/ClearRecentAlerts');
...
$(document).on('click', '.phone-recent-call', function(e){
    ...
    $.post('https://zb-phone/CallContact', JSON.stringify({
        ContactData: cData,
                ...
                if (status.CanCall) {
                    if (!status.InCall) {
                        if (QB.Phone.Data.AnonymousCall) {
                            QB.Phone.Notifications.Add("fas fa-phone", "Phone", "Je hebt een anonieme oproep gestart!");
                        }
                        ...
                        QB.Phone.Notifications.Add("fas fa-phone", "Phone", "Je bent al in gesprek!");
                    }
                } else {
                    QB.Phone.Notifications.Add("fas fa-phone", "Phone", "De persoon is al in gesprek!");
                }
            } else {
                QB.Phone.Notifications.Add("fas fa-phone", "Phone", "De persoon is niet beschikbaar!");
            }
        } else {
            QB.Phone.Notifications.Add("fas fa-phone", "Phone", "Je kan jezelf niet bellen!");
        }
    });
});
        ...
        if (cData.number !== QB.Phone.Data.PlayerData.charinfo.phone) {
            if (status.IsOnline) {
                if (status.CanCall) {
                    ...
                    QB.Phone.Notifications.Add("fas fa-phone", "Phone", "Deze persoon is al in gesprek!");
                }
            } else {
...
QB.Phone.Functions.LoadContacts = function(myContacts) {
    ...
    if (myContacts !== null) {
        $.each(myContacts, function(i, contact){
            ContactElement = '<div class="phone-contact" data-contactid="'+i+'"><div class="phone-contact-firstletter" style="background-color: #2986cc;">'+((contact.name).charAt(0)).toUpperCase()+'</div><div class="phone-contact-name">'+contact.name+'</div><div class="phone-contact-actions"><i class="fas fa-sort-down"></i></div><div class="phone-contact-action-buttons"> <i class="fas fa-phone-volume" id="phone-start-call"></i> <i class="fab fa-whatsapp" id="new-chat-phone" style="font-size: 2.5vh;"></i> <i class="fas fa-user-edit" id="edit-contact"></i> </div></div>'
...
$(document).on('click', '#new-chat-phone', function(e){
    ...
    if (ContactData.number !== QB.Phone.Data.PlayerData.charinfo.phone) {
        ...
        setTimeout(function(){
            ...
            $.post('https://zb-phone/GetWhatsappChat', JSON.stringify({phone: ContactData.number}), function(chat){
        ...
        QB.Phone.Notifications.Add("fa fa-phone-alt", "Whatsapp", "Je kan geen gesprek starten met jezelf..", "default", 3500);
    }
});
...
$(document).on('click', '#edit-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        $.post('https://zb-phone/EditContact', JSON.stringify({
        ...
        QB.Phone.Notifications.Add("fas fa-exclamation-circle", "Telefoon", "Vul alle velden in!");
    }
});
...
$(document).on('click', '#edit-contact-delete', function(e){
    ...
    $.post('https://zb-phone/DeleteContact', JSON.stringify({
        CurrentContactName: ContactName,
        CurrentContactNumber: ContactNumber,
        CurrentContactIban: ContactIban,
    }), function(PhoneContacts){
...
$(document).on('click', '#add-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        $.post('https://zb-phone/AddNewContact', JSON.stringify({
        ...
        if (SelectedSuggestion !== null) {
            $.post('https://zb-phone/RemoveSuggestion', JSON.stringify({
    ...
    $.post('https://zb-phone/CallContact', JSON.stringify({
        ...
        if (cData.number !== QB.Phone.Data.PlayerData.charinfo.phone) {
            if (status.IsOnline) {
                ...
                QB.Phone.Notifications.Add("fas fa-phone", "Telefoon", "De persoon is momenteel niet beschikbaar!");
            }
        } else {
...
$(document).on('click', '#outgoing-cancel', function(e){
    ...
    $.post('https://zb-phone/CancelOutgoingCall');
});

$(document).on('click', '#incoming-deny', function(e){
    e.preventDefault();

    $.post('https://zb-phone/DenyIncomingCall');
});

$(document).on('click', '#ongoing-cancel', function(e){
    e.preventDefault();
    
    $.post('https://zb-phone/CancelOngoingCall');
});


// Dit is voor saus
RemoveIncomingCallAlert = function() {
    ...
    QB.Phone.Data.CallActive = false;
    QB.Phone.Data.currentApplication = null;
    attemptCallieBingus = 0;
}

var attemptCallieBingus = 0;
IncomingCallAlert = function(CallData, Canceled, AnonymousCall) {
    if (!Canceled) {
        if (!QB.Phone.Data.CallActive) {
            attemptCallieBingus = attemptCallieBingus + 1;
            if (attemptCallieBingus === 10) {
                attemptCallieBingus = 0;
                setTimeout(() => {
                    RemoveIncomingCallAlert();
                }, 4000);
            }
            ...
            setTimeout(function(){
                ...
                if (AnonymousCall) {
                    Label = "Je hebt een inkomende oproep van Anoniem"
                }
...
QB.Phone.Functions.SetupCurrentCall = function(cData) {
    if (cData.InCall) {
        ...
        if (cData.CallType == "incoming") {
            ...
            $(".phone-currentcall-title").html("Bellen...");
...
$(document).on('click', '#incoming-answer', function(e){
    ...
    $.post('https://zb-phone/AnswerCall');
});

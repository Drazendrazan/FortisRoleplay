...
AddEventHandler('qb-phone:server:sendNewMail', function(mailData, fromServerSource)
    if fromServerSource ~= nil then
        -- Dit is EIGENLIJK alleen voor het zb-groothandel script
...
QBCore.Functions.CreateCallback('qb-phone:server:SendAnwbFactuur', function(source, cb, data)
    ...
    if Player.PlayerData.job ~= nil and Player.PlayerData.job.name == "mechanic" then
        ...
        TriggerEvent("zb-phone:server:banSpeler", source)
    end 
end)

RegisterNetEvent("zb-phone:server:banSpeler")
AddEventHandler("zb-phone:server:banSpeler", function(source)
    local src = source

...
QBCore.Commands.Add("trailer", "Plaatst het geselecteerde voertuig op de trailer.", {}, false, function(source, args)
    ...
    if Player.PlayerData.job.name == "tow" or Player.PlayerData.job.name == "mechanic" then
        TriggerClientEvent("zb-anwbmissies:client:trailer", source)
    end
end) 
...
QBCore.Functions.CreateCallback("zb-anwbmissies:server:check", function(source, cb, kenteken, hash, spawnNaam)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback("zb-anwbmissies:server:checkNeon", function(source, cb, kenteken, hash, spawnNaam)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('zb-anwbmissies:server:updateVehicleStatus')
AddEventHandler('zb-anwbmissies:server:updateVehicleStatus', function(plate, props)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if props ~= 0 then
...
RegisterServerEvent('zb-anwbmissies:server:geefuitrusting')
AddEventHandler('zb-anwbmissies:server:geefuitrusting', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('zb-anwbmissies:server:geeftunerofnitro')
AddEventHandler('zb-anwbmissies:server:geeftunerofnitro', function(item)
    print(item)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local item = item

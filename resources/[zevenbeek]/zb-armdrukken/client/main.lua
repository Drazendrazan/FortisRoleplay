...
AddEventHandler('evy_arm:start_cl', function()
  ...
  if place == 1 then
    ...
    while grade >= 0.10 and grade <= 0.90 do
      ...
      --   QBCore.Functions.Notify("Druk snel op "~INPUT_PICKUP~" om te winnen", "success")
      --   melding = 1
      -- end
      SetEntityAnimSpeed(PlayerPedId(), "mini@arm_wrestling", "sweep_a", 0.0)
      SetEntityAnimCurrentTime(PlayerPedId(), "mini@arm_wrestling", "sweep_a", grade)
    ...
    if grade >= 0.90 then
      ...
      while resultaatMelding == 0 do
        QBCore.Functions.Notify("Je hebt gewonnen!!", "success")
        resultaatMelding = 1
      end
    ...
    elseif grade <= 0.10 then
      ...
      while resultaatMelding == 0 do
        QBCore.Functions.Notify("Je hebt helaas verloren!", "error")
        resultaatMelding = 1
      end
...
AddEventHandler('evy_arm:check_cl', function(args)
  ...
  if args == 'place1' then
    ...
    while not started do
      ...
      while melding == 0 do
        QBCore.Functions.Notify("Aan het wachten op een tegenstander")
        melding = 1
      end
  ...
  elseif args == 'place2' then
    ...
    while not started do
      ...
      Wait(0)
      -- alert(text[globalConfig.language]['wait'])
      QBCore.Functions.Notify("Aan het wachten op een tegenstander", "success")
  ...
  elseif args == 'noplace' then
    QBCore.Functions.Notify("Deze tafel is vol!", "error")
  end
end)

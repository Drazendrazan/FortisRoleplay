...
AddEventHandler('qb-pings:client:DoPing', function(id)
    ...
    if not exports['police']:IsHandcuffed() then
        ...
        QBCore.Functions.Notify('Je kan momenteel niemand pingen!', 'error')
    end
end)
...
AddEventHandler('qb-pings:client:AcceptPing', function(PingData, SenderData)
    ...
    if not exports['police']:IsHandcuffed() then
        ...
        QBCore.Functions.Notify('Je kan momenteel geen ping accepteren!', 'error')
    end
end)

RegisterNetEvent('qb-pings:client:SendLocation')
AddEventHandler('qb-pings:client:SendLocation', function(PingData, SenderDatam, id)
    QBCore.Functions.Notify('De locatie is ingesteld op je GPS!', 'success')
    ...
    AddTextComponentSubstringPlayerName(id)
    EndTextCommandSetBlipName(CurrentPings[PingData.sender])

    SetTimeout(5 * (60 * 1000), function()
        QBCore.Functions.Notify('De ping van '..PingData.sender..' is verlopen..', 'error')

...
function CreateLog(Type, Log, Data)
    QBCore.Functions.TriggerCallback('pg-drugs/server/create-log', function(result)
    end, Type, Log, Data)
end

function ToggleDevMode(Bool)
    TriggerEvent('pg-drugs:client:ToggleDevmode')
    if Bool then
        while Bool do
            Wait(200)
...
function GetPlayersInArea(Coords, Radius)
	...
	QBCore.Functions.TriggerCallback('pg-drugs/server/get-active-players-in-radius', function(Players)
...
function GetBans()
    local Prom = promise:new()
    QBCore.Functions.TriggerCallback("pg-drugs/server/get-bans", function(Bans)
        Prom:resolve(Bans)
    end)
    return Citizen.Await(Prom)
end
...
function GetPlayers()
    local Prom = promise:new()
    QBCore.Functions.TriggerCallback("pg-drugs/server/get-players", function(Players)
        Prom:resolve(Players)
    end)
    return Citizen.Await(Prom)
end
...
function GetLogs()
    local Prom = promise:new()
    QBCore.Functions.TriggerCallback("pg-drugs/server/get-logs", function(Logs)
        Prom:resolve(Logs)
    end)
    return Citizen.Await(Prom)
end

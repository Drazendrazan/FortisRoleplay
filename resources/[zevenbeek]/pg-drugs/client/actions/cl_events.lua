...
RegisterNetEvent("Admin:Godmode", function(Result)
    ...
    TriggerServerEvent('pg-drugs/server/toggle-godmode', Result['player'])
end)
...
RegisterNetEvent('Admin:Spawn:Vehicle', function(Result)
    ...
    TriggerEvent('PGCore:Command:SpawnVehicle', Result['model'])
end)
...
RegisterNetEvent('Admin:Teleport:Marker', function(Result)
    ...
    TriggerEvent('PGCore:Command:GoToMarker')
end)
...
RegisterNetEvent('Admin:Teleport', function(Result)
    ...
    TriggerServerEvent('pg-drugs/server/teleport-player', Result['player'], Result['type'])
end)

RegisterNetEvent("Admin:Chat:Say", function(Result)
    if not IsPlayerAdmin() then return end

    TriggerServerEvent('pg-drugs/server/chat-say', Result['message'])
end)
...
RegisterNetEvent('Admin:Open:Clothing', function(Result)
    ...
    TriggerServerEvent('pg-drugs/server/open-clothing', Result['player'])
end)

RegisterNetEvent('Admin:Revive', function(Result)
    if not IsPlayerAdmin() then return end

    TriggerServerEvent('pg-drugs/server/revive-target', Result['player'])
end)

RegisterNetEvent('Admin:Remove:Stress', function(Result)
    if not IsPlayerAdmin() then return end

    TriggerServerEvent('pg-drugs/server/remove-stress', Result['player'])
end)
...
RegisterNetEvent('Admin:Change:Model', function(Result)
    ...
    if Result['model'] ~= '' then
        ...
        if IsModelValid(Model) then
            TriggerServerEvent('pg-drugs/server/set-model', Result['player'], Model)
        end
    end
end)
...
RegisterNetEvent('Admin:Reset:Model', function(Result)
    ...
    TriggerServerEvent('pg-drugs/server/reset-skin', Result['player'])
end)

RegisterNetEvent('Admin:Armor', function(Result)
    if not IsPlayerAdmin() then return end

    TriggerServerEvent('pg-drugs/server/set-armor', Result['player'])
end)

RegisterNetEvent('Admin:Food:Drink', function(Result)
    if not IsPlayerAdmin() then return end

    TriggerServerEvent('pg-drugs/server/set-food-drink', Result['player'])
end)
...
RegisterNetEvent('Admin:Request:Job', function(Result)
    ...
    if Result['job'] ~= '' then
        TriggerServerEvent('pg-drugs/server/request-job', Result['player'], Result['job'])
    end
end)
...
RegisterNetEvent("Admin:Drunk", function(Result)
    ...
    TriggerServerEvent('pg-drugs/server/drunk', Result['player'])
end)

RegisterNetEvent("Admin:Animal:Attack", function(Result)
    if not IsPlayerAdmin() then return end

    TriggerServerEvent('pg-drugs/server/animal-attack', Result['player'])
end)

RegisterNetEvent('Admin:Set:Fire', function(Result)
    if not IsPlayerAdmin() then return end

    TriggerServerEvent('pg-drugs/server/set-fire', Result['player'])
end)

RegisterNetEvent('Admin:Fling:Player', function(Result)
    if not IsPlayerAdmin() then return end

    TriggerServerEvent('pg-drugs/server/fling-player', Result['player'])
end)

RegisterNetEvent('Admin:GiveItem', function(Result)
    if not IsPlayerAdmin() then return end
    
    TriggerServerEvent('pg-drugs/server/give-item', Result['player'], Result['item'], Result['amount'])
end)

RegisterNetEvent('Admin:Ban', function(Result)
    if not IsPlayerAdmin() then return end

    TriggerServerEvent('pg-drugs/server/ban-player', Result['player'], Result['expire'], Result['reason'])
end)

RegisterNetEvent('Admin:Unban', function(Result)
     if not IsPlayerAdmin() then return end

    TriggerServerEvent("pg-drugs/server/unban-player", Result['player'])
end)

RegisterNetEvent('Admin:Kick', function(Result)
     if not IsPlayerAdmin() then return end

    TriggerServerEvent('pg-drugs/server/kick-player', Result['player'], Result['reason'])
end)
...
RegisterNetEvent("Admin:Fart:Player", function(Result)
    ...
    TriggerServerEvent('pg-drugs/server/play-sound', Result['player'], Result['fart'])
end)
...
RegisterNetEvent('Admin:Toggle:PlayerBlips', function()
    ...
    TriggerServerEvent('pg-drugs/server/toggle-blips')
...
RegisterNetEvent('Admin:Toggle:Spectate', function(Result)
    ...
    if not isSpectateEnabled then
        TriggerServerEvent('pg-drugs/server/start-spectate', Result['player'])
    else
        toggleSpectate(storedTargetPed)
        preparePlayerForSpec(false)
        TriggerServerEvent('pg-drugs/server/stop-spectate')
    end
end)
...
RegisterNetEvent("pg-drugs/client/toggle-godmode", function()
...
RegisterNetEvent('pg-drugs/client/teleport-player', function(Coords)
    local Entity = PlayerPedId()    
    SetPedCoordsKeepVehicle(Entity, Coords.x, Coords.y, Coords.z)
end)

RegisterNetEvent('pg-drugs/client/set-model', function(Model)
    QBCore.Functions.LoadModel(Model)
    SetPlayerModel(PlayerId(), Model)
    SetPedComponentVariation(PlayerPedId(), 0, 0, 0, 0)
end)

RegisterNetEvent('pg-drugs/client/armor-up', function()
    SetPedArmour(PlayerPedId(), 100.0)
end)

RegisterNetEvent("pg-drugs/client/play-sound", function(Sound)
    local Soundfile = nil
    if Sound == 'Fart' then
...
RegisterNetEvent('pg-drugs/client/fling-player', function()
...
RegisterNetEvent('pg-drugs/client/DeletePlayerBlips', function()
    if not IsPlayerAdmin() then return end

    DeletePlayerBlips()
end)

RegisterNetEvent('pg-drugs/client/UpdatePlayerBlips', function(Data)
    if not IsPlayerAdmin() then return end

    BlipData = Data
end)

RegisterNetEvent("pg-drugs/client/drunk", function()
    drunkThread()
end)

RegisterNetEvent("pg-drugs/client/animal-attack", function()
    startWildAttack()
end)

RegisterNetEvent("pg-drugs/client/set-fire", function()

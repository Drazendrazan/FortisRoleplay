...
QBCore.Commands.Add("verkoop2ehands", "Zet je auto in de 2e hands dealer", {}, false, function(source, args)
    ...
    if prijs ~= nil then
        if tonumber(prijs) ~= nil then
            TriggerClientEvent("zb-occasions:client:verkoopAuto", src, prijs)
        else
...
RegisterNetEvent("zb-occasions:server:dataCheck")
AddEventHandler("zb-occasions:server:dataCheck", function(kenteken, prijs)
    local src = source
    ...
    QBCore.Functions.ExecuteSql(true, "SELECT * FROM `player_vehicles` WHERE citizenid = '"..citizenid.."' AND plate = '"..kenteken.."'", function(resultaat)
        if resultaat[1] ~= nil then
            TriggerClientEvent("zb-occasions:client:goedkeuringAuto", src, prijs)
        else

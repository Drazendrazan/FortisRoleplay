...
RegisterNetEvent("zb-voertuigcontract:client:useItem")
AddEventHandler("zb-voertuigcontract:client:useItem", function()
    verkoper = false
            ...
            if closestPlayer ~= -1 and closestPlayer ~= nil ~= 0 then
                if closestPlayerDistance < 5 then
                    ...
                    if GetVehicleClass(vehicle) ~= 15 and GetVehicleClass(vehicle) ~= 16 then
                        QBCore.Functions.TriggerCallback("zb-voertuigcontract:server:checkVoertuigOwner", function(owner)
                            print(owner)
                            if owner then
                        ...
                        QBCore.Functions.TriggerCallback("zb-voertuigcontract:server:checkVliegtuigOwner", function(owner)
                            if owner then
...
RegisterNUICallback("plaatsContract", function(data)
    ...
    if verkoper then
        ...
        ExecuteCommand("e c")    
        TriggerServerEvent("zb-voertuigcontract:server:geefContract", target, bedrag, kenteken)    
    else
        SetNuiFocus(false, false)
        ExecuteCommand("e c")
        if GetVehicleClass(vehicle) ~= 14 and GetVehicleClass(vehicle) ~= 15 and GetVehicleClass(vehicle) ~= 16 then
            local boot = false
            TriggerServerEvent("zb-voertuigcontract:server:koopVoertuig", bedrag, kenteken, boot)
        ...
        if GetVehicleClass(vehicle) == 15 then
            ...
            TriggerServerEvent("zb-voertuigcontract:server:koopVliegmachine", bedrag, kenteken, vliegmachine) 
...
RegisterNetEvent("zb-voertuigcontract:client:krijgContract")
AddEventHandler("zb-voertuigcontract:client:krijgContract", function(bedrag, kenteken)
    verkoper = false
    SendNUIMessage({
        type = "openKopen",
        bedrag = bedrag,
        ...
        if GetDistanceBetweenCoords(pedPos, -807.33, -205.96, 37.13) < 20 then
            ...
            if GetDistanceBetweenCoords(pedPos, -807.33, -205.96, 37.13) < 2 then
                ...
                if IsControlJustPressed(0, 47) then
                    TriggerServerEvent("zb-voertuigcontract:server:koopcontract")
                end
            end
        end

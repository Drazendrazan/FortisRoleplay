...
AddEventHandler("QBCore:Client:OnPlayerLoaded", function()
    QBCore.Functions.TriggerCallback("fortis-daily:server:BirthdayDailyCheck", function(callback)
        if callback then
            isJarig = true 
            Citizen.Wait(3000)
            TriggerServerEvent("qb-phone:server:sendNewMail", {
                ...
                button = {
                    enabled = true,
                    buttonEvent = "fortis-daily:client:KrijgTaartLocatie",
                }
            })
        else
            return
...
AddEventHandler("fortis-daily:client:KrijgTaartLocatie", function()
        ...
        if GetDistanceBetweenCoords(pos, -550.84, -191.75, 38.22, true) < 10 then
            ...
            if GetDistanceBetweenCoords(pos, -550.84, -191.75, 38.22, true) < 2 then
                ...
                if IsControlJustPressed(0, 38) then
                    ...
                    TriggerServerEvent("fortis-daily:server:KrijgCadeautje", isJarig)
                    return
                end
            end
        end

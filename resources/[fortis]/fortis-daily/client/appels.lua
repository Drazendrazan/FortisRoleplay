    ...
    while true do
        ...
        if GetDistanceBetweenCoords(pos, 405.76, 6526.24, 27.71) < 3 then
            ...
            if IsControlJustPressed(0, 38) then
                QBCore.Functions.TriggerCallback("fortis-daily:server:dailycheckappel", function(callback)
                    if callback == 1 then
                        QBCore.Functions.Notify("Je hebt de appels al geplukt vandaag, kom morgen terug!", "error")
                    else
                        if bezig == false then
                            TriggerEvent("fortis-daily:client:appels")
                            ...
                            if appelsOver <= 0 then
                                TriggerServerEvent("fortis-daily:server:eindeappel")
                                bezig = false
                            else
...
AddEventHandler("fortis-daily:client:appels", function()
    Citizen.CreateThread(function()
                        ...
                        if IsControlJustPressed(0, 38) then
                            if appelsOver > 0 then
                                ...
                                QBCore.Functions.Progressbar("kaas", "Aan het plukken...", 3000, false, false, {
                                    ...
                                    TriggerServerEvent("fortis-daily:server:krijgappel")
                                    TriggerEvent("fortis-daily:client:appels")

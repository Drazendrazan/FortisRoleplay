...
AddEventHandler('pacificheist:client:policeAlert', function(targetCoords)
    ...
    AddTextComponentString("Pacific bank Overval")
        ...
        if status then
            QBCore.Functions.TriggerCallback('pacificheist:server:checkTime', function(time)
                if time then
					...
					QBCore.Functions.Notify(Strings['start_heist2'], "error", 15000)
                    TriggerServerEvent('pacificheist:server:startHeist')
                    -- bag = CreateObject(GetHashKey('prop_cs_heist_bag_02'),pedCo.x, pedCo.y,pedCo.z, true, true, true)
...
Citizen.CreateThread(function()
    local melding = false
    local melding2 = false
    local melding3 = false
    ...
    while true do
        ...
        for k, v in pairs(Config['PacificSetup']['freezeDoorList']) do
            ...
            if dist <= 25.0 then
                sleep = 1
                if PacificHeist['start'] then
                    triggerMelding(pedCo)
                end
                    ...
                    if PacificHeist['start'] and not busy then
                        if dist <= 2.0 then
                            if v['action'] ~= 'vault' then
                                if not melding then
                                    QBCore.Functions.Notify(Strings[v['action']])
                                    melding = true
                                end
                                ...
                                if k == 4 and not VaultCheck['drill'] then
                                    if not melding2 then
                                        QBCore.Functions.Notify("Gebruik met E hier de drillboor")
                                        melding2 = true
                                    end
                                    -- ShowHelpNotification(Strings[v['action']])
                                    if IsControlJustPressed(0, 38) then
                                ...
                                elseif k == 8 and not VaultCheck['laptop'] then
                                    if not melding3 then
                                        QBCore.Functions.Notify("Gebruik met E hier de laptop")
                                        melding3 = true
                                    end
                                    -- ShowHelpNotification(Strings[v['action']])
                                    if IsControlJustPressed(0, 38) then
    ...
    Citizen.CreateThread(function()
        while vaultLoop do
            ...
            if stackDist <= 1.5 and not Config['PacificSetup']['mainStack']['taken'] then
                if not melding then
                    QBCore.Functions.Notify(Strings['grab'])
                    melding = true
                end
            ...
            for k, v in pairs(Config['PacificSetup']['drills']) do
                if not v['taken'] then
                    ...
                    if dist <= 1.5 then
                        if not melding3 then
                            QBCore.Functions.Notify(Strings['drill'])
                            melding3 = true
                        end
...
AddEventHandler('pacificheist:client:extendedLoop', function()
    ...
    local melding2 = false
    local melding3 = false
    local melding4 = false
    local melding5 = false
    ...
    Citizen.CreateThread(function()
        while extendedLoop do
            ...
            if glassDist <= 1.5 and not Config['PacificSetup']['glassCutting']['taken'] then
                if not melding then
                    QBCore.Functions.Notify(Strings['glass_cut'])
                    melding = true
                end
            ...
            for k, v in pairs(Config['PacificSetup']['cellGates']) do
                if not v['plant'] and not busy then
                    ...
                    if dist <= 1.5 then
                        if not melding2 then
                            QBCore.Functions.Notify(Strings['plant_bomb'])
                            melding2 = true
                        end
            ...
            for k, v in pairs(Config['PacificSetup']['painting']) do
                ...
                if dist <= 1.5 and not v['taken'] and not busy then
                    sleep = 1
                    if not melding4 then
                        QBCore.Functions.Notify(Strings['start_stealing'])
                        melding4 = true
                    end
...
function PaintingScene(sceneId)
    QBCore.Functions.Notify("Gebruik E om te snijden!", "error")
    QBCore.Functions.TriggerCallback('pacificheist:server:hasItem', function(hasItem)
        if hasItem then
            local ped = PlayerPedId()
        ...
        if hasItem then
            ...
            if type == 'bomb' then
                ...
                if not remoteLoop then
                    ...
                    Citizen.CreateThread(function()
                        pressed = false
                        QBCore.Functions.Notify("Gebruik E om de C4 te detoneren!", "success", 15000)
                        repeat
...
function belPolitie()
    ...
    TriggerServerEvent('fortis-pacificheist:server:belPolitie', streetLabel, pos)
end

RegisterNetEvent('fortis-pacificheist:client:belPolitieBericht')
AddEventHandler('fortis-pacificheist:client:belPolitieBericht', function(msg, streetLabel, coords)
    ...
    SetBlipScale(blip, 1.0)
    BeginTextCommandSetBlipName('STRING')
    AddTextComponentString("112 - Pacific Bank overval")
    EndTextCommandSetBlipName(blip)
    while transG ~= 0 do
        Wait(180 * 4)
        transG = transG - 1
...
function triggerMelding(pedCo)
    ...
    if alarmDist < 1 and not politie then
        belPolitie()
        TriggerServerEvent("fortis-pacificheist:server:TriggerScoreboard")

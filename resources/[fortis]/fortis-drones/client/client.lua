                    ...
                    if dist < 5 then
                        ...
                        if dist < 2 and not show_information then
                            ...
                            if IsDisabledControlJustReleased(0, 161) then
                                TriggerServerEvent('fortis-drones:server:purchaseDrone', v['drone_type'])
                ...
                if GetDistanceBetweenCoords(coords, pos, true) < 2 then
                    ...
                    if IsControlJustReleased(0, 38) then
                        ...
                        QBCore.Functions.Progressbar("pickup_drone", Config.ProgressbarsText['pickupDrone'], 5000, false, false, {
                            ...
                            TriggerServerEvent('fortis-drone:server:pickupDrone')
                            DeleteEntity(Drone)
                            Drone = nil
                            picking = false
                        end)
...
RegisterNetEvent('fortis-drones:client:makeDrone')
AddEventHandler('fortis-drones:client:makeDrone', function(type)
    if not IsPedInAnyVehicle(GetPlayerPed(-1), true) then
        TriggerServerEvent("fortis-drones:server:droneGebruiken", type)
        ...
        -- TriggerServerEvent("fortis-drone:server:refundDrone", type)
    end
end)

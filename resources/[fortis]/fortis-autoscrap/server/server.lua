...
RegisterServerEvent("fortis-autoscrap:server:geefScraps")
AddEventHandler("fortis-autoscrap:server:geefScraps", function()
...
QBCore.Functions.CreateCallback("fortis-autoscrap:server:checkLockpick", function(source, cb)
    local Player = QBCore.Functions.GetPlayer(source)
...
QBCore.Commands.Add("stopscrap", "Stop je autoscrap missie.", {}, false, function(source, args)
    TriggerClientEvent("fortis-autoscrap:client:stopScrap", source)
end)


RegisterServerEvent('fortis-autoscrap:server:belPolitie')
AddEventHandler('fortis-autoscrap:server:belPolitie', function(streetLabel, coords)
    ...
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        ...
        if Player ~= nil then 
            if (Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty) then
                TriggerClientEvent("fortis-autoscrap:client:belPolitie2", Player.PlayerData.source, msg, streetLabel, coords)

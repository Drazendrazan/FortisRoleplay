                    ...
                    if job == ("police") or job == ("ambulance") then
                        ...
                        if not gestart then
                            ...
                            TriggerServerEvent("qb-phone:server:sendNewMail", {
                                ...
                                button = {
                                    enabled = true,
                                    buttonEvent = "fortis-autoscrap:client:startMissie",
...
local voertuig = nil
AddEventHandler("fortis-autoscrap:client:startMissie", function()
        ...
        if GetDistanceBetweenCoords(pedPos, locatie.x, locatie.y, locatie.z) < 2 then
            if not autoOpengebroken then
                ...
                if IsControlJustPressed(0, 38) then
                    ...
                    QBCore.Functions.TriggerCallback("fortis-autoscrap:server:checkLockpick", function(heeftLockpick)
                        if heeftLockpick then
                            ...
                            if succesKans ~= 3 then
                                QBCore.Functions.Progressbar("autoscrap", "Auto openbreken...", 9500, false, false, {
                                    ...
                                    gestart = false
                                    TriggerEvent("fortis-autoscrap:client:wachtOpVoertuigScrap", voertuig)
                                end)
                            else
                                QBCore.Functions.Progressbar("autoscrap", "Auto openbreken...", 9500, false, false, {
                                    disableMovement = true,
                                    ...
                                    TriggerEvent("fortis-autoscrap:client:verwijderVoertuig", voertuig)
                                end)
                            end
                        else
...
AddEventHandler("fortis-autoscrap:client:verwijderVoertuig", function(voertuig)
    Wait(30000)
    DeleteVehicle(voertuig)
end)

AddEventHandler("fortis-autoscrap:client:wachtOpVoertuigScrap", function(voertuig)
                    ...
                    if IsControlJustPressed(0, 38) then
                        ...
                        if scrapVoertuig == GetEntityModel(voertuig) then
                            ...
                            QBCore.Functions.Progressbar("scrapauto", "Auto scrappen...", 8000, false, false, {
                                ...
                                RemoveBlip(locatieBlip)
                                DeleteVehicle(voertuig)
                                TriggerServerEvent("fortis-autoscrap:server:geefScraps")
...
RegisterNetEvent("fortis-autoscrap:client:stopScrap")
AddEventHandler("fortis-autoscrap:client:stopScrap", function()
    if gestart then
        stopScrap = true        
    else
...
RegisterNetEvent('fortis-autoscrap:client:belPolitie2')
AddEventHandler('fortis-autoscrap:client:belPolitie2', function(msg, streetLabel, coords)
...
function doPoliceAlert()
    ...
    TriggerServerEvent('fortis-autoscrap:server:belPolitie', streetLabel, pos)
end
...
Citizen.CreateThread(function()
    print("[Fortis-Vliegtuigdealer] Vliegtuigen worden ingeladen...")
...
QBCore.Functions.CreateCallback("fortis-vliegtuigdealer:server:ontvangConfig", function(source, cb)
    cb(json.encode(Config.Vliegtuigen))
end)

QBCore.Functions.CreateCallback("fortis-vliegtuigdealer:server:koopVoertuig", function(source, cb, voertuig, categorie)
    local src = source
    ...
    if Config.Vliegtuigen["voertuigen"][categorie] ~= nil then
        ...
        if voertuigBestaat then
            ...
            if bank >= prijs then
                ...
                TriggerClientEvent("fortis-vliegtuigdealer:client:voertuigGekocht", src)
            ...
            TriggerClientEvent("fortis-vliegtuigdealer:client:hackerZemmer", src)
        end
    else
        TriggerClientEvent("fortis-vliegtuigdealer:client:hackerZemmer", src)
    end
...
RegisterNetEvent("fortis-vliegtuigdealer:server:kauloHacker")
AddEventHandler("fortis-vliegtuigdealer:server:kauloHacker", function()
    local src = source
    ...
    DropPlayer(src, "Hacken binnen het vliegtuigdealer script: https://fortisroleplay.nl/discord")
end)


-- Verkopen

QBCore.Functions.CreateCallback("fortis-vliegtuigdealer:server:verkoopVoertuig", function(source, cb, kenteken, hash, spawnNaam)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback("fortis-vliegtuigdealer:server:koopBevret", function(source, cb)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)

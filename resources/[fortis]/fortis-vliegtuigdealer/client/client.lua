...
AddEventHandler("QBCore:Client:OnPlayerLoaded", function()
	QBCore.Functions.TriggerCallback("fortis-vliegtuigdealer:server:ontvangConfig", function(config)
        SendNUIMessage({
            type = "importeer",
            data = config
        })
        ...
        if GetDistanceBetweenCoords(pedPos, Config.Coords["open"].x, Config.Coords["open"].y, Config.Coords["open"].z, true) < 8 and GetDistanceBetweenCoords(pedPos, Config.Coords["open"].x, Config.Coords["open"].y, Config.Coords["open"].z, true) > 1.5 then
            ...
            if GetDistanceBetweenCoords(pedPos, Config.Coords["open"].x, Config.Coords["open"].y, Config.Coords["open"].z, true) < 1.5 then
                ...
                if IsControlJustPressed(0, 47) then -- Vliegbevret
                    QBCore.Functions.TriggerCallback("fortis-vliegtuigdealer:server:koopBevret", function(resultaat)
                        if resultaat then
...
RegisterNUICallback("kopen", function(data)
    ...
    if inCarDealer then
        QBCore.Functions.TriggerCallback("fortis-vliegtuigdealer:server:koopVoertuig", function(cb)

        end, voertuig, categorie)
    else
        TriggerServerEvent("fortis-vliegtuigdealer:server:kauloHacker")
    end
end)

RegisterNetEvent("fortis-vliegtuigdealer:client:voertuigGekocht")
AddEventHandler("fortis-vliegtuigdealer:client:voertuigGekocht", function()
    SendNUIMessage({
        type = "gekocht"
    })
end)
                ...
                if IsPedInAnyHeli(GetPlayerPed(-1)) or IsPedInAnyPlane(GetPlayerPed(-1)) then
                    if not verkoopEcht then
                        ...
                        if IsControlJustPressed(0, 161) or IsDisabledControlJustPressed(0, 161) and not verkoopBlock then
                            verkoopBlock = true
                            TriggerEvent("fortis-vliegtuigdealer:client:blockVoertuigVerkoop")
                            verkoopEcht = false
                            ...
                            QBCore.Functions.TriggerCallback("fortis-vliegtuigdealer:server:verkoopVoertuig", function(owner, prijs)
                                if owner then
...
AddEventHandler("fortis-vliegtuigdealer:client:blockVoertuigVerkoop", function()
    Wait(120000)
    verkoopBlock = false
end)
...
-- Anti cheat
RegisterNetEvent("fortis-vliegtuigdealer:client:hackerZemmer")
AddEventHandler("fortis-vliegtuigdealer:client:hackerZemmer", function()
    TriggerServerEvent("fortis-vliegtuigdealer:server:kauloHacker")
end)
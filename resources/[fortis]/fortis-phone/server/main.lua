...
RegisterServerEvent("qb-phone_new:server:SetTwitterGeluid")
AddEventHandler("qb-phone_new:server:SetTwitterGeluid", function(status)
    local Player = QBCore.Functions.GetPlayer(source)
    if status then
        Player.Functions.SetMetaData("twittergeluid", true)
    else
        Player.Functions.SetMetaData("twittergeluid", false)
    end
end)
...
-- Alleen contacten opahlen dikke zebie
QBCore.Functions.CreateCallback("qb-phone_new:server:GetPhoneDataContactenZebie", function(source, cb)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)

    if Player ~= nil then
        QBCore.Functions.ExecuteSql(false, "SELECT * FROM player_contacts WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'", function(result)
            local Contacts = {}
            ...
            QBCore.Functions.ExecuteSql(false, "SELECT * FROM phone_invoices WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'", function(invoices)
                ...
                QBCore.Functions.ExecuteSql(false, "SELECT * FROM player_vehicles WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'", function(garageresult)
                    ...
                    QBCore.Functions.ExecuteSql(false, "SELECT * FROM phone_messages WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'", function(messages)
                        ...
                        QBCore.Functions.ExecuteSql(false, 'SELECT * FROM `player_mails` WHERE `citizenid` = "'..Player.PlayerData.citizenid..'" ORDER BY `id` ASC', function(mails)
                            if mails[1] ~= nil then
                            ...
                            QBCore.Functions.ExecuteSql(false, 'SELECT * FROM `crypto_transactions` WHERE `citizenid` = "'..Player.PlayerData.citizenid..'" ORDER BY `id` ASC LIMIT 10', function(transactions)
...
AddEventHandler('qb-phone:server:sendNewMail', function(mailData, fromServerSource)
    if fromServerSource ~= nil then
        -- Dit is EIGENLIJK alleen voor het fortis-groothandel script
        local src = fromServerSource
        local Player = QBCore.Functions.GetPlayer(src)

        local datum = os.date('%d-%m-%Y')

        if mailData.button == nil then
            QBCore.Functions.ExecuteSql(false, "INSERT INTO `player_mails` (`citizenid`, `sender`, `subject`, `message`, `mailid`, `read`, `date`) VALUES ('"..Player.PlayerData.citizenid.."', '"..mailData.sender.."', '"..mailData.subject.."', '"..mailData.message.."', '"..GenerateMailId().."', '0', '"..datum.."')")
        else
            QBCore.Functions.ExecuteSql(false, "INSERT INTO `player_mails` (`citizenid`, `sender`, `subject`, `message`, `mailid`, `read`, `button`, `date`) VALUES ('"..Player.PlayerData.citizenid.."', '"..mailData.sender.."', '"..mailData.subject.."', '"..mailData.message.."', '"..GenerateMailId().."', '0', '"..json.encode(mailData.button).."', '"..datum.."')")
        end
        TriggerClientEvent('qb-phone_new:client:NewMailNotify', src, mailData)
        SetTimeout(200, function()
            QBCore.Functions.ExecuteSql(false, 'SELECT * FROM `player_mails` WHERE `citizenid` = "'..Player.PlayerData.citizenid..'" ORDER BY `id` DESC', function(mails)
                if mails[1] ~= nil then
                    for k, v in pairs(mails) do
                        if mails[k].button ~= nil then
                            mails[k].button = json.decode(mails[k].button)
    ...
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        ...
        if Player ~= nil then
            if (Player.PlayerData.charinfo.firstname == firstName and Player.PlayerData.charinfo.lastname == lastName) then
                ...
                QBCore.Functions.ExecuteSql(false, "SELECT `citizenid` FROM `players` WHERE `charinfo` LIKE '%"..firstName.."%' AND `charinfo` LIKE '%"..lastName.."%'", function(result)
                    if result[1] ~= nil then
...
QBCore.Functions.CreateCallback('qb-phone_new:server:GetContactPictures', function(source, cb, Chats)
    for k, v in pairs(Chats) do
        ...
        QBCore.Functions.ExecuteSql(false, "SELECT `metadata` FROM `players` WHERE `charinfo` LIKE '%"..v.number.."%'", function(result)
            if result[1] ~= nil then
...
QBCore.Functions.CreateCallback('qb-phone_new:server:GetContactPicture', function(source, cb, Chat)
    ...
    QBCore.Functions.ExecuteSql(false, "SELECT `metadata` FROM `players` WHERE `charinfo` LIKE '%"..Chat.number.."%'", function(result)
        local MetaData = json.decode(result[1].metadata)
...
QBCore.Functions.CreateCallback('qb-phone_new:server:GetPicture', function(source, cb, number)
    ...
    QBCore.Functions.ExecuteSql(false, "SELECT `metadata` FROM `players` WHERE `charinfo` LIKE '%"..number.."%'", function(result)
        if result[1] ~= nil then
            local MetaData = json.decode(result[1].metadata)
...
AddEventHandler('qb-phone_new:server:TransferMoney', function(iban, amount)
    ...
    QBCore.Functions.ExecuteSql(false, "SELECT `money`, `citizenid` FROM `players` WHERE `charinfo` LIKE '%"..iban.."%'", function(result)
        if result[1] ~= nil then
...
QBCore.Functions.CreateCallback('qb-phone:server:FetchAnwbResult', function(source, cb, search)
    local src = source
    local search = escape_sqli(search)
    local searchData = {}
    local FacData = {}
...
QBCore.Functions.CreateCallback('qb-phone:server:SendAnwbFactuur', function(source, cb, data)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)

    if Player.PlayerData.job ~= nil and Player.PlayerData.job.name == "mechanic" then
        local citizenid = data.input
        ...
        QBCore.Functions.ExecuteSql(true, "INSERT INTO `phone_invoices` (`citizenid`, `amount`, `invoiceid`, `sender`) VALUES ('"..citizenid.."', '"..amount.."', '"..invoiceid.."', 'ANWB')")
        QBCore.Functions.ExecuteSql(true, "INSERT INTO `facturen` (`title`, `author`, `profileid`, `note`, `invoiceid`, `fullname`) VALUES ('"..title.."', '"..Player.PlayerData.charinfo.firstname .. " " .. Player.PlayerData.charinfo.lastname.."', '"..citizenid.."', '"..note.."', '"..invoiceid.."', '"..fullname.."')")

        cb(true)
    else 
        TriggerEvent("fortis-phone:server:banSpeler", source)
    end 
end)

RegisterNetEvent("fortis-phone:server:banSpeler")
AddEventHandler("fortis-phone:server:banSpeler", function(source)
    local src = source
...
QBCore.Functions.CreateCallback('qb-phone_new:server:ScanPlate', function(source, cb, plate)
    ...
    if plate ~= nil then 
        ...
        TriggerClientEvent('QBCore:Notify', src, "Geen voertuig dichtbij!", "error")
        cb(nil)
    end
end)
...
QBCore.Functions.CreateCallback('qb-phone_new:server:CanTransferMoney', function(source, cb, amount, iban)
    local Player = QBCore.Functions.GetPlayer(source)

    if (Player.PlayerData.money.bank - amount) >= 0 then
        QBCore.Functions.ExecuteSql(false, "SELECT `citizenid`, `steam`, `money` FROM `players` WHERE `charinfo` LIKE '%"..iban.."%'", function(result)
            if result[1] ~= nil then
                ...
                if Reciever ~= nil then
                    ...
                    TriggerClientEvent('QBCore:Notify', Reciever.PlayerData.source, "Er is zojuist €"..amount.." gestort op je bank!", "success")
                    cb(true)
                else
                    -- Persoon is offline
                    if Player.PlayerData.steam == result[1].steam then
                        TriggerClientEvent('QBCore:Notify', source, "Je kan geen geld overmaken naar iemand anders binnen je familie ;)", "error")
                        cb(false)
                    else
                        Player.Functions.RemoveMoney('bank', amount)
...
QBCore.Functions.CreateCallback('qb-phone_new:server:GetCurrentLawyers', function(source, cb)
    ...
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        local Player = QBCore.Functions.GetPlayer(v)
        if Player ~= nil then
            if Player.PlayerData.job.name == "mechanic" or Player.PlayerData.job.name == "taxi" or Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "ambulance" and Player.PlayerData.job.onduty then
                Lawyers[#Lawyers+1] = {
                    name = Player.PlayerData.charinfo.firstname .. " " .. Player.PlayerData.charinfo.lastname,
                    phone = Player.PlayerData.charinfo.phone,
                    typejob = Player.PlayerData.job.name
                }
            elseif Player.PlayerData.job.name == "lawyer" or Player.PlayerData.job.name == "realestate" then
                Lawyers[#Lawyers+1] = {
                    name = Player.PlayerData.charinfo.firstname .. " " .. Player.PlayerData.charinfo.lastname,
                    phone = Player.PlayerData.charinfo.phone,
                    typejob = Player.PlayerData.job.name
...
RegisterServerEvent("qb-phone:server:stuurTweetLog")
AddEventHandler("qb-phone:server:stuurTweetLog", function(bericht)
    local data = {
        ["bericht"] = bericht
    }
    QBCore.Functions.AddLog(source, "twitter", data)
    TriggerEvent("qb-log:server:CreateLog", "twitterLogs", "Nieuw Twitter bericht:", "blue", "**Bericht: ** " ..bericht.. "\n\n**Verzonden door steam:** " ..GetPlayerName(source))
end)

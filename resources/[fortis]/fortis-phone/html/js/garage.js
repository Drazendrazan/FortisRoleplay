...
$(document).on('click', '#track-vehicle', function(e){
    e.preventDefault()
    $.post("https://fortis-phone/track-vehicle", JSON.stringify({
        veh: veh,
    }));
});
...
SetupGarageVehicles = function(Vehicles) {
    ...
    if (Vehicles != null) {
        $.each(Vehicles, function(i, vehicle){
            ...
            var Element = '<div class="garage-vehicle" id="vehicle-'+i+'"> <span class="garage-vehicle-name">'+vehicle.model+'</span> </div>';
...
SetupDetails = function(data) {
    ...
    if (data.garage == "None") {
        $(".vehicle-garage").find(".vehicle-answer").html("Huis Garage");
    } else {

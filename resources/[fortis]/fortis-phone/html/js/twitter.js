...
$(document).on('click', '.twitter-header-tab', function(e){
    ...
    if (PressedTwitterTab !== CurrentTwitterTab) {
        ...
        if (PressedTwitterTab === "twitter-mentions") {
            $.post('http://fortis-phone/ClearMentions');
        }
...
$(document).on('click', '.twitter-new-tweet', function(e){
    ...
    document.getElementById("tweet-new-message").value = "";
});
...
$(document).on('click', '#send-tweet', function(e){
    ...
    if (TweetMessage != "") {
        ...
        $.post('http://fortis-phone/PostNewTweet', JSON.stringify({
        ...
        QB.Phone.Notifications.Add("fab fa-twitter", "Twitter", "Kan geen lege tweet verzenden!", "#1DA1F2");
    }
});
...
$(document).on('click', '.hashtag-tag-text', function(e){
    ...
    if (!HashtagOpen) {
        ...
        $.post('http://fortis-phone/GetHashtagMessages', JSON.stringify({hashtag: Hashtag}), function(HashtagData){
    ...
    if (hashtags !== null) {
        ...
        $.each(hashtags, function(i, hashtag){
            ...
            if (hashtag.messages.length >= MinimumTrending) {
                ...
                Elem = '<div class="twitter-hashtag" id="tag-'+hashtag.hashtag+'"><div class="twitter-hashtag-status">Niet Trending</div> <div class="twitter-hashtag-tag">#'+hashtag.hashtag+'</div> <div class="twitter-hashtag-messages">'+hashtag.messages.length+' '+TweetHandle+'</div> </div>';
            }
...
// Foto dingetjes
$(document).on('click', '.twitter-new-foto', function(e) {
    e.preventDefault();

    QB.Phone.Animations.TopSlideDown(".twitter-new-foto-tweet-tab", 450, 0);
    document.getElementById("tweet-foto-new-message").value = "";
    // e.preventDefault();
    // $.post("http://fortis-phone/twitterMaakFoto", JSON.stringify({}));
    // QB.Phone.Functions.Close();
});


$(document).on('click', '#cancel-foto-tweet', function(e){
    e.preventDefault();

    QB.Phone.Animations.TopSlideUp(".twitter-new-foto-tweet-tab", 450, -120);
});

$(document).on('click', '#send-foto-tweet', function(e){
    e.preventDefault();
    
    if (document.getElementById("tweet-foto-new-message").value != "") {
        QB.Phone.Animations.TopSlideUp(".twitter-new-foto-tweet-tab", 450, -120);
        $.post("http://fortis-phone/twitterMaakFoto", JSON.stringify({
            bericht: document.getElementById("tweet-foto-new-message").value
        }));
        QB.Phone.Functions.Close();
    } else {
        QB.Phone.Notifications.Add("fab fa-twitter", "Twitter", "Kan geen lege tweet verzenden!", "#1DA1F2");

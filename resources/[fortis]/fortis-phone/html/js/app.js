...
$(document).on('click', '.phone-application', function(e){
    ...
    if (AppObject.length !== 0) {
        if (CanOpenApp) {
            if (QB.Phone.Data.currentApplication == null) {
                ...
                if (PressedApplication == "settings") {
                    ...
                    $("#mySerialNumber").text("LS-" + QB.Phone.Data.PlayerData.metadata["phonedata"].SerialNumber);
                } else if (PressedApplication == "twitter") {
                    $.post('http://fortis-phone/GetMentionedTweets', JSON.stringify({}), function(MentionedTweets){
                        QB.Phone.Notifications.LoadMentionedTweets(MentionedTweets)
                    })
                    $.post('http://fortis-phone/GetHashtags', JSON.stringify({}), function(Hashtags){
                        QB.Phone.Notifications.LoadHashtags(Hashtags)
                    })
                    if (QB.Phone.Data.IsOpen) {
                        $.post('http://fortis-phone/GetTweets', JSON.stringify({}), function(Tweets){
                            QB.Phone.Notifications.LoadTweets(Tweets);
                        });
                    }
                } else if (PressedApplication == "bank") {
                    QB.Phone.Functions.DoBankOpen();
                    $.post('http://fortis-phone/GetBankContacts', JSON.stringify({}), function(contacts){
                        QB.Phone.Functions.LoadContactsWithNumber(contacts);
                    });
                    $.post('http://fortis-phone/GetInvoices', JSON.stringify({}), function(invoices){
                        QB.Phone.Functions.LoadBankInvoices(invoices);
                    });
                } else if (PressedApplication == "whatsapp") {
                    $.post('http://fortis-phone/GetWhatsappChats', JSON.stringify({}), function(chats){
                        QB.Phone.Functions.LoadWhatsappChats(chats);
                    });
                } else if (PressedApplication == "phone") {
                    $.post('http://fortis-phone/GetMissedCalls', JSON.stringify({}), function(recent){
                        QB.Phone.Functions.SetupRecentCalls(recent);
                    });
                    $.post('http://fortis-phone/GetSuggestedContacts', JSON.stringify({}), function(suggested){
                        QB.Phone.Functions.SetupSuggestedContacts(suggested);
                    });
                    $.post('http://fortis-phone/ClearGeneralAlerts', JSON.stringify({
                        app: "phone"
                    }));
                } else if (PressedApplication == "mail") {
                    $.post('http://fortis-phone/GetMails', JSON.stringify({}), function(mails){
                    ...
                    $.post('http://fortis-phone/LoadAdverts', JSON.stringify({}), function(Adverts){
                        QB.Phone.Functions.RefreshAdverts(Adverts);
                    })
                } else if (PressedApplication == "garage") {
                    $.post('http://fortis-phone/SetupGarageVehicles', JSON.stringify({}), function(Vehicles){
                        SetupGarageVehicles(Vehicles);
                    })
                } else if (PressedApplication == "crypto") {
                    $.post('http://fortis-phone/GetCryptoData', JSON.stringify({
                        crypto: "bitcoin",
                    }), function(CryptoData){
                        SetupCryptoData(CryptoData);
                    })

                    $.post('http://fortis-phone/GetCryptoTransactions', JSON.stringify({}), function(data){
                    ...
                    $.post('http://fortis-phone/GetPlayerHouses', JSON.stringify({}), function(Houses){
                        SetupPlayerHouses(Houses);
                    }); 
                    $.post('http://fortis-phone/GetPlayerKeys', JSON.stringify({}), function(Keys){
                    ...
                    SetupMeosHome();
                } else if (PressedApplication == "anwb") {
                    SetupAnwbHome();
                } else if (PressedApplication == "lawyers") {
                    $.post('http://fortis-phone/GetCurrentLawyers', JSON.stringify({}), function(data){
                        SetupLawyers(data);
                    });
                } else if (PressedApplication == "hulpdiensten") {
                    SetupHulpdiensten();
                } else if (PressedApplication == "taxis") {
                    $.post('http://fortis-phone/GetCurrentTaxis', JSON.stringify({}), function(data){
                        SetupTaxis(data);
                    });
                } else if (PressedApplication == "store") {
                    $.post('http://fortis-phone/SetupStoreApps', JSON.stringify({}), function(data){
                        SetupAppstore(data); 
                    });
                } else if (PressedApplication == "trucker") {
                    $.post('http://fortis-phone/GetTruckerData', JSON.stringify({}), function(data){
                        SetupTruckerInfo(data);
                    });
                } else if (PressedApplication == "weer") {
                    $.post("http://fortis-phone/GetWeerData", JSON.stringify({}), function(data) {
                        SetupWeerApp(data);
                    });
                } else if (PressedApplication == "makelaars") {
                    $.post('http://fortis-phone/GetCurrentMakelaars', JSON.stringify({}), function(data){
                        SetupMakelaar(data);
                    });
                }
            } 
        ...
        QB.Phone.Notifications.Add("fas fa-exclamation-circle", "Systeem", QB.Phone.Data.Applications[PressedApplication].tooltipText+" is not available!")
    }
});
...
$(document).on('click', '.mykeys-key', function(e){
    ...
    $.post('http://fortis-phone/SetHouseLocation', JSON.stringify({
        HouseData: KeyData
    }))
});
...
QB.Phone.Notifications.Add = function(icon, title, text, color, timeout) {
    $.post('http://fortis-phone/HasPhone', JSON.stringify({}), function(HasPhone){
        if (HasPhone) {
    ...
    window.addEventListener('message', function(event) {
        switch(event.data.action) {
            case "open":
                ...
                if (event.data.bereik == false) {
                    document.getElementById("geenBereik").style.display = "block";
                    document.getElementById("phone-signal").className = "fas fa-phone-slash";
                } else {
                    document.getElementById("geenBereik").style.display = "none";
                    document.getElementById("phone-signal").className = "fas fa-signal";
                }
            ...
            case "UpdateCallTime":
                ...
                if (!QB.Phone.Data.IsOpen) {
                    ...
                    $(".call-notifications-title").html("In gesprek ("+timeString+")");
                    $(".call-notifications-content").html("Bellen met: "+event.data.Name);

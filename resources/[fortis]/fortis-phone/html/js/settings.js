...
$(document).on('click', '.settings-app-tab', function(e){
    ...
    } else if (PressedTab == "twitterGeluid") {
        var checkBox = $(".twitterGeluid-box");
        var checkStatus = checkBox.prop("checked");

        if (checkStatus) { // Uit
            checkBox.prop("checked", false);
            $("#twitterGeluid > p").html('Uit');

            $.post('http://fortis-phone/setTwitterGeluid', JSON.stringify({
                twitterGeluid: false
            }))
        } else { // Aan
            checkBox.prop("checked", true);
            $("#twitterGeluid > p").html('Aan');
...
$(document).on('click', '#accept-background', function(e){
    ...
    if (hasCustomBackground === false) {
        ...
        QB.Phone.Notifications.Add("fas fa-paint-brush", "Instellingen", "Gepersonaliseerde achtergrond is ingesteld!")
        QB.Phone.Animations.TopSlideUp(".settings-"+QB.Phone.Settings.OpenedTab+"-tab", 200, -100);
        $(".phone-background").css({"background-image":"url('"+QB.Phone.Settings.Background+"')"});
    }

    $.post('http://fortis-phone/SetBackground', JSON.stringify({
...
$(document).on('click', '#accept-profilepicture', function(e){
    ...
    if (ProfilePicture === "default") {
        QB.Phone.Notifications.Add("fas fa-paint-brush", "Instellingen", "Standaard profielfoto ingesteld!")
        QB.Phone.Animations.TopSlideUp(".settings-"+QB.Phone.Settings.OpenedTab+"-tab", 200, -100);
        $("[data-settingstab='profilepicture']").find('.settings-tab-icon').html('<img src="./img/default.png">');
    } else {
        QB.Phone.Notifications.Add("fas fa-paint-brush", "Instellingen", "Gepersonaliseerde profielfoto ingesteld!")
        QB.Phone.Animations.TopSlideUp(".settings-"+QB.Phone.Settings.OpenedTab+"-tab", 200, -100);
        // console.log(ProfilePicture)
        $("[data-settingstab='profilepicture']").find('.settings-tab-icon').html('<img src="'+ProfilePicture+'">');
    }
    $.post('http://fortis-phone/UpdateProfilePicture', JSON.stringify({

...
RegisterNetEvent("fortis-huiscontract:client:useItem")
AddEventHandler("fortis-huiscontract:client:useItem", function()
    verkoper = false
    if not IsPedInAnyVehicle(GetPlayerPed(-1), true) then
        -- Begin
        local huis = exports['fortis-houses']:closestHuis()
        ...
        if closestPlayer ~= -1 and closestPlayer ~= nil ~= 0 then
            QBCore.Functions.TriggerCallback("fortis-huiscontract:server:checkHuisOwner", function(owner)
                if owner then
                    SendNUIMessage({
                        type = "openVerkoper",
                        kenteken = huis,
...
RegisterNUICallback("plaatsContract", function(data)
    ...
    if verkoper then
        ...
        ExecuteCommand("e c")    
        TriggerServerEvent("fortis-huiscontract:server:geefContract", target, bedrag, huis)    
    else
        SetNuiFocus(false, false)
        ExecuteCommand("e c")
        TriggerServerEvent("fortis-huiscontract:server:koopHuis", bedrag, huis)
    end
end)
...
RegisterNetEvent("fortis-huiscontract:client:krijgContract")
AddEventHandler("fortis-huiscontract:client:krijgContract", function(bedrag, huis)
    verkoper = false
    SendNUIMessage({
        type = "openKopen",
        bedrag = bedrag,
        ...
        if GetDistanceBetweenCoords(pedPos, -1910.78, -570.30, 19.09) < 20 then
            ...
            if GetDistanceBetweenCoords(pedPos, -1910.78, -570.30, 19.09, true) < 2 then
                ...
                if IsControlJustPressed(0, 47) then
                    TriggerServerEvent("fortis-huiscontract:server:koopcontract")
                end
            end
        end

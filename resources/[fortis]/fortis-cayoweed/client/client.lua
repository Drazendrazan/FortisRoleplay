            ...
            if IsControlJustPressed(0, 38) then
                ...
                if job == ("police") or job == ("ambulance") then
                    ...
                    if not IsPedInAnyVehicle(ped) then
                        if not bezig then
                            QBCore.Functions.TriggerCallback("fortis-cayoweed:server:checkPlukken", function(magPlukken, tijdOver)
                                if magPlukken then
                                    ...
                                    TriggerEvent("fortis-cayoweed:client:plukwiet")
                                    bezig = true
                                    aantalPlantjes = 0
                                else
            ...
            if GetDistanceBetweenCoords(pos, Config.Locaties["verpak"].x, Config.Locaties["verpak"].y, Config.Locaties["verpak"].z, true) < 2.5 then
                ...
                if IsControlJustPressed(0, 38) then
                    ...
                    if job == ("police") or job == ("ambulance") then
                        ...
                        if not IsPedInAnyVehicle(ped) then
                            QBCore.Functions.TriggerCallback("fortis-cayoweed:server:checkSpullen", function(resultaat, notificatie)
                                if resultaat then
...
RegisterNUICallback("verpakGeslaagd", function(data)
    TriggerServerEvent("fortis-cayoweed:server:verpakWiet")
    ClearPedTasks(GetPlayerPed(-1))
    SetNuiFocus(false, false)
end)

RegisterNUICallback("verpakGefaald", function(data)
    TriggerServerEvent("fortis-cayoweed:server:verpakFaal")
    ClearPedTasks(GetPlayerPed(-1))
...
AddEventHandler("fortis-cayoweed:client:plukwiet", function()
    Citizen.CreateThread(function()
...
RegisterNUICallback("plukGeslaagd", function(data)
    if bezigPluk then
        ...
        TriggerServerEvent("fortis-cayoweed:server:krijgwiet")
        TriggerEvent("fortis-cayoweed:client:plukwiet")
    else
        TriggerServerEvent("fortis-cayoweed:server:kauloHacker")
    end
end)
...
Citizen.CreateThread(function()
    QBCore.Functions.TriggerCallback("fortis-cayoweed:server:checkDealer", function(locatie)
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if GetDistanceBetweenCoords(pos, dealerLocatie.dealer.x, dealerLocatie.dealer.y, dealerLocatie.dealer.z, true) < 2 then
            ...
            if IsControlJustPressed(0, 38) then
                TriggerServerEvent("fortis-cayoweed:server:verkoopWiet")
            end
        end
        if letsleep then
            Citizen.Wait(1000)

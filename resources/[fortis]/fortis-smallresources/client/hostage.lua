...
function takeHostage()
	...
	if closestPlayer ~= nil and closestPlayer ~= -1 then
		...
		for i=1, #hostageAllowedWeapons do
			if HasPedGotWeapon(GetPlayerPed(-1), GetHashKey(hostageAllowedWeapons[i]), false) then
				if GetAmmoInPedWeapon(GetPlayerPed(-1), GetHashKey(hostageAllowedWeapons[i])) > 0 then
					...
					QBCore.Functions.Notify("Je hebt geen kogels!")
					return			
				end	
			else
				NoWeapon = true	
		...
		if NoWeapon then
			QBCore.Functions.Notify("Je kan niemand vastpakken met dit wapen!")
			return
		end
		...
		QBCore.Functions.Notify("Geen speler dichtbij!")
	end
...
Citizen.CreateThread(function()
	while true do 
		if holdingHostage then
			...
			QBCore.Functions.DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,"~g~G~w~ - Duwen, ~g~H~w~ - Doden")

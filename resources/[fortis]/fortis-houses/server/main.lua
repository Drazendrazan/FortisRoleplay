...
QBCore.Functions.CreateCallback('fortis-houses:server:gotMoney', function(source, cb, price)
	local src     	= source
	local pData 	= QBCore.Functions.GetPlayer(src)
	local bankBalance = pData.PlayerData.money["bank"]
...
QBCore.Functions.CreateCallback('fortis-houses:server:getHouseOwner', function(source, cb, house)
	cb(houseownercid[house])
end)

QBCore.Functions.CreateCallback('fortis-houses:server:getHouseKeyHolders', function(source, cb, house)
	local retval = {}

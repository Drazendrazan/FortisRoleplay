...
AddEventHandler('qb-houses:client:addGarage', function()
    if closesthouse ~= nil then 
        ...
        QBCore.Functions.Notify("Er is geen huis dichtbij!", "error")
    end
end)
    ...
    if (GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, true) < 1.5)then
        if hasKey then
            if Config.Houses[closesthouse].locked then
                ...
                QBCore.Functions.Notify("Huis staat open!", "success", 2500)
            else
                TriggerServerEvent('qb-houses:server:lockHouse', true, closesthouse)
                QBCore.Functions.Notify("Huis staat op slot!", "error", 2500)
            end
        else
            QBCore.Functions.Notify("Je hebt de sleutels van het huis niet!", "error", 3500)
        end
    else
        QBCore.Functions.Notify("Er is geen deur dichtbij!", "error", 3500)
    end
end)
            ...
            if(GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, false) < 30)then
                ...
                if hasKey then
                    ...
                    if not inside then
                        if closesthouse ~= nil then
                            if(GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, true) < 1.5)then
                                DrawText3Ds(Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, '~g~E~w~ - Betreden')
                    ...
                    if CurrentDoorBell ~= 0 then
                        if(GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[closesthouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[closesthouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z, true) < 1.5)then
                            DrawText3Ds(Config.Houses[closesthouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[closesthouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[closesthouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z + 0.35, '~g~G~w~ - Openen')
                            ...
                            if POIOffsets ~= nil then
                                if POIOffsets.exit ~= nil then
                                    if(GetDistanceBetweenCoords(pos, Config.Houses[CurrentHouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[CurrentHouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[CurrentHouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z, true) < 1.5)then
                                        DrawText3Ds(Config.Houses[CurrentHouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[CurrentHouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[CurrentHouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z, '~g~E~w~ - Verlaten')
                        ...
                        if closesthouse ~= nil then
                            if(GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, true) < 1.5)then
                                if not viewCam and Config.Houses[closesthouse].locked then
                                    DrawText3Ds(Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, '~g~E~w~ - Bekijken')
                                    if IsControlJustPressed(0, Keys["E"]) then
                                        TriggerServerEvent('qb-houses:server:viewHouse', closesthouse)
                                    end
                                end
                    ...
                    elseif isOwned then
                        if closesthouse ~= nil then
                            if not inOwned then
                                 if(GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, true) < 1.5)then
                                     if not Config.Houses[closesthouse].locked then
                                         DrawText3Ds(Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, '~g~Ontgrendeld~w~ / ~g~E~w~ - Betreden')
                                         if IsControlJustPressed(0, Keys["E"])  then
                                             enterNonOwnedHouse(closesthouse)
                                         end
                                     else
                                         DrawText3Ds(Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, '~r~Vergrendeld~w~ / ~g~G~w~ - Aanbellen')
                                         if IsControlJustPressed(0, Keys["G"]) then
                                             TriggerServerEvent('qb-houses:server:RingDoor', closesthouse)
											 CurrentDoorBell = 0
                                         end
                    ...
                    --[[if not inside then
                        if closesthouse ~= nil then
                            if(GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, true) < 1.5) then
								if Config.Houses[closesthouse].locked then
									DrawText3Ds(Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z - Config.MinZOffset + 0.35, 'De deur is ~r~vergrendeld / ~g~G~w~ - Aanbellen')
									if IsControlJustPressed(0, Keys["G"]) then
										enterOwnedHouse(closesthouse)
									end
								else
									DrawText3Ds(Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z - Config.MinZOffset + 0.35, '[~g~E~w~] Om naar ~b~binnen~w~ te gaan')
									if IsControlJustPressed(0, Keys["G"]) then
										enterOwnedHouse(closesthouse)
									end
								end
                        ...
                        if logoutLocation ~= nil then
                            if(GetDistanceBetweenCoords(pos, logoutLocation.x, logoutLocation.y, logoutLocation.z, true) < 1.5)then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    ...
                                    exports['fortis-interior']:DespawnInterior(houseObj, function()
...
AddEventHandler('qb-houses:client:RingDoor', function(player, house)
    if closesthouse == house and inside then
        ...
        QBCore.Functions.Notify("Er wordt aangebeld!")
    end
end)
...
AddEventHandler('qb-houses:client:giveHouseKey', function(data)
    ...
    if player ~= -1 and distance < 2.5 and closesthouse ~= nil then
        ...
        if housedist < 10 then
            ...
            QBCore.Functions.Notify("Je bent niet in de buurt van het huis!", "error")
        end
        ...
        if housedist < 5 then
            QBCore.Functions.TriggerCallback('qb-houses:server:getHouseOwner', function(result)
                if QBCore.Functions.GetPlayerData().citizenid == result then
                    ...
                    QBCore.Functions.Notify("Je bent niet de eigenaar!", "error")
                end
            end, closesthouse)
        else
            QBCore.Functions.Notify("Er is geen huis dichtbij!", "error")
...
function HouseKeysMenu()
    ...
    QBCore.Functions.TriggerCallback('qb-houses:server:getHouseKeyHolders', function(holders)
        ...
        if holders == nil or next(holders) == nil then
            QBCore.Functions.Notify("Geen sleutelhouders gevonden", "error", 3500)
            closeMenuFull()
        else
...
function removeOutfit(oData)
    ...
    QBCore.Functions.Notify(oData.outfitname.." succesvol verwijderd!", "success", 2500)
    closeMenuFull()
end
...
function enterOwnedHouse(house)
    ...
    if Config.Houses[house].tier == 1 then
        data = exports['fortis-interior']:CreateTier1House(coords)
    elseif Config.Houses[house].tier == 2 then
        data = exports['fortis-interior']:CreateTrevorsShell(coords)
    elseif Config.Houses[house].tier == 3 then
        data = exports['fortis-interior']:CreateMichaelShell(coords)
    elseif Config.Houses[house].tier == 4 then
        data = exports['fortis-interior']:CreateApartmentShell(coords) 
    elseif Config.Houses[house].tier == 5 then
        data = exports['fortis-interior']:CreateCaravanShell(coords)
    elseif Config.Houses[house].tier == 6 then
        data = exports['fortis-interior']:CreateFranklinShell(coords)
    elseif Config.Houses[house].tier == 7 then
        data = exports['fortis-interior']:CreateFranklinAuntShell(coords)
    elseif Config.Houses[house].tier == 8 then
        data = exports['fortis-interior']:CreateClassicHouse(coords)
    elseif Config.Houses[house].tier == 9 then
        data = exports['fortis-interior']:CreateClassicHouse2(coords)
    elseif Config.Houses[house].tier == 10 then
        data = exports['fortis-interior']:CreateClassicHouse3(coords)
    elseif Config.Houses[house].tier == 11 then
        data = exports['fortis-interior']:CreateHighEnd(coords)
    elseif Config.Houses[house].tier == 12 then
        data = exports['fortis-interior']:CreateHighEndV2(coords)
    elseif Config.Houses[house].tier == 13 then
        data = exports['fortis-interior']:CreateBarber(coords)
    elseif Config.Houses[house].tier == 14 then
        data = exports['fortis-interior']:CreateOffice1(coords)
    elseif Config.Houses[house].tier == 15 then
        data = exports['fortis-interior']:CreateLesterOsso(coords)
    elseif Config.Houses[house].tier == 16 then
        data = exports['fortis-interior']:CreateWarehouse1(coords)
    elseif Config.Houses[house].tier == 17 then
        data = exports['fortis-interior']:CreateWarehouse2(coords)
    elseif Config.Houses[house].tier == 18 then
        data = exports['fortis-interior']:CreateWarehouse3(coords)
    elseif Config.Houses[house].tier == 19 then
        data = exports['fortis-interior']:CreateGarageHigh(coords)
    elseif Config.Houses[house].tier == 20 then
        data = exports['fortis-interior']:CreateGarageMed(coords)
    elseif Config.Houses[house].tier == 21 then
        data = exports['fortis-interior']:CreateGarageLow(coords)
    elseif Config.Houses[house].tier == 22 then
        data = exports['fortis-interior']:CreateGunstore(coords)
    elseif Config.Houses[house].tier == 23 then
        data = exports['fortis-interior']:CreateMedium2(coords)
    elseif Config.Houses[house].tier == 24 then
        data = exports['fortis-interior']:CreateOfficeBig(coords)
    elseif Config.Houses[house].tier == 25 then
        data = exports['fortis-interior']:CreateStore2(coords)
    elseif Config.Houses[house].tier == 26 then
        data = exports['fortis-interior']:CreateStore3(coords)
    elseif Config.Houses[house].tier == 27 then
        data = exports['fortis-interior']:CreateContainer(coords)
    end
    ...
    TriggerEvent("fortis-bitcoin:client:betreedHuis", house)
...
function leaveOwnedHouse(house)
    if not FrontCam then
        ...
        exports['fortis-interior']:DespawnInterior(houseObj, function()
            ...
            TriggerEvent("fortis-bitcoin:client:verlaatHuis", house)
...
AddEventHandler('qb-houses:client:viewHouse', function(houseprice, firstname, lastname)
...
AddEventHandler('qb-houses:client:HomeInvasion', function()
    ...
    if closesthouse ~= nil then
        QBCore.Functions.TriggerCallback('police:server:HasPoliceJob', function(IsPresent)
            if IsPresent then
                local dist = GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, true)
                        ...
                        if not Config.Houses[closesthouse].IsRaming then
                            ...
                            Skillbar.Start({
                                ...
                                if RamsDone + 1 >= Config.RamsNeeded then
                                    TriggerServerEvent('qb-houses:server:lockHouse', false, closesthouse)
                                    QBCore.Functions.Notify('De deur is er succesvol uitgeramd!', 'success')
                                ...
                                QBCore.Functions.Notify('De deur was te sterk, probeer het nog een keer!', 'error')
                                DoRamAnimation(false)
                            end)
                            TriggerServerEvent('qb-houses:server:SetRamState', true, closesthouse)
                        else
                            QBCore.Functions.Notify('De deur wordt al geramd.', 'error')
                        end
                    else
                        QBCore.Functions.Notify('Dit huis staat al open!', 'error')
                    end
                else
                    QBCore.Functions.Notify('Er is geen deur dichtbij!', 'error')
                end
            else
                QBCore.Functions.Notify('Je bent geen hulpdienst!', 'error')
            end
        end)
    else
        QBCore.Functions.Notify('Er is geen deur dichtbij!', 'error')
...
AddEventHandler('qb-houses:client:ResetHouse', function()
    ...
    if closesthouse ~= nil then
        ...
        if Config.Houses[closesthouse].IsRammed then
            ...
            TriggerServerEvent('qb-houses:server:lockHouse', true, closesthouse)
            RamsDone = 0
            QBCore.Functions.Notify('Je hebt het huis op slot gedaan!', 'success')
        else
            QBCore.Functions.Notify('De deur is niet open gebroken!', 'error')
        end
    end
end)

function closestHuis()
    return closesthouse
end
	...
	while true do
		...
		if DecoMode then
			...
			if GetDistanceBetweenCoords(camPos.x, camPos.y, camPos.z, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, false) > 50.0 then
				DisableEditMode()
				closeDecorateUI()
				QBCore.Functions.Notify("Je bent uit de radius van je huis!")
			end
		end
	end
end)
...
AddEventHandler("qb-houses:client:decorate", function()
	...
	if inside then 
		if hasKey then 
			...
			QBCore.Functions.Notify("Je moet de sleutels hebben van het huis!", "error")
		end
	else
...
RegisterNUICallback("buySelectedObject", function(data, cb)
	QBCore.Functions.TriggerCallback('fortis-houses:server:gotMoney', function(has)
		if has then
			QBCore.Functions.Notify("Er is €" .. data.price .. " afgeschreven!", "success")
			SetNuiFocus(false, false)
			cursorEnabled = not cursorEnabled
			SaveDecorations()
			SelectedObj = nil

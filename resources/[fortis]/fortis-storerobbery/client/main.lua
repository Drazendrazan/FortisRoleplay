                ...
                if dist < 3 then
                    ...
                    if dist < 1.0 then
                        if not Config.Safes[safe].robbed then
                            DrawText3Ds(Config.Safes[safe].x, Config.Safes[safe].y, Config.Safes[safe].z, '~g~E~w~ - Kluis')
                            if IsControlJustPressed(0, Keys["E"]) then
                                ...
                                if job == ("police") or job == ("ambulance") then
                                    ...
                                    if CurrentCops >= Config.MinimumStoreRobberyPolice then
                                        ...
                                        QBCore.Functions.Notify("Onvoldoende agenten!", "error")
                                    end
                                end
                            end
                        else
                            DrawText3Ds(Config.Safes[safe].x, Config.Safes[safe].y, Config.Safes[safe].z, 'De kluis is leeg..')
                        end
                    end
                end
            end
    ...
    for k, v in pairs(Config.Registers) do
        ...
        if dist <= 1 and not Config.Registers[k].robbed then
            ...
            if job == ("police") then
                ...
                if CurrentCops >= Config.MinimumStoreRobberyPolice then
                    -- Kaasje
                    QBCore.Functions.TriggerCallback('qb-storerobbery:server:getRobberyTimeout', function(robberyTimeout)
                        if robberyTimeout > 0 then
                            QBCore.Functions.Notify("Er is te recent een kassa gekraakt, probeer het opnieuw over "..robberyTimeout.." seconden.")
                        else
                            if usingAdvanced then
                                ...
                                QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
                                    if result then
                                        ...
                                        QBCore.Functions.Notify("Het ziet er naar uit dat je een schroevendraaier nodig hebt!", "error")
                                    end
                                end, "screwdriverset")
                            end
                        end
    ...
    if currentRegister ~= 0 then
        QBCore.Functions.TriggerCallback('qb-storerobbery:server:getRobberyTimeout', function(robberyTimeout)
            if robberyTimeout > 0 then
                ...
                local lockpickTime = 45000
                LockpickDoorAnim(lockpickTime)
                QBCore.Functions.Progressbar("search_register", "Kassa legen..", lockpickTime, false, true, {
            ...
            -- trigger
            Wait(75000)
            TriggerServerEvent("qb-storerobbery:server:setRobberyTimeout", 300)
        end)
    else
        SendNUIMessage({
            action = "kekw",
...
RegisterNUICallback('fail', function()
    ...
    if (IsWearingHandshoes() and math.random(1, 100) <= 25) then
        local pos = GetEntityCoords(GetPlayerPed(-1))
        TriggerServerEvent("evidence:server:CreateFingerDrop", pos)
        QBCore.Functions.Notify("Je hebt jezelf gesneden aan de lockpick!")
    end
    lockpick(false)
end)

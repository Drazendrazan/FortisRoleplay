window.addEventListener("message", function(event) {
    if (event.data.type == "open") {
        ...
        fetch(`http://fortis-bitcoin/laptopData`, {
            method: 'POST',
            headers: {
...
document.onkeydown = function(evt) { /* Escape event */
    ...
    if (evt.key == "Escape") {
        ...
        $.post("http://fortis-bitcoin/sluiten", JSON.stringify({}));
    }
};
...
function BestelOnderdeelGPU() {
    ...
    if (x != 0 && x < 11) {
        $.post("http://fortis-bitcoin/bestellingStarten", JSON.stringify({
            type: "gpu",
            amount: x
        }));
        shitGekocht()
        ...
        document.getElementById("koop-button2").disabled = false;
        document.getElementById("koop-button1").disabled = false;
        $.post("http://fortis-bitcoin/alert", JSON.stringify({}));
    }
}
...
function uitbetalen() {
    ...
    $.post("http://fortis-bitcoin/minenUitbetalenCrypto", JSON.stringify({}));
    afsluiten();
    setTimeout(() => {
...
function serverVerwijderen(rackid) {
    $.post("http://fortis-bitcoin/verwijderKast", JSON.stringify({

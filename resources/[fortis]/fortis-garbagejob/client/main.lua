        ...
        if isLoggedIn then
            if PlayerJob.name == "garbage" then
                if distance < 10.0 then
                    ...
                    if distance < 1.5 then
                        if InVehicle then
                            DrawText3D(Config.Locations["vehicle"].coords.x, Config.Locations["vehicle"].coords.y, Config.Locations["vehicle"].coords.z, "~g~E~w~ - Parkeren")
                            if IsControlJustReleased(0, Keys["E"]) then
                                QBCore.Functions.TriggerCallback('qb-garbagejob:server:CheckBail', function(DidBail)
                                    if DidBail then
                                        BringBackCar()
                                    else
                                        QBCore.Functions.Notify("Je hebt niet betaald voor dit voertuig...")
                                    end
                                end)
                            end
                        else
                            DrawText3D(Config.Locations["vehicle"].coords.x, Config.Locations["vehicle"].coords.y, Config.Locations["vehicle"].coords.z, "~g~E~w~ - Garage")
                            if IsControlJustReleased(0, Keys["E"]) then
                                if Earnings > 0 then
                                    QBCore.Functions.Notify("Haal eerst je loonstrook op voordat je een nieuwe missie start!")
                                else
                                    QBCore.Functions.TriggerCallback('qb-garbagejob:server:HasMoney', function(HasMoney)
                                        if HasMoney then
                                            ...
                                            QBCore.Functions.SpawnVehicle("trash", function(veh)
                                                GarbageVehicle = veh
                                                SetVehicleNumberPlateText(veh, "VUIL"..tostring(math.random(1000, 9999)))
                                                ...
                                                IsWorking = true
                                                SetGarbageRoute()
                                                QBCore.Functions.Notify("Locatie succesvol ingesteld!")
                                            end, coords, true)
                                        else
                                            QBCore.Functions.Notify("Er is iets fout gegaan!")
                                        end
                                    end)
                                end
                            end
                        ...
                        if DeliveryBlip ~= nil then
                            ...
                            if Distance < 20 or hasZak then
                                ...
                                if not hasZak then
                                    if CanTakeBag then
                                        if Distance < 1.5 then
                                            DrawText3D2(DeliveryData.coords, "~g~E~w~ - Oppakken")
                                        ...
                                        elseif Distance < 10 then
                                            DrawText3D2(DeliveryData.coords, "Ga hier staan.")
                                        end
                                    end
                                else
                                    if DoesEntityExist(GarbageVehicle) then
                                        ...
                                        if TruckDist < 2 then
                                            DrawText3D(Coords.x, Coords.y, Coords.z, "~g~E~w~ - Smijten")
                                            if IsControlJustPressed(0, 51) then
                                                hasZak = false
                                                local AmountOfLocations = #Config.Locations["vuilnisbakken"]
                                                if (AmountOfBags - 1) == 0 then
                                                    Earnings = Earnings + math.random(450, 670)
                                                    if (GarbageLocation + 1) <= AmountOfLocations then
                                                        GarbageLocation = GarbageLocation + 1
                                                        SetGarbageRoute()
                                                        QBCore.Functions.Notify("Alles is opgehaald, ga naar de volgende!")
                                                    else
                                                        QBCore.Functions.Notify("Je bent klaar, ga naar de vuilnisbelt voor je loonstrook.")
                                                        IsWorking = false
                                                        RemoveBlip(DeliveryBlip)
                                                        SetRouteBack()
                                                    end
                                                    ...
                                                    if AmountOfBags > 1 then
                                                        QBCore.Functions.Notify("Er zijn nog "..AmountOfBags.." zakken over!")
                                                    else
                                                        QBCore.Functions.Notify("Er is nog "..AmountOfBags.." zak over!")
                                                    end
                                                    hasZak = false
                                                end
                                                DeliverAnim()

...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        case 27:
            ...
            $.post('http://fortis-occasions/close');
            break;
    }
});
...
$(document).on('click', '#sell-vehicle', function(){
    if ($('.vehicle-sell-price').val() != "") {
        if (!isNaN($('.vehicle-sell-price').val())) {
            $.post('http://fortis-occasions/sellVehicle', JSON.stringify({
            ...
            $.post('http://fortis-occasions/error', JSON.stringify({
...
$(document).on('click', '#buy-vehicle', function(){
    $.post('http://fortis-occasions/buyVehicle');
    $('.buy-container').fadeOut(100);
    $.post('http://fortis-occasions/close');
});

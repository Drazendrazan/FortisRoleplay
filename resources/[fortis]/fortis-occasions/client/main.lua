...
RegisterNetEvent("fortis-occasions:client:verkoopAuto")
AddEventHandler("fortis-occasions:client:verkoopAuto", function(prijs)
    if IsPedInAnyVehicle(PlayerPedId()) then
        if GetPedInVehicleSeat(GetVehiclePedIsIn(PlayerPedId(), false), -1) ~= 0 then
            ...
            TriggerServerEvent("fortis-occasions:server:dataCheck", kenteken, prijs)
        else
...
RegisterNetEvent("fortis-occasions:client:goedkeuringAuto")
AddEventHandler("fortis-occasions:client:goedkeuringAuto", function(prijs)
    local vehicleData = {}

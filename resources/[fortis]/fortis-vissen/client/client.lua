                ...
                if distance <= 3 and not gestart then
                    ...
                    if IsControlJustPressed(0, 38) then
                        if restartWacht > 0 then
                            ...
                            QBCore.Functions.TriggerCallback("fortis-vissen:server:checkHengel", function(callback)
                                if callback then
                                    local visspotNaam = locaties["naam"]
                                    TriggerEvent("fortis-vissen:client:prepareVissen", visspotNaam)
                                else
...
AddEventHandler("fortis-vissen:client:prepareVissen", function(visspotNaam)
    QBCore.Functions.TriggerCallback("fortis-vissen:server:vraagSpotAantal", function(visspotAantal)
        if visspotAantal <= 3 then
            TriggerServerEvent("fortis-vissen:server:reserveerVisspot", visspotNaam)
...
-- Start vissen
RegisterNetEvent("fortis-vissen:server:startVissen")
AddEventHandler("fortis-vissen:server:startVissen", function(visspot, visspotPlek)
    gestart = true
    local ped = GetPlayerPed(-1)
    local pedPositie = GetEntityCoords(ped)
    ...
    while true do
        ...
        if not stopTekst then
            ...
            if IsControlJustPressed(0, 47) then
                ...
                TriggerServerEvent("fortis-vissen:server:verwijderVisspot", visspotNaam, verwijderSpotNummer)
                Wait(100)
        ...
        if not vangen then
            ...
            TriggerEvent("fortis-vissen:client:vangen", wachtTijd)
        end
...
AddEventHandler("fortis-vissen:client:vangen", function(wachtTijd)
    ...
    if vangen then
        ...
        Skillbar.Start({
            ...
            if SucceededAttempts + 1 >= NeededAttempts then
                ...
                TriggerServerEvent("fortis-vissen:server:geefVis")
                SucceededAttempts = 0
                vangen = false
                stopTekst = false
                loadAnimDict("amb@lo_res_idles@")
...
Citizen.CreateThread(function()
    while true do
        ...
        if GetDistanceBetweenCoords(pedPositie, -1813.97, -1213.32, 13.01, true) < 2 then
            ...
            if IsControlJustPressed(0, 38) then
                TriggerServerEvent("fortis-vissen:server:verkoopVissen")
            end
        end

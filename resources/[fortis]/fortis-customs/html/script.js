...
$(document).ready(function(){
    console.log("fortis-customs: Succesvol geladen!");
...
function ChangeOption(type) {
    ...
    if (MenuData.CurrentModId !== "shoppingcart") {
        $.post('http://fortis-customs/OnIndexChange', JSON.stringify({
...
function SetupMods(mods, price) {
    $(".menu-options").html("");
    $("#menu-name").html("Welkom bij de tuningshop");
    ...
    if (mods.length > 0) {
        ...
        if (price !== null) {
            MenuData.TotalOptions = MenuData.TotalOptions + 1;
            var ModElement = '<div class="menu-option" data-menuindex="' + (MenuData.TotalOptions) + '"><div class="menu-option-name">Voertuig Repareren<div class="menu-option-price">$ ' + price + '</div></div></div>';
...
function SubmitOption() {
    ...
    if (OptionData.id !== "repairvehicle") {
        if (OptionData.id !== "close-menu" && OptionData.id !== "shoppingcart" && OptionData.id !== "main-menu" && OptionData.id !== "buy-upgrades") {
            ...
            $.post('http://fortis-customs/GetCurrentMod', JSON.stringify({
                data: OptionData
                ...
                $.post('http://fortis-customs/GetCartItem', JSON.stringify({
                    data: checkdata,
                }), function(marketitem){
                    if (OptionData.id !== null && OptionData.id !== undefined) {
                    ...
                    if (OptionData.buttons !== null && OptionData.buttons !== undefined) {
                        $.post('http://fortis-customs/SelectSound');
                        if (OptionData.buttons.length > 0) {
                            $(".menu-options").html("");
                            $("[data-menuindex='" + MenuData.SelectedIndex + "']").removeClass('menu-option-selected');
                            $("#menu-name").html(OptionData.label);
                            $("#menu-help").html("Selecteer het gewenste onderdeel");
                            MenuData.TotalOptions = 0;
                                ...
                                if (mod.price !== null && mod.price !== undefined) {
                                    ...
                                    if (MenuData.CurrentModId == "respray" || MenuData.CurrentModId == "wheels" || MenuData.CurrentModId == "wheelaccessories" || installedmod > -1) {
                                        if (installedmod > -1 && i == installedmod + 1) {
                                            ...
                                            $.post('http://fortis-customs/ToggleCartedItem', JSON.stringify({toggle: true}));
                                        } else if (mod.price == 0) {
                                            mod.bought = false;
                            ...
                            if (!OptionData.cart || OptionData.cart === undefined){
                                ...
                                if (CurrentEquiped.length > 0) {
                                    ...
                                    if (!$("[data-menuindex='" + MenuData.SelectedIndex + "']").hasClass('cart-option')){
                                        ...
                                        $.post('http://fortis-customs/RemoveCartItem', JSON.stringify({
                                        ...
                                        $.post('http://fortis-customs/AddItemToCart', JSON.stringify({
                                            ItemData: OptionData,
                                        }));
                                    } else {
                                        $(".cart-option").find('.menu-option-price').html(pricelabel);
            ...
            $.post('http://fortis-customs/SelectSound');
            $.post('http://fortis-customs/PurchaseUpgrades');
            ...
            $("#menu-help").html("Totale Kosten: $" + TotalCartPrice);
    
            MenuData.TotalOptions = MenuData.TotalOptions + 1;
            $(".menu-options").append('<div class="menu-option" data-menuindex="' + MenuData.TotalOptions + '"><div class="menu-option-name">Bevestigen<div class="menu-option-price">$' + TotalCartPrice + '</div></div></div>');
            $("[data-menuindex='" + MenuData.TotalOptions + "']").data('OptionData', {
                id: "buy-upgrades",
            });
            ...
            $.post('http://fortis-customs/QuitSound');
            $(".menu-container").animate({
                left: -45+"vh",
            }, 300, function(){
                $(".qbus-customs").css({"display":"none"});
            ...
            MenuData.SubMenu = [];
            MenuData.CurrentMenu = 0;
            $.post('http://fortis-customs/CloseMenu');
        }
    } else {
        $.post('http://fortis-customs/CanRepairVehicle', JSON.stringify({
...
function Back() {
    $.post('http://fortis-customs/BackSound');
    if (MenuData.CurrentMenu !== "shoppingcart") {
        if (MenuData.SubMenu[0] !== null && MenuData.SubMenu[0] !== undefined) {
            if (MenuData.CurrentMenu - 1 !== 0) {
                ...
                $.post('http://fortis-customs/CheckIfCartedItem');
                $(".menu-options").html("");
                $("#menu-name").html("Welkom bij de tuningshop");
                $("#menu-help").html("Kies een menu");
                MenuData.TotalOptions = 0;
            ...
            if (MenuData.CurrentMenu - 1 >= -1) {
                $.post('http://fortis-customs/GetShoppingCart', JSON.stringify({}), function(ShoppingCart){
                    $(".menu-options").html("");
                    var ModElement;
                    if (ShoppingCart.buttons.length > 0) {
                        ModElement = '<div class="menu-option winkelwagen" data-menuindex="1"><div class="menu-option-name">Winkelwagen <div class="menu-option-price"><i class="fas fa-shopping-cart equiped-check"></i> ' + ShoppingCart.buttons.length + '</div></div></div>'+
                        '<div class="menu-option" data-menuindex="2"><div class="menu-option-name">Tuning</div></div>' +
                        '<div class="menu-option" data-menuindex="3"><div class="menu-option-name">Sluiten</div></div>';
                        ...
                        ModElement = '<div class="menu-option" data-menuindex="1"><div class="menu-option-name">Tuning</div></div>'+
                        '<div class="menu-option" data-menuindex="2"><div class="menu-option-name">Sluiten</div></div>';
                        $(".menu-options").append(ModElement);

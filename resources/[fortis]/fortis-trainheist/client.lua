    ...
    QBCore.Functions.TriggerCallback('trainheist:server:checkPoliceCount', function(status)
        if status then
            QBCore.Functions.TriggerCallback('trainheist:server:checkTime', function(time)
                if time then
                    TriggerServerEvent("fortis-trainheist:server:triggerscoreboard")
                    QBCore.Functions.Notify(Strings['goto_ambush'], "success", 15000)
    ...
    while trainLoop do
        ...
        for k, v in pairs(Config['TrainHeist']['setupTrain']['containers']) do
            ...
            if dist <= 1.5 and not v['lock']['taken'] then
                ...
                if not melding then
                    QBCore.Functions.Notify(Strings['cutting'])
                    melding = true
                end 
                if IsControlJustPressed(0, 38) then
                    OpenContainer(k)
...
function FinishHeist()
    ...
    QBCore.Functions.Notify(Strings['deliver_to_buyer'])
...
RegisterNetEvent('fortis-trainheist:client:belPolitieBericht')
AddEventHandler('fortis-trainheist:client:belPolitieBericht', function(msg, streetLabel, coords)
    ...
    BeginTextCommandSetBlipName('STRING')
    AddTextComponentString("112 - Merryweather trein overval")
    EndTextCommandSetBlipName(blip)
    while transG ~= 0 do
        Wait(180 * 4)
        transG = transG - 1
...
function belPolitie()
    ...
    TriggerServerEvent('fortis-trainheist:server:belPolitie', streetLabel, pos)
end
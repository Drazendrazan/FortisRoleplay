...
AddEventHandler("QBCore:Client:OnPlayerLoaded", function()
	QBCore.Functions.TriggerCallback("fortis-cardealer:server:ontvangConfig", function(config)
        SendNUIMessage({
            type = "importeer",
            data = config
        })
...
RegisterNUICallback("proefrit", function(data)
    if proefrit then
        TriggerServerEvent("fortis-cardealer:server:kauloHacker")
    else
        QBCore.Functions.TriggerCallback("fortis-cardealer:server:ontvangConfig", function(config)
            local bestaat = false
...
RegisterNUICallback("kopen", function(data)
    ...
    if inCarDealer then
        QBCore.Functions.TriggerCallback("fortis-cardealer:server:koopVoertuig", function(cb)
...
RegisterNetEvent("fortis-cardealer:client:voertuigGekocht")
AddEventHandler("fortis-cardealer:client:voertuigGekocht", function(voertuig, kenteken)
    SendNUIMessage({
        type = "gekocht"
    })
            ...
            if GetDistanceBetweenCoords(pedCoords, Config.Coords["verkoopCirkel"].x, Config.Coords["verkoopCirkel"].y, Config.Coords["verkoopCirkel"].z, true) < 4 and IsPedInAnyVehicle(GetPlayerPed(-1)) then
                if not verkoopEcht then
                    ...
                    if IsControlJustPressed(0, 161) or IsDisabledControlJustPressed(0, 161) and not verkoopBlock then
                        verkoopBlock = true
                        TriggerEvent("fortis-cardealer:client:blockVoertuigVerkoop")
                        verkoopEcht = false

                        local kenteken = GetVehicleNumberPlateText(GetVehiclePedIsIn(GetPlayerPed(-1)))
                        
                        QBCore.Functions.TriggerCallback("fortis-cardealer:server:verkoopVoertuig", function(owner, prijs)
                            if owner then
...
AddEventHandler("fortis-cardealer:client:blockVoertuigVerkoop", function()
    Wait(120000)
    verkoopBlock = false
end)
...
-- Anti cheat
RegisterNetEvent("fortis-cardealer:client:hackerZemmer")
AddEventHandler("fortis-cardealer:client:hackerZemmer", function()
    TriggerServerEvent("fortis-cardealer:server:kauloHacker")
end)

                    ...
                    if not IsPedInAnyVehicle(GetPlayerPed(-1), false) then
                        if not bezig then
                            ...
                            if IsControlJustPressed(0, 38) and not bezig then
                                starten = true
                                TriggerEvent("fortis-sleutelmaker:client:startWerk")
...
AddEventHandler("fortis-sleutelmaker:client:startWerk", function()
    ...
    if starten then
        ...
        if vrijMail then
            ...
            TriggerServerEvent("qb-phone:server:sendNewMail", {
                ...
                button = {
                    enabled = true,
                    buttonEvent = "fortis-sleutelmaker:client:acepteer", Oorzaak1
                }
            })
            vrijMail = false
            return
...
AddEventHandler("fortis-sleutelmaker:client:acepteer", function(Oorzaak1)
    if starten then
        DeleteEntity(NPC)
        Citizen.Wait(10)
        DeleteEntity(NPC)
            ...
            if GetDistanceBetweenCoords(pedPos, locatie3.x, locatie3.y, locatie3.z, false) <50 and starten then
                ...
                if GetDistanceBetweenCoords(pedPos, locatie3.x, locatie3.y, locatie3.z, false) < 2 then
                    ...
                    if IsControlJustPressed(0, 38) then
                        ...
                        Citizen.Wait(2500)
                        TriggerEvent("fortis-sleutelmaker:client:startWerk2", gekozenUitleg, gekozenVoordeur, uitleg, locatie, locatieBeveiliging)
                        return
                    end    
                end
            end
...
AddEventHandler("fortis-sleutelmaker:client:startWerk2", function(gekozenUitleg, gekozenVoordeur, uitleg, locatie, locatieBeveiliging)
                    ...
                    if GetDistanceBetweenCoords(pedPos, locatie3.x, locatie3.y, locatie3.z, false) <2 then
                        ...
                        if IsControlJustPressed(0, 38) and successLockpick then
                            ...
                            if math.random(1, 10) ~= 8 then
                                TriggerServerEvent("fortis-sleutelmaker:server:successBetaald")
                                return
                            else
                                TriggerServerEvent("fortis-sleutelmaker:server:successBetaaldExtra")
                                return
                            end
                        ...
                        if vervangen then
                            if IsControlJustPressed(0, 38) then
                                if math.random(1, 8) ~= 8 then
                                    TriggerServerEvent("fortis-sleutelmaker:server:successBetaald", vervangen)
                                    vrijMail = true
                                    NPCSpawn = false
                                    vervangen = false
                                    text = true
                                    ...
                                    TriggerServerEvent("fortis-sleutelmaker:server:successBetaaldExtra", vervangen)
                                    letsleep = true
                                    vrijMail = true
                                    NPCSpawn = false
                                    vervangen = false

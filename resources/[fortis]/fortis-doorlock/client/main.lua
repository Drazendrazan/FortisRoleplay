		...
		for k,doorID in ipairs(QB.Doors) do
			...
			if distance < maxDistance then
				...
				if doorID.objCoords == nil then
					...
					if doorID.objCoords ~= nil then
						doorID.textCoords = doorID.textCoords
					end
				end
				...
				if QB.Doors[k].pickable then
					if QB.Doors[k].locked then
						if hasItem then
							...
							QBCore.Functions.Notify("Je lockpick is te slap!", "error")
						end
					else
...
function IsAuthorized(doorID)
	...
	for _,job in pairs(doorID.authorizedJobs) do
		if job == PlayerData.job.name then -- openen via een job zoals bijv. police of ambulance in de config onder authorizedJobs
			return true
		else
			if job == PlayerData.citizenid then -- openen op basis van citizenID in de config onder authorizedJobs
				return true
			end
		end
	end

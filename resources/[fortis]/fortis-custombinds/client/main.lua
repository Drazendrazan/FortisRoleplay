...
RegisterNetEvent('fortis-custombinds:client:openUI')
AddEventHandler('fortis-custombinds:client:openUI', function()
    openBindingMenu()
end)
                ...
                if IsControlJustPressed(0, Keys[v]) or IsDisabledControlJustPressed(0, Keys[v]) then
                    ...
                    if next(keyMeta) ~= nil then
                        if keyMeta[v]["command"] ~= "" then
                            ...
                            QBCore.Functions.Notify('Er is niks gebind aan ['..v..'], gebruik /binds om een commando te binden.', 'primary', 4000)
                        end
                    else
                        QBCore.Functions.Notify('Je hebt geen commandos gebind, /binds om een commando te binden.', 'primary', 4000)
                    end
                end
            end
...
RegisterNUICallback('save', function(data)
    ...
    QBCore.Functions.Notify('Commandos succesvol opgeslagen!', 'success')

    TriggerServerEvent('fortis-custombinds:server:setKeyMeta', keyData)
end)
...
RegisterNetEvent("fortis-humaneheist:server:belPolitie")
AddEventHandler("fortis-humaneheist:server:belPolitie", function(straat, coords)
    ...
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        ...
        if Player ~= nil then
            if (Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty) then
                TriggerClientEvent("fortis-humaneheist:client:belPolitieBericht", Player.PlayerData.source, msg, straat, coords)
...
RegisterNetEvent("fortis-humaneheist:server:triggerscoreboard")
AddEventHandler("fortis-humaneheist:server:triggerscoreboard", function()
...
RegisterNetEvent("fortis-humaneheist:server:koopArtikelen")
AddEventHandler("fortis-humaneheist:server:koopArtikelen", function(artikel, prijs)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if artikel == "bag" or artikel == "cutter" or artikel == "laptop" and prijs == 1500 or prijs == 5000 or prijs == 2000 then
      ...
      TriggerEvent("fortis-humaneheist:server:kauloHacker", source)  
    end
 
  
end)

RegisterNetEvent("fortis-humaneheist:server:kauloHacker")
AddEventHandler("fortis-humaneheist:server:kauloHacker", function(source)
    local src = source
    ...
    DropPlayer(src, "Hacken binnen het humaneheist script: https://fortisroleplay.nl/discord")
end)

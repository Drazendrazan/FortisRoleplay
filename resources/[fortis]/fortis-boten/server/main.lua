...
AddEventHandler('qb-diving:server:BuyBoat', function(boatModel, BerthId)
    ...
    elseif PlayerMoney.bank >= BoatPrice then
        ...
        TriggerClientEvent('QBCore:Notify', src, 'Je hebt niet genoeg geld je mist €'..missingMoney, 'error', 4000)
    end
end)
...
QBCore.Functions.CreateCallback('qb-diving:server:GetMyBoats', function(source, cb, dock)
    ...
    if dock ~= nil then
        QBCore.Functions.ExecuteSql(true, "SELECT * FROM `player_boats` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."' AND `boathouse` = '"..dock.."'", function(result)
...
QBCore.Commands.Add("wetsuit", "Trekt je duikpak uit.", {}, false, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
...
function HasCoral(src)
    ...
    for k, v in pairs(QBDiving.ScrapTypes) do

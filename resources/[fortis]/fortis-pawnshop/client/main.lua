			...
			if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.PawnLocation.x, Config.PawnLocation.y, Config.PawnLocation.z, true) < 1.5 then
				if GetClockHours() >= 1 and GetClockHours() <= 4 then
					...
					elseif sellItemsSet and sellPrice ~= 0 then
						DrawText3D(Config.PawnLocation.x, Config.PawnLocation.y, Config.PawnLocation.z, "~g~E~w~ - Verkopen (€"..sellPrice..")")
						...
						DrawText3D(Config.PawnLocation.x, Config.PawnLocation.y, Config.PawnLocation.z, "~g~Winkel~w~ - Je hebt niks om te verkopen..")
					end
				else
					DrawText3D(Config.PawnLocation.x, Config.PawnLocation.y, Config.PawnLocation.z, "Winkel gesloten, opent om ~r~1:00")
				end
			end
		end
		if not inRange then

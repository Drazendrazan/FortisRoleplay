...
Citizen.CreateThread(function()
	while true do
		...
		if NetworkIsSessionStarted() then
			TriggerEvent("fortis-multicharacter:client:kiesKarakter")
			return
		end
	end
end)
...
function getKarakters()
    QBCore.Functions.TriggerCallback("fortis-multicharacter:server:getKarakters", function(result)
        SendNUIMessage({
            type = "sendKarakters",
            karakters = result
        })
...
RegisterNetEvent("fortis-multicharacter:client:kiesKarakter")
AddEventHandler("fortis-multicharacter:client:kiesKarakter", function()
    if charPed ~= nil then
    ...
    TriggerEvent("fortis-smallresources:client:geenHPbar")
...
RegisterNUICallback("selectKarakter", function(data)
    ...
    QBCore.Functions.TriggerCallback("fortis-multicharacter:server:getSkin", function(model, data)
...
RegisterNUICallback("speelKarakter", function(data)
    ...
    Wait(2000)
    DoScreenFadeOut(200)
    TriggerServerEvent("fortis-multicharacter:server:loadUserData", data["data"])
    SetEntityAsMissionEntity(charPed, true, true)
    DeleteEntity(charPed)
    SetNuiFocus(false, false)
    TriggerEvent("fortis-hud:client:laadHudZienChar")
...
RegisterNUICallback("maakNieuwKarakter", function(rawData)
    if string.find(rawData["data"].voornaam:lower(), "<script") or string.find(rawData["data"].achternaam:lower(), "<script") then
        TriggerServerEvent("fortis-multicharacter:server:banSpelerHacker")
    else
        ...
        DoScreenFadeOut(150)
        TriggerEvent("fortis-hud:client:laadHudZienChar")
        TriggerServerEvent("fortis-multicharacter:server:maakNieuwKarakter", cData)
        Citizen.Wait(500)
    end
end)

RegisterNetEvent("fortis-multicharacter:client:closeNUI")
AddEventHandler("fortis-multicharacter:client:closeNUI", function()
    SetNuiFocus(false, false)
end)


RegisterNUICallback("verwijderKarakter", function(data)
    TriggerServerEvent("fortis-multicharacter:server:deleteCharacter", data["data"].citizenid)
end)

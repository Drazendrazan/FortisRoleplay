...
RegisterNetEvent("fortis-screenshots:server:adminMenuMaakScreen")
AddEventHandler("fortis-screenshots:server:adminMenuMaakScreen", function(targetId)
...
QBCore.Functions.CreateCallback("fortis-screenshots:server:twitterMaakFoto", function(source, cb)
    ...
    function(error, data)
        if not error then
            local url = "http://51.89.91.147/tweets/"..fotoid..".jpg"
            cb(true, url)
        else
            cb(false, "nee")
        end

...
RegisterNetEvent('fortis-gevangenis:client:stuurGevangenis')
AddEventHandler('fortis-gevangenis:client:stuurGevangenis', function(maandenGeven, spelerID)
    Player = QBCore.Functions.GetPlayerData()
    if Player.job.name == "police" then
        if Player.job.onduty then
            -- Speler mag mensen naar de gevangenis sturen
            if maandenGeven > 0 then
                TriggerServerEvent("fortis-gevangenis:server:stuurGevangenis", maandenGeven, spelerID)
            else
...
RegisterNetEvent("fortis-gevangenis:client:uitzitten")
AddEventHandler("fortis-gevangenis:client:uitzitten", function(maandenDB, communicatieAfpakken)
    ...
    if communicatieAfpakken then -- Gebeurt alleen als je eerste keer jilla in gaat
        TriggerServerEvent("fortis-gevangenis:server:pakTelliesAf")
    ...
    maanden = maandenDB
    TriggerEvent("fortis-gevangenis:client:jobs")
    ...
    while maanden > 0 do
        if not uitgebroken then
            ...
            Citizen.Wait(60000)
            maanden = maanden - 1
            TriggerServerEvent("fortis-gevangenis:server:updateMaanden", maanden)
        else
            return
        end
    end
    ...
    if not uitgebroken then
        TriggerEvent("fortis-gevangenis:client:uitchecken")
    end
end)


AddEventHandler("fortis-gevangenis:client:jobs", function()
        ...
        if not klaar then
                            ...
                            QBCore.Functions.Progressbar("opdrukken", "Aan het voorbereiden...", 5000, false, false, {
                                ...
                                Skillbar.Start({
                                    ...
                                    if SucceededAttempts + 1 >= NeededAttempts then
                                        ClearPedTasks(ped)
                                        SucceededAttempts = 0
                                        TriggerServerEvent("fortis-gevangenis:server:krijgGeld")
                                    else
                                        Skillbar.Repeat({
                        ...
                        if not smeren then
                            ...
                            QBCore.Functions.Progressbar("smeren", "Broodjes aan het uitpakken...", 10000, false, false, {
                                ...
                                Skillbar.Start({
                                    ...
                                    if SucceededAttempts + 1 >= NeededAttempts then
                                        ClearPedTasks(ped)
                                        TriggerServerEvent("fortis-gevangenis:server:krijgBroodjes")
                                    else
                                        Skillbar.Repeat({
                                            duration = math.random(1200, 1600),
                                            pos = math.random(10, 40),
                ...
                if kanOmkopen then
                    if GetDistanceBetweenCoords(pos, Config.Locaties["guardLocatie"].x, Config.Locaties["guardLocatie"].y, Config.Locaties["guardLocatie"].z) < 2.5 then
                        ...
                        if IsControlJustPressed(0, 38) then
                            kanOmkopen = false
                            if maanden > 5 then
                                TriggerServerEvent("fortis-gevangenis:server:guardOmkopen")
                                if kans == 3 then
                                    maanden = maanden - 3
                                    TriggerServerEvent("fortis-gevangenis:server:updateMaanden", maanden)
            ...
            if GetDistanceBetweenCoords(pos, Config.Locaties["schoonLocatie"].x, Config.Locaties["schoonLocatie"].y, Config.Locaties["schoonLocatie"].z) < 2.5 then
                ...
                if IsControlJustPressed(0, 47) then
                    if schoonmaken then 
                        schoonmaken = false
                        TriggerEvent("fortis-gevangenis:client:schoonmaken")
...
AddEventHandler("fortis-gevangenis:client:schoonmaken", function()
    Citizen.CreateThread(function()
...
AddEventHandler("fortis-gevangenis:client:uitchecken", function()
    if not uitgebroken then
            ...
            if GetDistanceBetweenCoords(pos, 1789.09, 2596.48, 46.00, true) < 3 and klaar then
                ...
                if IsControlJustPressed(0, 38) then
                    QBCore.Functions.Progressbar("uitchecken", "Aan het uitchecken...", 4500, false, false, {
                        ...
                        TriggerServerEvent("fortis-gevangenis:server:klaarMetStraf")
    ...
    while true do
        ...
        if maanden > 0 then
            if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), 1779.20, 2583.55, 45.79) > 350 then
                ...
                maanden = -1
                bezig = false
                TriggerServerEvent("fortis-gevangenis:server:klaarUitbraak")
            end
        else
            Wait(1500)
        end

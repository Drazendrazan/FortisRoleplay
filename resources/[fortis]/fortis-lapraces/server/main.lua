...
QBCore.Functions.CreateCallback('fortis-lapraces:server:GetRacingLeaderboards', function(source, cb)
    cb(Races)
end)
...
RegisterServerEvent('fortis-lapraces:server:FinishPlayer')
AddEventHandler('fortis-lapraces:server:FinishPlayer', function(RaceData, TotalTime, TotalLaps, BestLap)
    local src = source
    ...
    if Races[RaceData.RaceId].Records ~= nil and next(Races[RaceData.RaceId].Records) ~= nil then
        if BLap < Races[RaceData.RaceId].Records.Time then
            ...
            TriggerClientEvent('qb-phone_new:client:RaceNotify', src, 'Je hebt het record verbroken van: '..RaceData.RaceName..' met een tijd van: '..SecondsToClock(BLap)..'!')
        end
    else
        Races[RaceData.RaceId].Records = {
            Time = BLap,
        ...
        QBCore.Functions.ExecuteSql(false, "UPDATE `lapraces` SET `records` = '"..json.encode(Races[RaceData.RaceId].Records).."' WHERE `raceid` = '"..RaceData.RaceId.."'")
        TriggerClientEvent('qb-phone_new:client:RaceNotify', src, 'Je hebt het record gezet op: '..RaceData.RaceName..' met een tijd van: '..SecondsToClock(BLap)..'!')
    end
    AvailableRaces[AvailableKey].RaceData = Races[RaceData.RaceId]
    TriggerClientEvent('fortis-lapraces:client:PlayerFinishs', -1, RaceData.RaceId, PlayersFinished, Player)
...
RegisterServerEvent('fortis-lapraces:server:CreateLapRace')
AddEventHandler('fortis-lapraces:server:CreateLapRace', function(RaceName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    
    if IsWhitelisted(Player.PlayerData.citizenid) then
        if IsNameAvailable(RaceName) then
            TriggerClientEvent('fortis-lapraces:client:StartRaceEditor', source, RaceName)
        else
            TriggerClientEvent('QBCore:Notify', source, 'Er is al een race met die naam.', 'error')
        end
    else
        TriggerClientEvent('QBCore:Notify', source, 'Je bent niet gerechtigd om races te maken.', 'error')
    end
end)

QBCore.Functions.CreateCallback('fortis-lapraces:server:GetRaces', function(source, cb)
    cb(AvailableRaces)
end)

QBCore.Functions.CreateCallback('fortis-lapraces:server:GetListedRaces', function(source, cb)
    cb(Races)
end)

QBCore.Functions.CreateCallback('fortis-lapraces:server:GetRacingData', function(source, cb, RaceId)
    cb(Races[RaceId])
end)

QBCore.Functions.CreateCallback('fortis-lapraces:server:HasCreatedRace', function(source, cb)
    cb(HasOpenedRace(QBCore.Functions.GetPlayer(source).PlayerData.citizenid))
end)

QBCore.Functions.CreateCallback('fortis-lapraces:server:IsAuthorizedToCreateRaces', function(source, cb, TrackName)
...
QBCore.Functions.CreateCallback('fortis-lapraces:server:GetTrackData', function(source, cb, RaceId)
...
RegisterServerEvent('fortis-lapraces:server:JoinRace')
AddEventHandler('fortis-lapraces:server:JoinRace', function(RaceData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if CurrentRace ~= nil then
        ...
        if (AmountOfRacers - 1) == 0 then
            ...
            TriggerClientEvent('QBCore:Notify', src, 'Je bent de enige in de race, de race zal nu stoppen.', 'error')
            TriggerClientEvent('fortis-lapraces:client:LeaveRace', src, Races[CurrentRace])
        else
    ...
    TriggerClientEvent('fortis-lapraces:client:JoinRace', src, Races[RaceId], RaceData.Laps)
    TriggerClientEvent('qb-phone_new:client:UpdateLapraces', -1)
...
RegisterServerEvent('fortis-lapraces:server:LeaveRace')
AddEventHandler('fortis-lapraces:server:LeaveRace', function(RaceData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local RaceName
    ...
    if (AmountOfRacers - 1) == 0 then
        ...
        table.remove(AvailableRaces, AvailableKey)
        TriggerClientEvent('QBCore:Notify', src, 'Je was de enige in de race, de race zal nu stoppen.', 'error')
        TriggerClientEvent('fortis-lapraces:client:LeaveRace', src, Races[RaceId])
...
RegisterServerEvent('fortis-lapraces:server:SetupRace')
AddEventHandler('fortis-lapraces:server:SetupRace', function(RaceId, Laps)
    ...
    if Races[RaceId] ~= nil then
        if not Races[RaceId].Waiting then
            if not Races[RaceId].Started then
                    ...
                    if Races[RaceId].Waiting then
                        ...
                        for cid,_ in pairs(Races[RaceId].Racers) do
                            ...
                            if RacerData ~= nil then
                                TriggerClientEvent('fortis-lapraces:client:LeaveRace', RacerData.PlayerData.source, Races[RaceId])
                            end
                        end
                        table.remove(AvailableRaces, AvailableKey)
                ...
                TriggerClientEvent('QBCore:Notify', source, 'Deze race is al actief..', 'error')
            end
        else
            TriggerClientEvent('QBCore:Notify', source, 'Deze race is al actief..', 'error')
        end
    else
        TriggerClientEvent('QBCore:Notify', source, 'Deze race bestaat niet', 'error')
    end
end)

RegisterServerEvent('fortis-lapraces:server:UpdateRaceState')
AddEventHandler('fortis-lapraces:server:UpdateRaceState', function(RaceId, Started, Waiting)
    Races[RaceId].Waiting = Waiting
    Races[RaceId].Started = Started
end)

RegisterServerEvent('fortis-lapraces:server:UpdateRacerData')
AddEventHandler('fortis-lapraces:server:UpdateRacerData', function(RaceId, Checkpoint, Lap, Finished)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('fortis-lapraces:client:UpdateRaceRacerData', -1, RaceId, Races[RaceId])
end)

RegisterServerEvent('fortis-lapraces:server:StartRace')
AddEventHandler('fortis-lapraces:server:StartRace', function(RaceId)
    local src = source
    ...
    if RaceId ~= nil then
        if AvailableRaces[AvailableKey].SetupCitizenId == MyPlayer.PlayerData.citizenid then
            ...
            for CitizenId,_ in pairs(Races[RaceId].Racers) do
                local Player = QBCore.Functions.GetPlayerByCitizenId(CitizenId)
                if Player ~= nil then
                    TriggerClientEvent('fortis-lapraces:client:RaceCountdown', Player.PlayerData.source)
                end
            end
            TriggerClientEvent('qb-phone_new:client:UpdateLapraces', -1)
        else
            TriggerClientEvent('QBCore:Notify', src, 'Je bent niet de maker van de race..', 'error')
        end
    else
        TriggerClientEvent('QBCore:Notify', src, 'Je zit niet in de race..', 'error')
    end
end)

RegisterServerEvent('fortis-lapraces:server:SaveRace')
AddEventHandler('fortis-lapraces:server:SaveRace', function(RaceData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Commands.Add("togglesetup", "Zet Racing setup aan/uit", {}, false, function(source, args)
    ...
    if IsWhitelisted(Player.PlayerData.citizenid) then
        ...
        if not Config.RaceSetupAllowed then
            TriggerClientEvent('QBCore:Notify', source, 'Er kunnen niet meer races gemaakt wroden', 'error')
        else
            TriggerClientEvent('QBCore:Notify', source, 'Er kunnen weer races gemaakt worden!', 'success')
        end
    else
        TriggerClientEvent('QBCore:Notify', source, 'Je hebt niet de rechten om dit te doen.', 'error')
    end
end)
    ...
    if IsWhitelisted(Player.PlayerData.citizenid) then
        ...
        if RaceName ~= nil then
            ...
            if Races[RaceId].Started then
                ...
                TriggerClientEvent('QBCore:Notify', source, 'De race is nog niet begonnen.', 'error')
            end
        end
    else
        TriggerClientEvent('QBCore:Notify', source, 'Je hebt niet de rechten om dit te doen', 'error')
...
QBCore.Functions.CreateCallback('fortis-lapraces:server:CanRaceSetup', function(source, cb)

...
RegisterNetEvent('fortis-lapraces:client:StartRaceEditor')
AddEventHandler('fortis-lapraces:client:StartRaceEditor', function(RaceName)
    print(RaceName)
    if not RaceData.InCreator then
        ...
        QBCore.Functions.Notify('Je bent al een race aan het maken.', 'error')
    end 
end)
        ...
        while RaceData.InCreator do
            ...
            if PlayerVeh ~= 0 then
                ...
                if IsControlJustPressed(0, Keys["8"]) or IsDisabledControlJustPressed(0, Keys["8"]) then
                    if CreatorData.Checkpoints ~= nil and next(CreatorData.Checkpoints) ~= nil then
                        ...
                        QBCore.Functions.Notify('Je hebt geen checkpoint geplaatst ..', 'error')
                    end
                end
                ...
                if IsControlJustPressed(0, Keys["K"]) or IsDisabledControlJustPressed(0, Keys["K"]) then
                    if CreatorData.Checkpoints ~= nil and #CreatorData.Checkpoints >= 2 then
                        ...
                        QBCore.Functions.Notify('Minimale hoeveehlheid checkpoints is 10', 'error')
                    end
                end
                ...
                if IsControlJustPressed(0, Keys["]"]) or IsDisabledControlJustPressed(0, Keys["]"]) then
                    if CreatorData.TireDistance + 1.0 ~= 16.0 then
                        ...
                        QBCore.Functions.Notify('Je kan niet hoger dan 15')
                    end
                end
                ...
                if IsControlJustPressed(0, Keys["["]) or IsDisabledControlJustPressed(0, Keys["["]) then
                    if CreatorData.TireDistance - 1.0 ~= 1.0 then
                        ...
                        QBCore.Functions.Notify('Je kan niet lager dan 2')
                    end
                end
            else 
            ...
            if IsControlJustPressed(0, Keys["9"]) or IsDisabledControlJustPressed(0, Keys["9"]) then
                if not CreatorData.ConfirmDelete then
                    ...
                    QBCore.Functions.Notify('Druk op [9] om te bevestigen', 'error', 5000)
                else
                    ...
                    QBCore.Functions.Notify('Race-editor gestopt!', 'error')
...
function SaveRace()
    ...
    TriggerServerEvent('fortis-lapraces:server:SaveRace', CreatorData)

    QBCore.Functions.Notify('Race: '..CreatorData.RaceName..' is opgeslagen!', 'success')
...
function DeleteCheckpoint()
    ...
    if RaceData.ClosestCheckpoint ~= 0 then
        if CreatorData.Checkpoints[RaceData.ClosestCheckpoint] ~= nil then
            ...
            QBCore.Functions.Notify('Je kan niet te snel gaan..', 'error')
        end
    else
        QBCore.Functions.Notify('Je kan niet te snel gaan..', 'error')
    end
...
RegisterNetEvent('fortis-lapraces:client:UpdateRaceRacerData')
AddEventHandler('fortis-lapraces:client:UpdateRaceRacerData', function(RaceId, RaceData)
...
RegisterNetEvent('fortis-lapraces:client:JoinRace')
AddEventHandler('fortis-lapraces:client:JoinRace', function(Data, Laps)
    if not RaceData.InRace then
        RaceData.InRace = true
        SetupRace(Data, Laps)
        TriggerServerEvent('fortis-lapraces:server:UpdateRaceState', CurrentRaceData.RaceId, false, true)
    else
        QBCore.Functions.Notify('Je bent al bezig met een race..', 'error')
    end
end)

RegisterNetEvent('fortis-lapraces:client:LeaveRace')
AddEventHandler('fortis-lapraces:client:LeaveRace', function(data)
    QBCore.Functions.Notify('Je hebt de race verlaten!')
    for k, v in pairs(CurrentRaceData.Checkpoints) do
...
RegisterNetEvent('fortis-lapraces:client:RaceCountdown')
AddEventHandler('fortis-lapraces:client:RaceCountdown', function()
    TriggerServerEvent('fortis-lapraces:server:UpdateRaceState', CurrentRaceData.RaceId, true, false)
    if CurrentRaceData.RaceId ~= nil then
        while Countdown ~= 0 do
            if CurrentRaceData.RaceName ~= nil then
                if Countdown == 10 then
                    QBCore.Functions.Notify('De race gaat beginnen binnen 10 seconden', 'error', 2500)
        ...
        QBCore.Functions.Notify('Je bent niet bezig met een race..', 'error')
    end
end)
                ...
                if CheckpointDistance < MaxDistance then
                    if CurrentRaceData.TotalLaps == 0 then
                        if CurrentRaceData.CurrentCheckpoint + 1 < #CurrentRaceData.Checkpoints then
                            ...
                            SetNewWaypoint(CurrentRaceData.Checkpoints[CurrentRaceData.CurrentCheckpoint + 1].coords.x, CurrentRaceData.Checkpoints[CurrentRaceData.CurrentCheckpoint + 1].coords.y)
                            TriggerServerEvent('fortis-lapraces:server:UpdateRacerData', CurrentRaceData.RaceId, CurrentRaceData.CurrentCheckpoint, CurrentRaceData.Lap, false)
                            DoPilePfx()
                            ...
                            PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                            CurrentRaceData.CurrentCheckpoint = CurrentRaceData.CurrentCheckpoint + 1
                            TriggerServerEvent('fortis-lapraces:server:UpdateRacerData', CurrentRaceData.RaceId, CurrentRaceData.CurrentCheckpoint, CurrentRaceData.Lap, true)
                            FinishRace()
                        end
                    else
                        if CurrentRaceData.CurrentCheckpoint + 1 > #CurrentRaceData.Checkpoints then
...
function FinishRace()
    TriggerServerEvent('fortis-lapraces:server:FinishPlayer', CurrentRaceData, CurrentRaceData.TotalTime, CurrentRaceData.TotalLaps, CurrentRaceData.BestLap)
    if CurrentRaceData.BestLap ~= 0 then
        QBCore.Functions.Notify('Race klaar '..SecondsToClock(CurrentRaceData.TotalTime)..', De beste rond is: '..SecondsToClock(CurrentRaceData.BestLap))
    else
        QBCore.Functions.Notify('Race klaar '..SecondsToClock(CurrentRaceData.TotalTime))
    end
    for k, v in pairs(CurrentRaceData.Checkpoints) do
        if CurrentRaceData.Checkpoints[k].blip ~= nil then
            RemoveBlip(CurrentRaceData.Checkpoints[k].blip)
...
RegisterNetEvent('fortis-lapraces:client:PlayerFinishs')
AddEventHandler('fortis-lapraces:client:PlayerFinishs', function(RaceId, Place, FinisherData)
    if CurrentRaceData.RaceId ~= nil then
        if CurrentRaceData.RaceId == RaceId then
            QBCore.Functions.Notify(FinisherData.PlayerData.charinfo.firstname..' Gefinished op plaats : '..Place, 'error', 3500)
        end
    end
end)
...
RegisterNetEvent('fortis-lapraces:client:WaitingDistanceCheck')
AddEventHandler('fortis-lapraces:client:WaitingDistanceCheck', function()
    Wait(1000)
    Citizen.CreateThread(function()
        while true do
                ...
                if CurrentRaceData.Checkpoints[1] ~= nil then
                    ...
                    if dist > 115.0 then
                        if ToFarCountdown ~= 0 then
                            ...
                            QBCore.Functions.Notify('Ga terug naar de start binnen: '..ToFarCountdown..'s', 'error', 500)
                        else
                            TriggerServerEvent('fortis-lapraces:server:LeaveRace', CurrentRaceData)
                            ToFarCountdown = 10
                            break
                        end
                        Citizen.Wait(1000)

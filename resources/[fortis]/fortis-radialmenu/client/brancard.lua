...
AddEventHandler('hospital:client:TakeBrancard', function()
    ...
    if Vehicle ~= 0 then
        ...
        if Obj ~= nil or Obj ~= 0 then
            ...
            QBCore.Functions.Notify("Er is iets fout gegaan!", 'error')
        end
    else
        QBCore.Functions.Notify("Je staat niet bij een ambulance!", 'error')
    end
end)
            ...
            if Distance < 2.0 then
                if not IsAttached then
                    if not IsLayingOnBed then
                        DrawText3Ds(OffsetCoords.x, OffsetCoords.y, OffsetCoords.z, '~g~E~w~ - Brancard Oppakken / ~g~H~w~ - Vastzetten')
                ...
                if IsAttached and not IsLayingOnBed then
                    if Vehicle ~= 0 then
                        local vehClass = GetVehicleClass(closestVehicle)
                        local vehicleCoords = GetEntityCoords(closestVehicle)
                        local distensie = GetDistanceBetweenCoords(PlayerPos, vehicleCoords)
                        offset = {
                            x = Config.BrancardClasses[vehClass].x,
                            y = Config.BrancardClasses[vehClass].y,
                            z = Config.BrancardClasses[vehClass].z,
                        }
                        if Config.BrancardClasses[vehClass].allowed then
                            if distensie < 5 then
                                DrawText3Ds(vehicleCoords.x, vehicleCoords.y, vehicleCoords.z, '~g~U~w~ - Plaats in voertuig')
                                if IsControlJustPressed(0, Keys["U"]) then
                                    IsAttached = false
                                    AttachEntityToEntity(BrancardObject, closestVehicle, 0, offset.x, offset.y, offset.z + 0.3, 0, 0, 180.0, 1, 1, 1, 1, 1, 1)
                ...
                if not IsLayingOnBed then
                    if not IsAttached then
                        DrawText3Ds(OffsetCoords.x, OffsetCoords.y, OffsetCoords.z + 0.2, '~g~G~w~ - Liggen')
                    ...
                    if not IsAttached then
                        DrawText3Ds(OffsetCoords.x, OffsetCoords.y, OffsetCoords.z + 0.2, '~g~G~w~ - Opstaan')
                        if IsControlJustPressed(0, Keys["G"]) then
                            IsLayingOnBed = false
                            GetOffBrancard()
                        end
...
AddEventHandler('qb-radialmenu:client:Result', function(IsBusy, type)
    ...
    if type == "lay" then
        ...
        if not IsBusy then
            ...
            QBCore.Functions.Notify("Deze brancard wordt al gebruikt!", "error")
            IsAttached = false
        end
    end
end)
...
Citizen.CreateThread(function()
    while true do
        if IsAttached or IsLayingOnBed then
			...
			if not IsEntityPlayingAnim(GetPlayerPed(-1), 'anim@heists@box_carry@', 'idle', false) then
				LoadAnim("anim@heists@box_carry@")
				TaskPlayAnim(PlayerPed, 'anim@heists@box_carry@', 'idle', 8.0, 8.0, -1, 50, 0, false, false, false)
			end

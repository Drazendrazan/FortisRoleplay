...
AddEventHandler('qb-trunk:client:KidnapTrunk', function()
    ...
    if (distance ~= -1 and distance < 2) then
        if isKidnapping then
            ...
            QBCore.Functions.Notify('Je hebt de persoon niet gekidnapt!', 'error')
        end
    end
end)
                    ...
                    if not isBusy then
                        if not isKidnapped then
                            ...
                            if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                                ...
                                QBCore.Functions.Notify('De kofferbak staat op slot!', 'error', 2500)
                            end
                        end
                    else
                        QBCore.Functions.Notify('Er zit al iemand in de kofferbak!', 'error', 2500)
                    end 
                else
                    QBCore.Functions.Notify('Je zit al in de kofferbak!', 'error', 2500)
                end 
            else
                QBCore.Functions.Notify('Je past niet in deze kofferbak!', 'error', 2500)
            end
        end, plate)
    else
        QBCore.Functions.Notify('Je past niet in deze kofferbak!', 'error', 2500)
                    ...
                    if not inTrunk then
                        if not isBusy then
                            if GetVehicleDoorAngleRatio(closestVehicle, 5) > 0 then
                                ...
                                Citizen.Wait(500)
                                SetVehicleDoorShut(closestVehicle, 5, false)
                                QBCore.Functions.Notify('Je zit in de kofferbak!', 'Goodluck', 4000)
                                TrunkCam(true)
                            else
                                QBCore.Functions.Notify('De kofferbak staat op slot!', 'error', 2500)
                            end
        ...
        if inTrunk then
            if not isKidnapped then
                ...
                if DoesEntityExist(vehicle) then
                    DrawText3Ds(drawPos.x, drawPos.y, drawPos.z + 0.75, '~g~[E]~w~ - Uitstappen')
                    ...
                    if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                        DrawText3Ds(drawPos.x, drawPos.y, drawPos.z + 0.5, '~g~[G]~w~ - Kofferbak Sluiten')
                        ...
                        DrawText3Ds(drawPos.x, drawPos.y, drawPos.z + 0.5, '~g~[G]~w~ - Kofferbak Openen')
                        if IsControlJustPressed(0, Keys["G"]) then
                            if not IsVehicleSeatFree(vehicle, -1) then

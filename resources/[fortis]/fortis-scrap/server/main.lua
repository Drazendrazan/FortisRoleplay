...
RegisterNetEvent("fortis-scrap:server:itemGeven")
AddEventHandler("fortis-scrap:server:itemGeven", function(materiaal, key)
    local src = source
...
QBCore.Functions.CreateCallback("fortis-scrap:server:checkScrap", function(source, cb, materiaal)
...
RegisterServerEvent("fortis-scrap:server:klaar")
AddEventHandler("fortis-scrap:server:klaar", function(materiaal, eerste)
    local src = source
    local steam = GetPlayerIdentifiers(src)[1]
    local materiaal = materiaal
    local eerste = eerste

...
AddEventHandler('qb-weed:server:placePlant', function(currentHouse, coords, sort)
    ...
    local plantId = maakPlantID()

    QBCore.Functions.ExecuteSql(true, "INSERT INTO `house_plants` (`building`, `coords`, `gender`, `sort`, `plantid`) VALUES ('"..currentHouse.."', '"..coords.."', '"..gender.."', '"..sort.."', '"..plantId.."')")
    TriggerClientEvent('qb-weed:client:refreshHousePlants', -1, currentHouse)
end)

function maakPlantID()
    local id = nil
    local gevonden = false

    while not gevonden do
        id = math.random(1111111111, 9999999999)
        QBCore.Functions.ExecuteSql(true, "SELECT * FROM `house_plants` WHERE `plantid` = '"..id.."'", function(result)
            if #result == 0 then
                gevonden = true
            end
        end)
...
Citizen.CreateThread(function()
    while true do
        QBCore.Functions.ExecuteSql(false, "SELECT * FROM `house_plants`", function(housePlants)
            for k, v in pairs(housePlants) do
                Wait(50)
                if housePlants[k].health > 40 then
            ...
            if house ~= nil then 
                QBCore.Functions.ExecuteSql(false, "SELECT * FROM `house_plants` WHERE plantid = '"..plantId.."' AND building = '"..house.."'", function(result)
                    if result[1] ~= nil then
                        ...
                        TriggerClientEvent('QBCore:Notify', src, 'Je hebt de plant geoogst!', 'success', 3500)
                        TriggerClientEvent('qb-weed:client:refreshHousePlants', -1, house)
                    else
                        TriggerClientEvent('QBCore:Notify', src, 'Er zitten geen bladeren meer aan de plant!', 'error', 3500)
                    end
                end)
            else
                TriggerClientEvent('QBCore:Notify', src, 'Er is iets mis gegaan!', 'error', 3500)
            end
        else
            TriggerClientEvent('QBCore:Notify', src, 'Je hebt geen zakje!', 'error', 3500)
        end
    else
        TriggerClientEvent('QBCore:Notify', src, 'Je hebt geen zakje!', 'error', 3500)
    end

        ...
        if not serverOpgestart then
            ...
            for k, v in pairs(magazijnen) do
                ...
                for _k, _v in pairs(v.sources) do
                    TriggerClientEvent("fortis-groothandel:client:syncMagazijnHandelData", _v, magazijnen[k].handel_data)
                end
            end
        end
...
-- Speler logt in
RegisterNetEvent("fortis-groothandel:server:playerLoggedIn")
AddEventHandler("fortis-groothandel:server:playerLoggedIn", function()
    local src = source
        ...
        QBCore.Functions.ExecuteSql(true, "SELECT * FROM `groothandels` WHERE citizenid = '"..citizenid.."' OR werknemers LIKE '%"..citizenid.."%';", function(row)
            if #row == 0 then return else
                ...
                if row.citizenid == citizenid then
                    ...
                    TriggerClientEvent("fortis-groothandel:client:syncWerknemers", src, json.encode(fullWerknemersTable))
                    row["werknemer"] = false
                else
                    -- Is medewerker
                    row["werknemer"] = true
                ...
                TriggerClientEvent("fortis-groothandel:client:syncMagazijn", src, row)
            end
        end)
    end
end)
...
RegisterNetEvent("fortis-groothandel:server:koopGroothandel")
AddEventHandler("fortis-groothandel:server:koopGroothandel", function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
            ...
            if #resultaat == 0 then
                ...
                if bank >= Config.groothandelPrijs then
                    ...
                    QBCore.Functions.ExecuteSql(true, "SELECT * FROM `groothandels` WHERE citizenid = '"..citizenid.."'", function(groothandelRow)
                        ...
                        TriggerClientEvent("fortis-groothandel:client:syncMagazijn", src, groothandelRow)
                        local legeWerknemersTable = {}
                        TriggerClientEvent("fortis-groothandel:client:syncWerknemers", src, json.encode(legeWerknemersTable))
...
QBCore.Functions.CreateCallback("fortis-groothandel:server:getMagazijnSpots", function(source, cb, magazijnId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent("fortis-groothandel:server:verlaatMagazijn")
AddEventHandler("fortis-groothandel:server:verlaatMagazijn", function(spotKeyID)
    local src = source
...
RegisterNetEvent("fortis-groothandel:server:vraagHandelData")
AddEventHandler("fortis-groothandel:server:vraagHandelData", function(groothandelID)
    local src = source

    if groothandelID ~= nil and magazijnen[groothandelID] ~= nil and magazijnen[groothandelID].handel_data ~= nil then
        TriggerClientEvent("fortis-groothandel:client:syncMagazijnHandelData", src, magazijnen[groothandelID].handel_data)
    end
end)

-- Speler neemt werknemer aan in magazijn callback :(
QBCore.Functions.CreateCallback("fortis-groothandel:server:werknemerAannemen", function(source, cb, cidSpeler, groothandelID)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
                ...
                QBCore.Functions.ExecuteSql(true, "SELECT * FROM `groothandels` WHERE citizenid = '"..cidSpeler.."' OR werknemers LIKE '%"..cidSpeler.."%'", function(resultaat)
                    if #resultaat == 0 then
                        ...
                        TriggerEvent("qb-phone:server:sendNewMail", {
                            ...
                            button = {
                                enabled = true,
                                buttonEvent = "fortis-groothandel:client:joinGroothandel",
                                buttonData = {
...
RegisterNetEvent("fortis-groothandel:server:werknemerOntslaan")
AddEventHandler("fortis-groothandel:server:werknemerOntslaan", function(citizenid, groothandelID)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
                ...
                if deletedWerknemer then
                    ...
                    for k, v in pairs(magazijnen[magazijn_resultaat.id].sources) do
                        if srcVanOntslagenSpeler.PlayerData.source ~= nil and v == srcVanOntslagenSpeler.PlayerData.source then
                            ...
                            TriggerClientEvent("fortis-groothandel:client:spelerOntslagen", srcVanOntslagenSpeler.PlayerData.source)
                        else
                            -- Stuur regular sync
                            TriggerClientEvent("fortis-groothandel:client:syncMagazijn", v, magazijn_resultaat)
                        end
                    end
                end
            end
...
RegisterNetEvent("fortis-groothandel:server:joinGroothandel")
AddEventHandler("fortis-groothandel:server:joinGroothandel", function(magazijnID)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
                        ...
                        QBCore.Functions.ExecuteSql(true, "SELECT * FROM `groothandels` WHERE citizenid = '"..cidSpeler.."' OR werknemers LIKE '%"..cidSpeler.."%';", function(row)
                            if #row == 0 then return else
                                ...
                                if srcVanEigenaar ~= nil then
                                    ...
                                    TriggerClientEvent("fortis-groothandel:client:syncWerknemers", srcVanEigenaar.PlayerData.source, json.encode(fullWerknemersTable))
                                end
                                ...
                                for k, v in pairs(magazijnen[row.id].sources) do
                                    TriggerClientEvent("fortis-groothandel:client:syncMagazijn", v, row)
                                end
                            end
                        end)
...
RegisterNetEvent("fortis-groothandel:server:koopWerknemerSlot")
AddEventHandler("fortis-groothandel:server:koopWerknemerSlot", function(groothandelID)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
            ...
            if #tmp_groothandel > 0 then
                ...
                if bank >= Config.werknemerSlotPrijs then
                    ...
                    if tmp_groothandel.data.werknemerspots < 5 then
                        ...
                        Wait(100)
                        TriggerClientEvent("fortis-groothandel:client:syncMagazijn", src, tmp_groothandel)
                    else
...
RegisterNetEvent("fortis-groothandel:server:veranderSpecialisaties")
AddEventHandler("fortis-groothandel:server:veranderSpecialisaties", function(groothandelID)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
        ...
        QBCore.Functions.ExecuteSql(true, "SELECT * FROM groothandels WHERE citizenid = '"..citizenidOWNER.."' AND id = "..groothandelID.."", function(tmp_groothandel)
            if #tmp_groothandel > 0 then
                ...
                for k, v in pairs(magazijnen[tmp_groothandel.id].sources) do
                    TriggerClientEvent("fortis-groothandel:client:syncMagazijn", v, tmp_groothandel)
                end
...
RegisterNetEvent("fortis-groothandel:server:balansOpnemen")
AddEventHandler("fortis-groothandel:server:balansOpnemen", function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent("fortis-groothandel:server:koopVoertuig")
AddEventHandler("fortis-groothandel:server:koopVoertuig", function(voertuig)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local bedrag = nil
...
QBCore.Functions.CreateCallback("fortis-groothandel:server:verwijderHandelRij", function(source, cb, rijID, type, groothandelID, aantal)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
        ...
        if type == "inkopen" then
            if magazijnen[groothandelID].handel_data.inkopen[rijID] ~= nil then
                ...
                for k, v in pairs(magazijnen[groothandelID].sources) do
                    TriggerClientEvent("fortis-groothandel:client:syncMagazijnHandelData", v, magazijnen[groothandelID].handel_data)
                end
...
RegisterNetEvent("fortis-groothandel:server:inkoopklaar")
AddEventHandler("fortis-groothandel:server:inkoopklaar", function(groothandelID, rondeData)
    local src = source
...
RegisterNetEvent("fortis-groothandel:server:verkoopklaar")
AddEventHandler("fortis-groothandel:server:verkoopklaar", function(groothandelID, rondeData)
    local src = source

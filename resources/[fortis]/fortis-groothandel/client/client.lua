...
AddEventHandler("QBCore:Client:OnPlayerLoaded", function()
    Wait(5000)
    TriggerServerEvent("fortis-groothandel:server:playerLoggedIn")
end)
...
RegisterNUICallback("koopGroothandel", function()
    TriggerServerEvent("fortis-groothandel:server:koopGroothandel")
end)

-- Groothandel syncen vanaf server side
RegisterNetEvent("fortis-groothandel:client:syncMagazijn")
AddEventHandler("fortis-groothandel:client:syncMagazijn", function(tmp_groothandel_data)
...
RegisterNetEvent("fortis-groothandel:client:syncWerknemers")
AddEventHandler("fortis-groothandel:client:syncWerknemers", function(tmp_werknemer_data)
...
RegisterNetEvent("fortis-groothandel:client:spelerOntslagen")
AddEventHandler("fortis-groothandel:client:spelerOntslagen", function()
...
RegisterNetEvent("fortis-groothandel:client:syncMagazijnHandelData")
AddEventHandler("fortis-groothandel:client:syncMagazijnHandelData", function(tmp_groothandelHandel_data)
...
RegisterNUICallback("werknemerAannemen", function(data, cb)
    if data.bsn ~= nil then
        QBCore.Functions.TriggerCallback("fortis-groothandel:server:werknemerAannemen", function(resultaat, bericht)
...
RegisterNUICallback("werknemerOntslaan", function(data, cb)
    if data.cid ~= nil then
        TriggerServerEvent("fortis-groothandel:server:werknemerOntslaan", data.cid, groothandel.id)
    end
end)

AddEventHandler("fortis-groothandel:client:joinGroothandel", function(magazijnInviteData)
    if magazijnInviteData ~= nil then
        TriggerServerEvent("fortis-groothandel:server:joinGroothandel", magazijnInviteData.magazijnID)
    else
...
RegisterNUICallback("koopWerknemerSlot", function(data, cb)
    if groothandelEigenaar and groothandel ~= nil then
        if groothandel.data.werknemerspots < 5 then
            TriggerServerEvent("fortis-groothandel:server:koopWerknemerSlot", groothandel.id)
        else
...
RegisterNUICallback("balansOpnemen", function(data, cb)
    if groothandelEigenaar and groothandel ~= nil then
        TriggerServerEvent("fortis-groothandel:server:balansOpnemen")
    end
end)

RegisterNUICallback("koopVoertuig", function(data, cb)
    if groothandelEigenaar and groothandel ~= nil then
        TriggerServerEvent("fortis-groothandel:server:koopVoertuig", data.voertuig)
    end
end)

RegisterNUICallback("veranderSpecialisaties", function (data, cb)
    if groothandelEigenaar and groothandel ~= nil then
        TriggerServerEvent("fortis-groothandel:server:veranderSpecialisaties", groothandel.id)
    end
end)
            ...
            if GetDistanceBetweenCoords(pedCoords, groothandel.data.coords["deur"].x, groothandel.data.coords["deur"].y, groothandel.data.coords["deur"].z, true) < 15 then
                ...
                if GetDistanceBetweenCoords(pedCoords, groothandel.data.coords["deur"].x, groothandel.data.coords["deur"].y, groothandel.data.coords["deur"].z, true) < 1.5 then
                    ...
                    if IsControlJustPressed(0, 38) and not enteringMagazijn then
                        ...
                        QBCore.Functions.TriggerCallback("fortis-groothandel:server:getMagazijnSpots", function(_spotKeyID)
                            if _spotKeyID ~= nil then
                                spotKeyID = _spotKeyID
                                -- Begin
                                if groothandelHandelData == nil then
                                    TriggerServerEvent("fortis-groothandel:server:vraagHandelData", groothandel.id)
                                end
        ...
        if groothandel ~= nil and inMagazijnData ~= nil then
            ...
            if GetDistanceBetweenCoords(pedCoords, exitCoords.x, exitCoords.y, exitCoords.z, true) < 10 then
                ...
                if GetDistanceBetweenCoords(pedCoords, exitCoords.x, exitCoords.y, exitCoords.z, true) < 1.5 then
                    ...
                    if IsControlJustPressed(0, 38) and not enteringMagazijn then
                        TriggerServerEvent("fortis-groothandel:server:verlaatMagazijn", spotKeyID)
                        enteringMagazijn = true
...
RegisterNUICallback("startRonde", function(data, cb)
    ...
    if not bezigMetRonde then
        ...
        rondeData.transport == 3 and groothandel.data.voertuigen.vrachtwagen then
            QBCore.Functions.TriggerCallback("fortis-groothandel:server:verwijderHandelRij", function(resultaat)
                if resultaat.correct == true then
                    QBCore.Functions.Notify("Je bent gestart met je ronde. Stap in het voertuig!", "success")
                    TriggerEvent("fortis-groothandel:client:startRonde", rondeData, rondeType)
                else
...
AddEventHandler("fortis-groothandel:client:startRonde", function(rondeData, rondeType)
    bezigMetRonde = true
    ...
    while true do
        ...
        if GetDistanceBetweenCoords(pos, voertuigCoords.x, voertuigCoords.y, voertuigCoords.z) < 4 then
            if IsControlJustPressed(0, 49) then
                ...
                TriggerEvent("fortis-groothandel:client:bezigMetRonde", rondeData, rondeType)
                return
            end
        end
    end
...
AddEventHandler("fortis-groothandel:client:bezigMetRonde", function(rondeData, rondeType)
    local voertuig = nil
                        ...
                        if GetVehiclePedIsIn(GetPlayerPed(-1)) == rondeVoertuig then
                            if klaar then
                                if rondeType == "inkopen" then
                                    ...
                                    TriggerServerEvent("fortis-groothandel:server:inkoopklaar", groothandel.id, rondeData)
                                    ...
                                    TriggerServerEvent("fortis-groothandel:server:verkoopklaar", groothandel.id, rondeData)

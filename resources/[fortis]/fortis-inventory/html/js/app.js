var InventoryOption = "28, 202, 155";
...
function FormatAttachmentInfo(data) {
    $.post("http://fortis-inventory/GetWeaponData", JSON.stringify({
        weapon: data.name,
        ...
        if (data.AttachmentData !== null && data.AttachmentData !== undefined) {
            if (data.AttachmentData.length > 0) {
                ...
                $(".weapon-attachments-title").html('<span style="font-weight: bold; letter-spacing: .1vh;">Er zitten geen extra onderdelen op dit wapen.</span>');
            }
        } else {
            $(".weapon-attachments-title").html('<span style="font-weight: bold; letter-spacing: .1vh;">Er zitten geen extra onderdelen op dit wapen.</span>');
        }
...
function handleAttachmentDrag() {
    ...
    $(".weapon-attachments-remove").droppable({
        ...
        drop: function(event, ui) {
            $.post('http://fortis-inventory/RemoveAttachment', JSON.stringify({
...
$(document).on('click', '#weapon-attachments', function(e){
    ...
    if (!Inventory.IsWeaponBlocked(ClickedItemData.name)) {
        ...
        $.post('http://fortis-inventory/Notify', JSON.stringify({
            message: "Attachments zijn niet beschikbaar op dit wapen.",
            type: "error"
        }))
    }
});
...
function FormatItemInfo(itemData) {
    if (itemData != null && itemData.info != "") {
        if (itemData.name == "id_card") {
            ...
            $(".item-info-title").html('<p>'+itemData.label+'</p>')
            $(".item-info-description").html('<p><strong>Voornaam: </strong><span>' + itemData.info.firstname + '</span></p><p><strong>Achternaam: </strong><span>' + itemData.info.lastname + '</span></p><p><strong>Birth Date: </strong><span>' + itemData.info.birthdate + '</span></p><p><strong>Licenses: </strong><span>' + itemData.info.type + '</span></p>');
        } else if (itemData.name == "lawyerpass") {
            $(".item-info-title").html('<p>'+itemData.label+'</p>')
            $(".item-info-description").html('<p><strong>Pass-ID: </strong><span>' + itemData.info.id + '</span></p><p><strong>Voornaam: </strong><span>' + itemData.info.firstname + '</span></p><p><strong>Last Name: </strong><span>' + itemData.info.lastname + '</span></p><p><strong>CSN: </strong><span>' + itemData.info.citizenid + '</span></p>');
            ...
            if (itemData.info.attachments != null) {
                ...
                $(".item-info-description").html('<p><strong>Serienummer: </strong><span>' + itemData.info.serie + '</span></p><p><strong>Ammo: </strong><span>' + itemData.info.ammo + '</span></p><p><strong>Attachments: </strong><span>' + attachmentString + '</span></p>');
            } else{
                $(".item-info-description").html('<p><strong>Serienummer: </strong><span>' + itemData.info.serie + '</span></p><p><strong>Ammo: </strong><span>' + itemData.info.ammo + '</span></p><p>' + itemData.description + '</p>');
            }
        } else if (itemData.name == "filled_evidence_bag") {
            $(".item-info-title").html('<p>'+itemData.label+'</p>')
            if (itemData.info.type == "casing") {
                $(".item-info-description").html('<p><strong>Bewijsmateriaal: </strong><span>' + itemData.info.label + '</span></p><p><strong>Kogelsoort: </strong><span>' + itemData.info.ammotype + '</span></p><p><strong>Caliber: </strong><span>' + itemData.info.ammolabel + '</span></p><p><strong>Serienummer: </strong><span>' + itemData.info.serie + '</span></p><p><strong>Plaats delict: </strong><span>' + itemData.info.street + '</span></p><br /><p>' + itemData.description + '</p>');
            }else if (itemData.info.type == "blood") {
                $(".item-info-description").html('<p><strong>Bewijsmateriaal: </strong><span>' + itemData.info.label + '</span></p><p><strong>Bloedgroep: </strong><span>' + itemData.info.bloodtype + '</span></p><p><strong>DNA Code: </strong><span>' + itemData.info.dnalabel + '</span></p><p><strong>Plaats delict: </strong><span>' + itemData.info.street + '</span></p><br /><p>' + itemData.description + '</p>');
            }else if (itemData.info.type == "fingerprint") {
                $(".item-info-description").html('<p><strong>Bewijsmateriaal: </strong><span>' + itemData.info.label + '</span></p><p><strong>Vingerafdruk: </strong><span>' + itemData.info.fingerprint + '</span></p><p><strong>Plaats delict: </strong><span>' + itemData.info.street + '</span></p><br /><p>' + itemData.description + '</p>');
            }else if (itemData.info.type == "dna") {
                $(".item-info-description").html('<p><strong>Bewijsmateriaal: </strong><span>' + itemData.info.label + '</span></p><p><strong>DNA Code: </strong><span>' + itemData.info.dnalabel + '</span></p><br /><p>' + itemData.description + '</p>');
            }
            ...
            $(".item-info-title").html('<p>'+itemData.label+'</p>')
            $(".item-info-description").html('<p><strong>Hoeveelheid: </strong><span>€' + itemData.info.cash + '</span></p>');
        ...
        } else if (itemData.name == "bratva_vip") {
            $(".item-info-title").html('<p>'+itemData.label+'</p>')
            $(".item-info-description").html('<p><strong>Voornaam: </strong><span>' + itemData.info.firstname + '</span></p><p><strong>Achternaam: </strong><span>' + itemData.info.lastname + '</span></p><p><strong>Geboorte datum: </strong><span>' + itemData.info.birthdate + '</span></p>');
        } else {
            $(".item-info-title").html('<p>'+itemData.label+'</p>')
...
function handleDragDrop() {
    ...
    $("#item-use").droppable({
        ...
        drop: function(event, ui) {
            ...
            if(fromData.useable) {
                ...
                $.post("http://fortis-inventory/UseItem", JSON.stringify({
    ...
    $("#item-geef").droppable({
        ...
        drop: function(event, ui) {
            ...
            fromData = ui.draggable.data("item");
            fromInventory = ui.draggable.parent().attr("data-inventory");
            aantalInput = $("#item-amount").val();
            Inventory.Close();
            $.post("http://fortis-inventory/GeefItem", JSON.stringify({
                inventory: fromInventory,
                item: fromData,
                aantalInput: aantalInput,
            }));
        }
    });
    ...
    $("#item-drop").droppable({
        ...
        drop: function(event, ui) {
            ...
            $.post("http://fortis-inventory/DropItem", JSON.stringify({
                inventory: fromInventory,
                item: fromData,
...
function updateweights($fromSlot, $toSlot, $fromInv, $toInv, $toAmount) {
    ...
    $("#player-inv-weight").removeClass("inv-waarschuwing");
    $("#player-inv-weight").removeClass("inv-geenwaarschuwing");

    if((totalWeight / 1000).toFixed(2) >= 100) {
        $("#player-inv-weight").addClass("inv-waarschuwing");
    } else {
        $("#player-inv-weight").addClass("inv-geenwaarschuwing");
    }
...
$(document).on('click', '.CombineItem', function(e){
    ...
    if (combineslotData.toData.combinable.anim != null) {
        $.post('http://fortis-inventory/combineWithAnim', JSON.stringify({
        ...
        $.post('http://fortis-inventory/combineItem', JSON.stringify({
...
function optionSwitch($fromSlot, $toSlot, $fromInv, $toInv, $toAmount, toData, fromData) {
    console.log("optionSwitch getriggerd #kaasje");
    ...
    $.post("http://fortis-inventory/SetInventoryData", JSON.stringify({
...
function swap($fromSlot, $toSlot, $fromInv, $toInv, $toAmount) {
    ...
    if (fromData !== undefined && fromData.amount >= $toAmount) {   
        ...
        if((toData != undefined || toData != null) && toData.name == fromData.name && !fromData.unique) {
            ...
            $.post("http://fortis-inventory/PlayDropSound", JSON.stringify({}));
            $.post("http://fortis-inventory/SetInventoryData", JSON.stringify({
                fromInventory: $fromInv.attr("data-inventory"),
                toInventory: $toInv.attr("data-inventory"),
                fromSlot: $fromSlot,
            ...
            // Fix voor dikke gare problemen
            $("#tandwiel").fadeIn();
            $("#tandwiel").delay(800).fadeOut();
        } else {
            if (fromData.amount == $toAmount) {
                if (toData != undefined && toData.combinable != null && isItemAllowed(fromData.name, toData.combinable.accept)) {

                    $.post('http://fortis-inventory/getCombineItem', JSON.stringify({item: toData.combinable.reward}), function(item){
                ...
                if (toData != undefined) {
                    // Hier hier hier hier hier hier hier hier hier hier hie rhier  hier hier
                    InventoryError($fromInv, $fromSlot);
                    return;
                    ...
                    // Het blocken van stashen van wapens in een kofferbak/handschoenkastje
                    var kankerhoertje = $toInv.attr("data-inventory").split("-")[0];
                    var kankerStashes = ["trunk", "handschoenkastje"];
                    if (fromData.name.includes("weapon_") && kankerStashes.includes(kankerhoertje)) {
                        InventoryError($fromInv, $fromSlot);
                        return;
                    }
...
function InventoryError($elinv, $elslot) {
    $elinv.find("[data-slot=" + $elslot + "]").css("background", "rgba(28, 202, 155, 0.5)").css("transition", "background 500ms");
    setTimeout(function() {
        $elinv.find("[data-slot=" + $elslot + "]").css("background", "rgba(255, 255, 255, 0.03)");
    }, 500)
    $.post("http://fortis-inventory/PlayDropFail", JSON.stringify({}));
}
...
(() => {
    ...
    Inventory.Open = function(data) {
        ...
        document.getElementById("contantGeldTXT").innerHTML = data.cashGeld;
        $("#xpWrapper").hide();
        ...
        if (data.other != null) 
        {
            ...
            if (name != null && (name.split("-")[0] == "itemshop" || name == "crafting")) {
                if (name.split("-")[0] == "crafting") {
                    ...
                    $("#xpWrapper").show();
                    // Fixen van progressbar
                    document.getElementById("craftingProgress").value = data.other.currentPoints;
                    document.getElementById("craftingProgress").max = data.other.neededPoints;
                    
                    $("#xp-current-points").html(data.other.currentPoints);
                    $("#xp-needed-points").html(data.other.neededPoints);
                    if (data.other.currentPoints == 5000 && data.other.neededPoints == 5000) {
                        $("#xp-current-level").html(data.other.currentLevel + " (MAX LEVEL)");
                    } else {
                        $("#xp-current-level").html(data.other.currentLevel);
                    }
                }
            } else {
    ...
    Inventory.Close = function() {
        ...
        $.post("http://fortis-inventory/CloseInventory", JSON.stringify({}));
    };
...
$(document).on('click', '#rob-money', function(e){
    ...
    $.post('http://fortis-inventory/RobMoney', JSON.stringify({
        TargetId: TargetId
    }));
    $("#rob-money").remove();
});
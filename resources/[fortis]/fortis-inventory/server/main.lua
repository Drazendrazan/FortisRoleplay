...
AddEventHandler('inventory:server:OpenInventory', function(name, id, other)
	...
	if name ~= nil and id ~= nil then
		...
		elseif name == "glovebox" then
			...
			secondInv.label = "Handschoenkastje-"..id
			secondInv.maxweight = 10000
			secondInv.inventory = {}
			secondInv.slots = 5
			if Gloveboxes[id] ~= nil and Gloveboxes[id].isOpen then
				secondInv.name = "none-inv"
				secondInv.label = "Handschoenkastje-None"
				secondInv.maxweight = 10000
				secondInv.inventory = {}
				secondInv.slots = 0
			else
		...
		elseif name == "crafting" then
			...
			secondInv.inventory = other.items
			secondInv.slots = #other.items
			-- Berekenen van crafting XP
			local punten = Player.PlayerData.metadata.craftingrep
			local levels = {
				[0] = 0,
				[1] = 120,
				[2] = 150,
			...
			if punten >= 1500 then
				local tmp_keyNext = currentLevel + 1
				secondInv.currentLevel = currentLevel
				secondInv.currentPoints = 5000
				secondInv.neededPoints = 5000
			else
				local tmp_keyNext = currentLevel + 1
				secondInv.currentLevel = currentLevel
				secondInv.currentPoints = punten
				secondInv.neededPoints = levels[tmp_keyNext]
			end
		...
		elseif name == "otherplayer" then
			...
			if OtherPlayer ~= nil then
				...
				if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty or QBCore.Functions.HasPermission(src, "admin") or QBCore.Functions.HasPermission(src, "god") then
			...
			if Drops[id] ~= nil and not Drops[id].isOpen then
				secondInv.name = id
				secondInv.label = "Grond-"..tostring(id)
				...
				secondInv.name = "none-inv"
				secondInv.label = "Grond-Onbekend"
				secondInv.maxweight = 100000
				secondInv.inventory = {}
				secondInv.slots = 0
			end
...
AddEventHandler('inventory:server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount)
	...
	if fromInventory == "player" or fromInventory == "hotbar" then
		...
		if fromItemData ~= nil and fromItemData.amount >= fromAmount then
			...
			elseif QBCore.Shared.SplitStr(toInventory, "-")[1] == "otherplayer" then
				...
				if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
						...
						TriggerEvent("qb-log:server:CreateLog", "robbing", "Item Omgewisseld", "orange", "**".. GetPlayerName(src) .. "** (BSN: *"..Player.PlayerData.citizenid.."* | *"..src.."*) heeft een item omgewisseld, naam: **"..itemInfo["name"].."**, hoeveelheid: **" .. toAmount .. "** omgewisseld met: **" .. fromItemData.name .. "**, hoeveelheid: **" .. fromAmount.. "** met speler: **".. GetPlayerName(OtherPlayer.PlayerData.source) .. "** (BSN: *"..OtherPlayer.PlayerData.citizenid.."* | ID: *"..OtherPlayer.PlayerData.source.."*)")
					end
				else
					...
					TriggerEvent("qb-log:server:CreateLog", "robbing", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (BSN: *"..Player.PlayerData.citizenid.."* | *"..src.."*) liet een item vallen, naam: **"..itemInfo["name"].."**, hoeveelheid: **" .. fromAmount .. "** to player: **".. GetPlayerName(OtherPlayer.PlayerData.source) .. "** (citizenid: *"..OtherPlayer.PlayerData.citizenid.."* | id: *"..OtherPlayer.PlayerData.source.."*)")
				end
				local itemInfo = QBCore.Shared.Items[fromItemData.name:lower()]
			...
			elseif QBCore.Shared.SplitStr(toInventory, "-")[1] == "trunk" then
				...
				if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
						...
						TriggerEvent("qb-log:server:CreateLog", "trunk", "Item Omgewisseld", "orange", "**".. GetPlayerName(src) .. "** (BSN: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) heeft een item omgewisseld, naam: **"..itemInfo["name"].."**, hoeveelheid: **" .. toAmount .. "** omgewisseld met: **" .. fromItemData.name .. "**, hoeveelheid: **" .. fromAmount .. "** - Kentekenplaat: *" .. plate .. "*")
					end
				else
					local itemInfo = QBCore.Shared.Items[fromItemData.name:lower()]
					...
					TriggerEvent("qb-log:server:CreateLog", "trunk", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (BSN: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) liet een item vallen, naam: **"..itemInfo["name"].."**, hoeveelheid: **" .. fromAmount .. "** - Kentekenplaat: *" .. plate .. "*")
				end
				local itemInfo = QBCore.Shared.Items[fromItemData.name:lower()]
			...
			elseif QBCore.Shared.SplitStr(toInventory, "-")[1] == "glovebox" then
				...
				if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
						...
						TriggerEvent("qb-log:server:CreateLog", "glovebox", "Item Omgewisseld", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - plate: *" .. plate .. "*")
					end
				else
					local itemInfo = QBCore.Shared.Items[fromItemData.name:lower()]
			...
			elseif QBCore.Shared.SplitStr(toInventory, "-")[1] == "traphouse" then
				...
				local toItemData = exports['fortis-traphouses']:GetInventoryData(traphouseId, toSlot)
				local IsItemValid = exports['fortis-traphouses']:CanItemBeSaled(fromItemData.name:lower())
				if IsItemValid then
					Player.Functions.RemoveItem(fromItemData.name, fromAmount, fromSlot)
					TriggerClientEvent("inventory:client:CheckWeapon", src, fromItemData.name)
					if toItemData ~= nil then
						...
						local toAmount = tonumber(toAmount) ~= nil and tonumber(toAmount) or toItemData.amount
						if toItemData.name ~= fromItemData.name then
							exports['fortis-traphouses']:RemoveHouseItem(traphouseId, fromSlot, itemInfo["name"], toAmount)
							Player.Functions.AddItem(toItemData.name, toAmount, fromSlot, toItemData.info)
					...
					local itemInfo = QBCore.Shared.Items[fromItemData.name:lower()]
					exports['fortis-traphouses']:AddHouseItem(traphouseId, toSlot, itemInfo["name"], fromAmount, fromItemData.info, src)
				else
				...
				if toInventory == nil or toInventory == 0 then
					...
					if toItemData ~= nil then
						...
						if toItemData.name ~= fromItemData.name then
							...
							TriggerEvent("qb-log:server:CreateLog", "drop", "Item Omgewisseld", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - dropid: *" .. toInventory .. "*")
						end
					else
						local itemInfo = QBCore.Shared.Items[fromItemData.name:lower()]
	...
	elseif QBCore.Shared.SplitStr(fromInventory, "-")[1] == "otherplayer" then
		...
		if fromItemData ~= nil and fromItemData.amount >= fromAmount then
			local itemInfo = QBCore.Shared.Items[fromItemData.name:lower()]
			if itemInfo["name"] == "radio" then
				TriggerClientEvent('qb-radio:onRadioDrop', playerId)
			end
			if toInventory == "player" or toInventory == "hotbar" then
				local toItemData = Player.Functions.GetItemBySlot(toSlot)
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
						...
						Player.Functions.RemoveItem(toItemData.name, toAmount, toSlot)
						AddToStash(stashId, fromSlot, toSlot, itemInfo["name"], toAmount, toItemData.info) -- added
	...
	elseif QBCore.Shared.SplitStr(fromInventory, "-")[1] == "traphouse" then
		...
		if fromItemData ~= nil and fromItemData.amount >= fromAmount then
			...
			if toInventory == "player" or toInventory == "hotbar" then
				local toItemData = Player.Functions.GetItemBySlot(toSlot)
				exports['fortis-traphouses']:RemoveHouseItem(traphouseId, fromSlot, itemInfo["name"], fromAmount)
				if toItemData ~= nil then
					local itemInfo = QBCore.Shared.Items[toItemData.name:lower()]
					local toAmount = tonumber(toAmount) ~= nil and tonumber(toAmount) or toItemData.amount
					if toItemData.name ~= fromItemData.name then
						Player.Functions.RemoveItem(toItemData.name, toAmount, toSlot)
						exports['fortis--traphouses']:AddHouseItem(traphouseId, fromSlot, itemInfo["name"], toAmount, toItemData.info, src)
	...
	elseif QBCore.Shared.SplitStr(fromInventory, "-")[1] == "itemshop" then
		...
		if QBCore.Shared.SplitStr(shopType, "_")[1] == "Dealer" then
			if QBCore.Shared.SplitStr(itemData.name, "_")[1] == "weapon" then
				...
				if Player.Functions.RemoveMoney("cash", price, "dealer-item-bought") then
					...
					TriggerClientEvent('QBCore:Notify', src, itemInfo["label"] .. " succesvol gekocht!", "success")
		...
		elseif QBCore.Shared.SplitStr(shopType, "_")[1] == "Itemshop" then
			if Player.Functions.RemoveMoney("cash", price, "itemshop-bought-item") then
				...
				TriggerEvent("qb-log:server:CreateLog", "shops", "Winkelsysteem", "green", "**"..GetPlayerName(src) .. "** kocht " .. itemInfo["label"] .. " voor €"..price)
	...
	elseif fromInventory == "crafting" then
		...
		if hasCraftItems(src, itemData.costs, fromAmount) then
			...
			TriggerClientEvent('QBCore:Notify', src, "Je hebt niet de juiste spullen...", "error")
		end
...
function IsVehicleOwned(plate)
	...
	-- Kanker mongole check maar ewa ja soms moet die shit, checken of het een vliegtuig is of heli misschien, idk
	if not val then
		QBCore.Functions.ExecuteSql(true, "SELECT * FROM `player_planes` WHERE `plate` = '"..plate.."'", function(result)
			if (result[1] ~= nil) then
				val = true
			else
				val = false
	...
	if not val then
		QBCore.Functions.ExecuteSql(true, "SELECT * FROM `player_boats` WHERE `plate` = '"..plate.."'", function(result)
			if (result[1] ~= nil) then
				val = true
			else
				val = false
...
QBCore.Commands.Add("resetinv", "Reset een inventory (in het geval van -none)", {{name="type", help="stash/trunk/glovebox"},{name="id/plate", help="ID of Kentekenplaat"}}, true, function(source, args)
	...
	if invType ~= nil and invId ~= nil then 
		...
		elseif invType == "stash" then
			...
			TriggerClientEvent('QBCore:Notify', source,  "Geen inventaris gevonden!", "error")
		end
	else
		TriggerClientEvent('QBCore:Notify', source,  "Argumenten niet allemaal ingevuld.", "error")
	end
end, "admin")

QBCore.Commands.Add("giveitem", "Geeft een item aan een speler.", {{name="id", help="ID"},{name="item", help="Naam van het item, geen label."}, {name="amount", help="Hoeveelheid"}}, true, function(source, args)
		...
		if amount > 0 then
			if itemData ~= nil then
				...
				elseif itemData["name"] == "printerdocument" then
					info.url = "https://www.youtube.com/watch?v=anZeANvMKtI"
				end

				if Player.Functions.AddItem(itemData["name"], amount, false, info) then
					TriggerClientEvent('QBCore:Notify', source, "Je hebt " ..GetPlayerName(tonumber(args[1])).." " .. itemData["label"] .. " ("..amount.. ") gegeven!", "success")
				else
					TriggerClientEvent('QBCore:Notify', source,  "Kan item niet geven!", "error")
				end
			else
				TriggerClientEvent('chatMessage', source, "SYSTEEM", "error", "Item niet gevonden!")
			end
		else
			TriggerClientEvent('chatMessage', source, "SYSTEEM", "error", "Amount moet hoger dan 0 zijn!")
		end
	else
...
QBCore.Functions.CreateUseableItem("werkpas", function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
	local voornaam = Player.PlayerData.charinfo.firstname
	local achternaam = Player.PlayerData.charinfo.lastname
	local baan = Player.PlayerData.job.label
	if Player.Functions.GetItemBySlot(item.slot) ~= nil then
        TriggerClientEvent("inventory:client:ShowWerkpas", -1, source, voornaam, achternaam, baan, item.info)
    end
end)

QBCore.Functions.CreateUseableItem("vliegbrevet", function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
	local voornaam = Player.PlayerData.charinfo.firstname
	local achternaam = Player.PlayerData.charinfo.lastname
	if Player.Functions.GetItemBySlot(item.slot) ~= nil then
        TriggerClientEvent("inventory:client:ShowVliegbevret", -1, source, voornaam, achternaam, item.info)
    end
end)
...
QBCore.Functions.CreateUseableItem("tablet", function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
	if Player.Functions.GetItemBySlot(item.slot) ~= nil then
        TriggerClientEvent("qb-blackmarket:client:toggleDatabank", source)
    end
end)
...
RegisterServerEvent("fortis-inventory:server:geefItem")
AddEventHandler("fortis-inventory:server:geefItem", function(item, aantal, naarSpeler)
	local src = source
	local item = item
	local aantal = aantal
	local vanSpeler = QBCore.Functions.GetPlayer(src)
	local naarSpelerPlayer = QBCore.Functions.GetPlayer(naarSpeler)

	if vanSpeler.Functions.GetItemByName(item) ~= nil and vanSpeler.Functions.GetItemByName(item).amount >= aantal then
		local itemInfo = vanSpeler.Functions.GetItemByName(item)["info"]
		local weightInfo = QBCore.Shared.Items[item]
		if QBCore.Config.Player.MaxWeight - QBCore.Player.GetTotalWeight(naarSpelerPlayer.PlayerData.items) > weightInfo["weight"] * aantal then
			vanSpeler.Functions.RemoveItem(item, aantal) 
			naarSpelerPlayer.Functions.AddItem(item, aantal, nil, itemInfo)
			TriggerClientEvent("QBCore:Notify", naarSpeler, "Je kreeg "..aantal.."x "..item.." van de persoon naast je.", "success")
		else
			TriggerClientEvent("QBCore:Notify", naarSpeler, "Je hebt te weinig ruimte om de items te ontvangen!", "error")
			TriggerClientEvent("QBCore:Notify", src, "De persoon aan wie je de items wilt geven heeft te weinig ruimte!", "error")
		end	
	else
		TriggerClientEvent("QBCore:Notify", src, "Je probeerde een item te geven wat je niet hebt!", "error")
		-- local geefSteam = GetPlayerName(source)
		-- local steamid = vanSpeler["PlayerData"]["steam"]
		-- local url = ""
		-- local fotoid = geefSteam.."-hacker-" .. math.random(0000000, 9999999)
    	...
    	-- function(error, data)
    	--     if not error then
    	--         url = "http://51.89.91.147/cdn/"..fotoid..".jpg"
...
-- Contant geld check
QBCore.Functions.CreateCallback("fortis-inventory:server:vraagContantGeld", function(source, cb)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	local contant = Player.PlayerData.money["cash"]
	cb(contant)
end)


-- test dingejte
QBCore.Functions.CreateCallback("fortis-inventory:server:combineerTestCheckDing", function(source, cb, item1, item2)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)

	local terug = false
	if Player.Functions.GetItemByName(item1) ~= nil and Player.Functions.GetItemByName(item2) then
		terug = true
	end
...
RegisterNetEvent("fortis-inventory:server:verwijderKnuppelItem")
AddEventHandler("fortis-inventory:server:verwijderKnuppelItem", function()
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)

	if Player ~= nil then
		if Player.Functions.GetItemByName("weapon_bat") ~= nil then
			Player.Functions.RemoveItem("weapon_bat", 1)
		end
	end
end)
-- Dikke ban hatsa flatsa [combine item reward abuse dev tools]
RegisterNetEvent("fortis-inventory:server:banCombineHacker")
AddEventHandler("fortis-inventory:server:banCombineHacker", function()
    local src = source
    local reason = "Hacken met script tags [Fortis-Inventory Combine Item]"

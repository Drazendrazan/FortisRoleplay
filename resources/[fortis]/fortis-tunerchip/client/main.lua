...
function setVehData(veh,data)
    ...
    if ban then
        TriggerServerEvent("fortis-tunerchip:server:foei")
    else
...
RegisterNUICallback('save', function(data)
    QBCore.Functions.TriggerCallback('fortis-tunerchip:server:HasChip', function(HasChip)
        if HasChip then
            ...
            TriggerServerEvent('fortis-tunerchip:server:TuneStatus', GetVehicleNumberPlateText(veh), true)
        end
    end)
end) 

RegisterNetEvent('fortis-tunerchip:client:TuneStatus')
AddEventHandler('fortis-tunerchip:client:TuneStatus', function()
    local ped = PlayerPedId()
    ...
    if vehModel ~= 0 then
        QBCore.Functions.TriggerCallback('fortis-tunerchip:server:GetStatus', function(status)
            if status then
...
RegisterNUICallback('checkItem', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback("fortis-tunerchip:server:HasChip", function(result)
        if result then
            retval = true 
        end
        cb(retval)
...
RegisterNetEvent('fortis-tunerchip:client:openChip')
AddEventHandler('fortis-tunerchip:client:openChip', function()
    local ped = PlayerPedId()

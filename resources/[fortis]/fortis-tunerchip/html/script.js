...
$(document).on('click', "#save-neon", function(){
    if (RainbowNeon) {
        $.post('https://fortis-tunerchip/saveNeon', JSON.stringify({
...
QBTuneropenen = function() {
    $.post('https://fortis-tunerchip/checkItem', JSON.stringify({item: "tunerlaptop"}), function(hasItem){
        if (hasItem) {
...
QBTunerSluiten = function() {
    ...
    $.post('https://fortis-tunerchip/exit');
}
...
QBCore.Functions.CreateCallback("fortis-bmshop:server:checkDealer", function(source, cb)
    cb(locatie)
end)

RegisterServerEvent("fortis-bmshop:server:koopAttachment")
AddEventHandler("fortis-bmshop:server:koopAttachment", function(id)
    local src = source
    ...
    if viesgeld ~= nil then
        if viesgeld.amount < attachment.prijs then
            ...
            TriggerClientEvent("fortis-bmshop:client:emote", src)
        end
    else
        TriggerClientEvent("QBCore:Notify", source, "Je hebt niet genoeg vies geld bij je.", "error")
    end

...
local products = {
	[1] = {
        ...
        amount = 100,
        info = {
			description = "Verkregen door ANWB, mag niet gebruikt worden voor crafting!"
		},
        type = "item",
        slot = 1,
    },
...
RegisterNetEvent('fortis-vehicletuning:client:SendEmergencyMessage')
AddEventHandler('fortis-vehicletuning:client:SendEmergencyMessage', function(message)
    local coords = GetEntityCoords(GetPlayerPed(-1))
    TriggerServerEvent("fortis-vehicletuning:server:SendEmergencyMessage", coords, message)
    TriggerEvent("police:client:CallAnim")
end)

RegisterNetEvent('fortis-vehicletuning:server:SendEmergencyMessageCheck')
AddEventHandler('fortis-vehicletuning:server:SendEmergencyMessageCheck', function(MainPlayer, message, coords)
    local PlayerData = QBCore.Functions.GetPlayerData()
    if PlayerData ~= nil then
        if ((PlayerData.job.name == "mechanic")) then
            if PlayerData.job.onduty then
                TriggerEvent('chatMessage', "ANWB Melding - " .. MainPlayer.PlayerData.charinfo.phone, "warning", message)
                ...
                AddTextComponentString("ANWB Melding")
...
RegisterNetEvent('fortis-vehicletuning:client:impoundveh')
AddEventHandler('fortis-vehicletuning:client:impoundveh', function()
	QBCore.Functions.GetPlayerData(function(PlayerData)
		PlayerJob = PlayerData.job
        if PlayerData.job.name == "mechanic" then
            if PlayerData.job.onduty then
			    local cloVeh = QBCore.Functions.GetClosestVehicle()
			    if cloVeh ~= nil and cloVeh ~= 0 then
			    	QBCore.Functions.Progressbar("repair_part", "Voertuig in beslag nemen..", math.random(5000, 10000), false, true, {
			    		disableMovement = true,
			    		disableCarMovement = true,
			    		disableMouse = false,
			    		disableCombat = true,
			    	}, {}, {}, {}, function()
			    		local plate = GetVehicleNumberPlateText(cloVeh)
			    		QBCore.Functions.Notify("In beslag genomen: " .. plate, "success")
			    		TriggerServerEvent("police:server:Impound", plate, false, 150)
			    		QBCore.Functions.DeleteVehicle(cloVeh)
			    	end, function()
			    		QBCore.Functions.Notify("In beslag nemen geannuleerd..", "error")
			    	end)
			    else
			    	QBCore.Functions.Notify('Er is geen voertuig in de buurt!', 'error')
                end
            end
		end
	end)
...
Citizen.CreateThread(function()
    ...
    BeginTextCommandSetBlipName("STRING")
    AddTextComponentSubstringPlayerName("ANWB Kantoor")
    EndTextCommandSetBlipName(Blip)
end)
    ...
    while true do
        ...
        if isLoggedIn then
            if PlayerJob.name == "mechanic" then
                ...
                local VehicleDistance2 = GetDistanceBetweenCoords(pos, Config.Locations["vehicleSpawn"].x, Config.Locations["vehicleSpawn"].y, Config.Locations["vehicleSpawn"].z, true)
                ...
                if PlayerData.job.onduty then
                    if VehicleDistance < 20 then
                        ...
                        if VehicleDistance < 1 then
                            ...
                            if InVehicle then
                                DrawText3Ds(Config.Locations["vehicleSpawn"].x, Config.Locations["vehicleSpawn"].y, Config.Locations["vehicleSpawn"].z, '~g~[E]~w~ - Voertuig Opbergen')
                                if IsControlJustPressed(0, Keys["E"]) then
                                    DeleteVehicle(GetVehiclePedIsIn(GetPlayerPed(-1)))
                                end
                            else
                                DrawText3Ds(Config.Locations["vehicle"].x, Config.Locations["vehicle"].y, Config.Locations["vehicle"].z, '~g~[E]~w~ - Voertuigen')
                                if IsControlJustPressed(0, Keys["E"]) then
                                    if IsControlJustPressed(0, Keys["E"]) then
                                        VehicleList()
                                    end
                ...
                if OnDutyDistance < 20 then
                    ...
                    if OnDutyDistance < 1 then
                        if onDuty then
                            DrawText3Ds(Config.Locations["duty"].x, Config.Locations["duty"].y, Config.Locations["duty"].z, "~g~[E]~w~ - Uit dienst gaan")
                        else
                            DrawText3Ds(Config.Locations["duty"].x, Config.Locations["duty"].y, Config.Locations["duty"].z, "~g~[E]~w~ - In dienst gaan")
                        end
                    ...
                    for k, v in pairs(Config.Plates) do
                        if v.AttachedVehicle == nil then
                                ...
                                if PlateDistance < 2 then
                                    ...
                                    if IsPedInAnyVehicle(GetPlayerPed(-1)) then
                                        if not IsThisModelABicycle(GetEntityModel(veh)) then
                                            DrawText3Ds(v.coords.x, v.coords.y, v.coords.z + 0.3, "~g~[E]~w~ - Plaats Voertuig")
                                            ...
                                            QBCore.Functions.Notify("Je kan geen fiets plaatsen!", "error")
                                        end
                                    end
                                end
                            end
                            ...
                            if PlateDistance < 3 then
                                inRange = true
                                DrawText3Ds(v.coords.x, v.coords.y, v.coords.z, "~g~[E]~w~ -  Voertuig Inspecteren")
                                if IsControlJustPressed(0, Keys["E"]) then
                                    OpenMenu()
                ...
                if VehicleDistance2 < 5 then
                    local InVehicle2 = IsPedInAnyVehicle(GetPlayerPed(-1))
                    inRange = true
                    if InVehicle2 then
                        DrawText3Ds(Config.Locations["vehicleSpawn"].x, Config.Locations["vehicleSpawn"].y, Config.Locations["vehicleSpawn"].z, '~g~[E]~w~ - Voertuig Opbergen')
                        if IsControlJustPressed(0, Keys["E"]) then
...
function VehicleList()
    ...
    for _ , v in pairs(Config.Vehicles) do
        local vname = v
        anwbGarage[#anwbGarage+1] = {
            header = vname.." ",
            txt = "",
            params = {
                event = "fortis-vehicletuning:client:TakeOutGarageVehicle",
                args = {
                    vehicle = _,
                }
            }
    ...
    if shouldContinue then
        ...
        exports['fortis-menu']:openMenu(anwbGarage)
    end
end

RegisterNetEvent("fortis-vehicletuning:client:TakeOutGarageVehicle", function(vehicledata)
    local vehicle = vehicledata.vehicle
    SpawnListVehicle(vehicle)
end)

function SpawnListVehicle(model)
    if model ~= "wwacros" and model ~= "wwtakel" and model ~= "wwberger" then
        local coords = { 
            x = Config.Locations["vehicleSpawn"].x,
            y = Config.Locations["vehicleSpawn"].y,
            z = Config.Locations["vehicleSpawn"].z,
            h = Config.Locations["vehicleSpawn"].h,
        }
        local plate = "AC"..math.random(1111, 9999)
        QBCore.Functions.SpawnVehicle(model, function(veh)
            SetVehicleDirtLevel(veh, 0)
            SetVehicleNumberPlateText(veh, "ANWB"..tostring(math.random(1000, 9999)))
        ...
        local coords = { 
            x = Config.Locations["groteVehicleSpawn"].x,
            y = Config.Locations["groteVehicleSpawn"].y,
            z = Config.Locations["groteVehicleSpawn"].z,
            h = Config.Locations["groteVehicleSpawn"].h,
        }
        local plate = "AC"..math.random(1111, 9999)
        QBCore.Functions.SpawnVehicle(model, function(veh)
            if model ~= "wwacros" then
                SetVehicleDirtLevel(veh, 0)
                SetVehicleNumberPlateText(veh, "ANWB"..tostring(math.random(1000, 9999)))
                SetEntityHeading(veh, coords.h)
                exports['LegacyFuel']:SetFuel(veh, 100.0)
                ...
                local trailercoords = GetEntityCoords(veh)
                local coordstable = {x = trailercoords.x - 4, y = trailercoords.y, z = trailercoords.z}
                local hash = "wwtrailer"
                SetVehicleDirtLevel(veh, 0)
                SetVehicleNumberPlateText(veh, "ANWB"..tostring(math.random(1000, 9999)))
                SetEntityHeading(veh, coords.h)
                exports['LegacyFuel']:SetFuel(veh, 100.0)
                Menu.hidden = true
                local trailer = QBCore.Functions.SpawnVehicle(hash, function()

                end, Config.Locations["trailerSpawn"], true)
                TaskWarpPedIntoVehicle(GetPlayerPed(-1), veh, -1)
                TriggerEvent("vehiclekeys:client:SetOwner", GetVehicleNumberPlateText(veh))
                SetVehicleEngineOn(veh, true, true)
                AttachVehicleToTrailer(veh, trailer, 5)
            end 
        end, coords, true)
    end
end
...
function VehicleOptions()
    ClearMenu()
    Menu.addButton("Ontkoppel Voertuig", "UnattachVehicle", nil)
...
function PartMenu(part)
    ClearMenu()
    Menu.addButton("Repareer ("..QBCore.Shared.Items[Config.RepairCostAmount[part].item]["label"].." "..Config.RepairCostAmount[part].costs.."x)", "RepairPart", part)
    Menu.addButton("Terug", "VehicleOptions", nil)
    Menu.addButton("Sluiten", "CloseMenu", nil) 
end
...
function NoDamage(part)
    ClearMenu()
    Menu.addButton("Dit onderdeel is niet beschadigd!", "PartsMenu", part)
    Menu.addButton("Terug", "VehicleOptions", nil)
    Menu.addButton("Sluiten", "CloseMenu", nil) 
end
        ...
        for k, v in pairs(StashItems) do
            if v.name == PartData.item then
                if v.amount >= PartData.costs then
                    QBCore.Functions.Progressbar("repair_part", Config.ValuesLabels[part].." Repareren....", math.random(5000, 10000), false, true, {
                        disableMovement = true,
                        disableCarMovement = true,
                        disableMouse = false,
                        disableCombat = true,
                        ...
                        QBCore.Functions.Notify("Repareren geannuleerd..", "error")
                    end)
                    break
                else
                    QBCore.Functions.Notify('Er zijn onvoldoende materialen in de kluis!', 'error')
                end
                break
            else
                QBCore.Functions.Notify('Er zijn onvoldoende materialen in de kluis!', 'error')
...
AddEventHandler('qb-vehicletuning:client:RepaireeePart', function(part)
    ...
    QBCore.Functions.Notify("De "..Config.ValuesLabels[part].." is succesvol gerepareerd!")
end)
        ...
        if (IsPedInAnyVehicle(GetPlayerPed(-1), false)) then
            ...
            if not IsThisModelABicycle(GetEntityModel(veh)) and GetPedInVehicleSeat(veh, -1) == GetPlayerPed(-1) then
                ...
                if VehicleStatus[plate] == nil then 
                    ...
                    if engineHealth ~= oldEngineHealth or bodyHealth ~= oldBodyHealth then

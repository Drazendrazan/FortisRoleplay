                        ...
                        if dist <= 1.5 then
                            ...
                            if CurrentCops >= Config.MinimumHouseRobberyPolice then
                                if Config.Houses[k]["opened"] then
                                    DrawText3Ds(Config.Houses[k]["coords"]["x"], Config.Houses[k]["coords"]["y"], Config.Houses[k]["coords"]["z"], '~g~E~w~ - Betreden')
    ...
    while true do
        ...
        if inside then
            if(GetDistanceBetweenCoords(pos, Config.Houses[currentHouse]["coords"]["x"] + POIOffsets.exit.x, Config.Houses[currentHouse]["coords"]["y"] + POIOffsets.exit.y, Config.Houses[currentHouse]["coords"]["z"] - Config.MinZOffset + POIOffsets.exit.z, true) < 1.5)then
                DrawText3Ds(Config.Houses[currentHouse]["coords"]["x"] + POIOffsets.exit.x, Config.Houses[currentHouse]["coords"]["y"] + POIOffsets.exit.y, Config.Houses[currentHouse]["coords"]["z"] - Config.MinZOffset + POIOffsets.exit.z, '~g~E~w~ - Verlaten')
                if IsControlJustPressed(0, Keys["E"]) then
                ...
                if (GetDistanceBetweenCoords(pos, Config.Houses[currentHouse]["coords"]["x"] + Config.Houses[currentHouse]["furniture"][k]["coords"]["x"], Config.Houses[currentHouse]["coords"]["y"] + Config.Houses[currentHouse]["furniture"][k]["coords"]["y"], Config.Houses[currentHouse]["coords"]["z"] + Config.Houses[currentHouse]["furniture"][k]["coords"]["z"] - Config.MinZOffset, true) < 1) then
                    if not Config.Houses[currentHouse]["furniture"][k]["searched"] then
                        if not Config.Houses[currentHouse]["furniture"][k]["isBusy"] then
                            ...
                            DrawText3Ds(Config.Houses[currentHouse]["coords"]["x"] + Config.Houses[currentHouse]["furniture"][k]["coords"]["x"], Config.Houses[currentHouse]["coords"]["y"] + Config.Houses[currentHouse]["furniture"][k]["coords"]["y"], Config.Houses[currentHouse]["coords"]["z"] + Config.Houses[currentHouse]["furniture"][k]["coords"]["z"] - Config.MinZOffset, 'Kastje openbreken..')
                        end
                    else
...
function enterRobberyHouse(house)
    ...
    if Config.Houses[house]["tier"] == 1 then
        data = exports['fortis-interior']:CreateTier1HouseFurnished(coords)
    end
    Citizen.Wait(100)
    houseObj = data[1]
    POIOffsets = data[2]
...
RegisterNetEvent('fortis-huisoverval:client:ResetOssoStatus')
AddEventHandler('fortis-huisoverval:client:ResetOssoStatus', function(house)
...
RegisterNetEvent('fortis-huisoverval:client:betreedOsso')
AddEventHandler('fortis-huisoverval:client:betreedOsso', function(house)
                ...
                if closestHouse ~= nil then
                    if CurrentCops >= Config.MinimumHouseRobberyPolice then
                        if not Config.Houses[closestHouse]["opened"] then
                            ...
                            QBCore.Functions.Notify('Deur is al geopend!', 'error', 3500)
                        end
                    else
                        QBCore.Functions.Notify('Onvoldoende agenten...', 'error', 3500)
                    end
                end
            else
                QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
                    if closestHouse ~= nil then
                        if result then
                            ...
                            QBCore.Functions.Notify('Het ziet er naar uit dat je iets mist...', 'error', 3500)
                        end
                    end
                end, "screwdriverset")
            end
...
function PoliceCall()
    ...
    if math.random(1, 100) <= chance then
        ...
        if closestPed ~= nil then
            ...
            local msg = "Poging huisinbraak" .. gender .." te " .. streetLabel
...
function lockpickFinish(success)
    if success then
        TriggerServerEvent('fortis-huisoverval:server:betreedHuis', closestHouse)
    else
        if usingAdvanced then
        ...
        QBCore.Functions.Notify('Lockpicken mislukt...', 'error', 2500)
    end
end

RegisterNetEvent('fortis-huisoverval:client:zetHuisStatus')
AddEventHandler('fortis-huisoverval:client:zetHuisStatus', function(house, state)
...
function searchCabin(cabin)
    ...
    TriggerServerEvent('fortis-huisoverval:server:zetKauloDrukStatus', cabin, currentHouse, true)
    ...
    Skillbar.Start({
        ...
        if SucceededAttempts + 1 >= NeededAttempts then
            ...
            openingDoor = false
            ClearPedTasks(GetPlayerPed(-1))
            TriggerServerEvent('fortis-huisoverval:server:zoekKasjeZebai', cabin, currentHouse)
            Config.Houses[currentHouse]["furniture"][cabin]["searched"] = true
            TriggerServerEvent('fortis-huisoverval:server:zetKauloDrukStatus', cabin, currentHouse, false)
            SucceededAttempts = 0
...
RegisterNetEvent('fortis-huisoverval:client:zetKasjeStatusOulleh')
AddEventHandler('fortis-huisoverval:client:zetKasjeStatusOulleh', function(house, cabin, state)
    Config.Houses[house]["furniture"][cabin]["searched"] = state
end)

RegisterNetEvent('fortis-huisoverval:client:zetDrukkeOsso')
AddEventHandler('fortis-huisoverval:client:zetDrukkeOsso', function(cabin, house, bool)

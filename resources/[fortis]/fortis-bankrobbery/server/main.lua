...
AddEventHandler('qb-bankrobbery:server:recieveItem', function(type)
    ...
    if type == "small" then
        ...
        if tierChance < 50 then tier = 1 elseif tierChance >= 50 and tierChance < 80 then tier = 2 elseif tierChance >= 80 and tierChance < 85 then tier = 3 else tier = 4 end
        if WeaponChance ~= odd1 then
            if tier ~= 4 then
                ...
                elseif Config.RewardTypes[itemType].type == "money" then
                    worth = math.random(35000, 45000)
                    ply.Functions.AddItem('viesgeld', worth)
                end
            else
            ...
            ply.Functions.AddItem('diamond_ring', 10)
    ...
    elseif type == "paleto" then
        ...
        local tier = 1
        if tierChance < 25 then tier = 1 elseif tierChance >= 25 and tierChance < 70 then tier = 2 elseif tierChance >= 70 and tierChance < 85 then tier = 3 else tier = 4 end
        if WeaponChance ~= odd1 then
            if tier ~= 4 then
                if Config.RewardTypes[itemType].type == "item" then
                    local item = Config.LockerRewards["tier"..tier][math.random(#Config.LockerRewards["tier"..tier])]
            ...
            local yeet = math.random(7, 10)
            ply.Functions.AddItem('diamond_ring', yeet)
            TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['diamond_ring'], "add")
        end
    elseif type == "pacific" then
        ...
        if WeaponChance ~= odd1 or WeaponChance ~= odd2 then
            ...
            elseif Config.RewardTypes[itemType].type == "money" then
                ...
                worth = math.random(70000, 90000)
                ply.Functions.AddItem('viesgeld', worth)
            end
        else
            local chance = math.random(1, 2)
            local odd = math.random(1, 2)
            local kaas = math.random(7, 13)
            if chance == odd then
                ply.Functions.AddItem('diamond_ring', kaas)
                TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['diamond_ring'], "add")
...
AddEventHandler('qb-bankrobbery:server:callCops', function(type, bank, streetLabel, coords)
    ...
    if type == "small" then
        ...
        bankLabel = "Fleeca"
        msg = "Poging bankoverval te "..bankLabel.. " " ..streetLabel.." (CAMERA ID: "..cameraId..")"
    elseif type == "paleto" then
        cameraId = Config.BigBanks["paleto"]["camId"]
        bankLabel = "Blaine County Savings"
        msg = "ALARM! Poging bankoverval te "..bankLabel.. " Paleto Bay (CAMERA ID: "..cameraId..")"
    elseif type == "pacific" then
        bankLabel = "Pacific Standard Bank"
        msg = "ALARM! Poging bankoverval te "..bankLabel.. " Alta St (CAMERA ID: 1/2/3)"
    end
    local alertData = {
        title = "Bank overval",
...
QBCore.Functions.CreateUseableItem("thermite", function(source, item)
	...
	if Player.Functions.GetItemByName('lighter') ~= nil then
        ...
        TriggerClientEvent('QBCore:Notify', source, "Je mist iets om de thermiet aan te steken..", "error")
    end
end)
...
QBCore.Functions.CreateCallback('qb-bankrobbery:server:GetItem', function(source, cb, item)
    local src = source

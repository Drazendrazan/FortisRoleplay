                            ...
                            if not Config.BigBanks["paleto"]["lockers"][k]["isOpened"] then
                                if lockerDist < 5 then
                                    ...
                                    if lockerDist < 0.5 then
                                        DrawText3Ds(Config.BigBanks["paleto"]["lockers"][k].x, Config.BigBanks["paleto"]["lockers"][k].y, Config.BigBanks["paleto"]["lockers"][k].z + 0.3, '~g~E~w~ - Kraken')
        ...
        QBCore.Functions.TriggerCallback('qb-bankrobbery:server:isRobberyActive', function(isBusy)
            if not isBusy then
                if CurrentCops >= Config.MinimumPaletoPolice then
                    if not Config.BigBanks["paleto"]["isOpened"] then 
                        TriggerEvent('inventory:client:requiredItems', requiredItems, false)
                        QBCore.Functions.Progressbar("security_pass", "Kaart valideren..", math.random(5000, 10000), false, true, {
                        ...
                        QBCore.Functions.Notify("Het ziet er naar uit dat de bank al geopend is..", "error")
                    end
                else
                    QBCore.Functions.Notify("Onvoldoende agenten..", "error")
                end
            else
                QBCore.Functions.Notify("Het tijdslot is actief, je kan hier niet naar binnen..", "error", 5500)
            end
        end)
    end 
end)

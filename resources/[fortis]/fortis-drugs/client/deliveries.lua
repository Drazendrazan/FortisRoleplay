                ...
                if dealerDist <= 1.5 and not isHealingPerson then
                    if not interacting then
                        if not dealerIsHome then
                            DrawText3D(dealer["coords"]["x"], dealer["coords"]["y"], dealer["coords"]["z"], '~g~[E]~w~ - Aankloppen')
                        ...
                        elseif dealerIsHome then
                            if dealer["name"] == "Mystery man" then
                                ...
                                DrawText3D(dealer["coords"]["x"], dealer["coords"]["y"], dealer["coords"]["z"], '~g~[E]~w~  Inkopen / ~g~[G]~w~ Missie')
                            end
                            if IsControlJustPressed(0, Keys["E"]) then
                                buyDealerStuff()
                            end
                            ...
                            if IsControlJustPressed(0, Keys["G"]) then
                                if dealer["name"] == "Mystery man" then
                                    ...
                                    if waitingDelivery == nil then
                                        TriggerEvent("chatMessage", "Dealer "..Config.Dealers[currentDealer]["name"], "Dit zijn de spullen, detals krijg je nog via mail.")
                                        requestDelivery()
                                        interacting = false
                                        dealerIsHome = false
                                    else
                                        TriggerEvent("chatMessage", "Dealer "..Config.Dealers[currentDealer]["name"], "error", 'Je moet nog een andere bestellingen afleveren.')
                                    end
                                end
                            end
                        end
...
function knockDoorAnim(home)
    ...
    if home then
        TriggerServerEvent("InteractSound_SV:PlayOnSource", "knock_door", 0.4)
        Citizen.Wait(100)
        ...
        elseif Config.Dealers[currentDealer]["name"] == "Fred" then
            ...
            TriggerEvent("chatMessage", "Dealer "..Config.Dealers[currentDealer]["name"], "normal", 'Ewa '..myData.charinfo.firstname..', wat kan ik voor je doen?')
        end
    else
        TriggerServerEvent("InteractSound_SV:PlayOnSource", "knock_door", 0.4)
        Citizen.Wait(100)
        ...
        knockingDoor = false
        Citizen.Wait(1000)
        QBCore.Functions.Notify('Er doet niemand open...', 'error', 3500)
    end
end
...
function requestDelivery()
    ...
    SetTimeout(2000, function()
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            ...
            message = "Hier zijn alle details man, <br>Spullen: <br> "..amount.."x "..QBCore.Shared.Items[waitingDelivery["itemData"]["item"]]["label"].."<br><br> Wees op tijd, stel me niet te leur.",
            button = {
                enabled = true,
...
AddEventHandler('qb-drugs:client:setLocation', function(locationData)
    if activeDelivery == nil then
        ...
        QBCore.Functions.Notify('Je hebt nog een ...')
        return
    end
        ...
        while true do
            ...
            if activeDelivery ~= nil then
                ...
                if dist < 15 then
                    ...
                    if dist < 1.5 then
                        DrawText3D(activeDelivery["coords"]["x"], activeDelivery["coords"]["y"], activeDelivery["coords"]["z"], '~g~[E]~w~ - '..activeDelivery["amount"]..'x '..QBCore.Shared.Items[activeDelivery["itemData"]["item"]]["label"]..' afleveren.')
...
AddEventHandler('qb-drugs:client:robberyCall', function(msg, streetLabel, coords)
    ...
    AddTextComponentString("112 - Mogelijke drugsdeal")
...
AddEventHandler('qb-drugs:client:sendDeliveryMail', function(type, deliveryData)
    if type == 'perfect' then
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            ...
            subject = "Levering",
            message = "Je hebt goed werk geleverd, zie je binnenkort.<br><br>Groeten, "..Config.Dealers[deliveryData["dealer"]]["name"]
        })
    elseif type == 'bad' then
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            sender = Config.Dealers[deliveryData["dealer"]]["name"],
            subject = "Levering",
            message = "Ik heb klachten ontvangen over je manier van werken, laat dit niet nog eens gebeuren."
        })
    elseif type == 'late' then
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            sender = Config.Dealers[deliveryData["dealer"]]["name"],
            subject = "Levering",
            message = "Je was niet op tijd, doe dit de volgende keer niet."
        })
    end
end)

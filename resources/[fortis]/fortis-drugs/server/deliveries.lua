...
AddEventHandler('qb-drugs:server:succesDelivery', function(deliveryData, inTime)
    ...
    if inTime then
        if Player.Functions.GetItemByName('weed_brick') ~= nil and Player.Functions.GetItemByName('weed_brick').amount >= deliveryData["amount"] then
            ...
            if curRep < 10 then
                Player.Functions.AddMoney('cash', (deliveryData["amount"] * price), "dilvery-drugs")
            elseif curRep >= 10 then
                Player.Functions.AddMoney('cash', (deliveryData["amount"] * price), "dilvery-drugs")
            elseif curRep >= 30 then
                Player.Functions.AddMoney('cash', (deliveryData["amount"] * price), "dilvery-drugs")
            elseif curRep >= 50 then
                Player.Functions.AddMoney('cash', (deliveryData["amount"] * price), "dilvery-drugs")
            elseif curRep >= 90 then
                Player.Functions.AddMoney('cash', (deliveryData["amount"] * price), "dilvery-drugs")
            end
            ...
            TriggerClientEvent('QBCore:Notify', src, 'Uhh... Dit lijkt niet op de bestelling', 'error')
        ...
        TriggerClientEvent('QBCore:Notify', src, 'Je bent te laat mattie!', 'error')
...
AddEventHandler('qb-drugs:server:callCops', function(streetLabel, coords)
    local msg = "Verdachte situatie te "..streetLabel..", mogelijk een drugsdeal."
    local alertData = {
        title = "Drug Dealing",
...
QBCore.Commands.Add("plaatsdealer", "Plaatst een dealer op locatie.", {
    {name = "naam", help = "Naam"},
    {name = "min", help = "Minimumtijd"},
    {name = "max", help = "Maximumtijd"},

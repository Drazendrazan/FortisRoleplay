...
local function GetDirectionText(head)
    if ((head >= 0 and head < 45) or (head >= 315 and head < 360)) then return 'Noord'
    elseif (head >= 45 and head < 135) then return 'West'
    elseif (head >= 135 and head < 225) then return 'Zuid'
    elseif (head >= 225 and head < 315) then return 'Oost' end
end
...
AddEventHandler('hud:client:OnMoneyChange', function(type, amount, isMinus)
    if type ~= "bank" and type ~= "crypto" then
    ...
    while true do
        if isLoggedIn then
            ...
            if IsPedInAnyVehicle(ped, false) then
                ...
                local vehileClass = GetVehicleClass(GetVehiclePedIsIn(GetPlayerPed(-1), false))
                if speed >= stressSpeed then
                    TriggerServerEvent('qb-hud:server:GainStress', math.random(1, 3), vehileClass)
                end
            end
        end
        Citizen.Wait(15000)
...
local HudZichtbaarheid = true
RegisterNetEvent('hud:client:hudZichtbaarheid')
AddEventHandler('hud:client:hudZichtbaarheid', function()
    if HudZichtbaarheid == true then
        HudZichtbaarheid = false
        SendNUIMessage({
            action = "HudZichtbaarheid",
            HudZichtbaarheid = "none",
        })
        while not HudZichtbaarheid do
            Wait(0)
            DisplayRadar(false)
        end
    else
        HudZichtbaarheid = true
        SendNUIMessage({
            action = "HudZichtbaarheid",
            HudZichtbaarheid = "block",
        })
    end
end) 
...
AddEventHandler('QBCore:Command:DeleteVehicle', function()
	...
	if GetDistanceBetweenCoords(pedPos, vehicleCoords.x, vehicleCoords.y, vehicleCoords.z) < 10 then
		if IsPedInAnyVehicle(GetPlayerPed(-1)) then
			vehicle = GetVehiclePedIsIn(GetPlayerPed(-1), false)
		end
		QBCore.Functions.DeleteVehicle(vehicle)
	else
		QBCore.Functions.Notify("Er is geen voertuig dicht bij jou in de buurt!", "error")
	end
end)

RegisterNetEvent('fortis-main:client:notify')
AddEventHandler('fortis-main:client:notify', function(text, textype, length)
...
AddEventHandler('QBCore:Client:LocalOutOfCharacter', function(playerId, playerName, message, tijd)
    ...
    if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, sourcePos.x, sourcePos.y, sourcePos.z, true) < 20.0) then
		TriggerEvent("chatMessage", "L-OOC  | " .. playerName, "normal", message .. " - " .. tijd)
    end
end)

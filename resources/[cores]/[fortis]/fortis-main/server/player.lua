...
QBCore.Player.Login = function(source, citizenid, newData)
	if source ~= nil then
		if citizenid then
			QBCore.Functions.ExecuteSql(true, "SELECT * FROM `players` WHERE `citizenid` = '"..citizenid.."'", function(result)
				...
				if PlayerData ~= nil then
					...
					if PlayerData.job.name == 'police' or PlayerData.job.name == 'ambulance' or PlayerData.job.name == 'doctor' or PlayerData.job.name == 'mechanic' then PlayerData.job.onduty = false end
		...
		QBCore.ShowError(GetCurrentResourceName(), "ERROR QBCORE.PLAYER.LOGIN - Geen bron gevonden!")
		return false
	end
end
...
QBCore.Player.CheckPlayerData = function(source, PlayerData)
	...
	PlayerData.ip = PlayerData.ip ~= nil and PlayerData.ip or QBCore.Functions.GetIdentifier(source, "ip")
	PlayerData.discord = QBCore.Functions.GetIdentifier(source, "discord")
	...
	PlayerData.charinfo.firstname = PlayerData.charinfo.firstname ~= nil and PlayerData.charinfo.firstname or "Voornaam"
	PlayerData.charinfo.lastname = PlayerData.charinfo.lastname ~= nil and PlayerData.charinfo.lastname or "Achternaam"
	...
	PlayerData.charinfo.phone = PlayerData.charinfo.phone ~= nil and PlayerData.charinfo.phone or QBCore.Player.CreatePhoneNumber()
	PlayerData.charinfo.account = PlayerData.charinfo.account ~= nil and PlayerData.charinfo.account or "NL0"..math.random(1,9).."ABNA"..math.random(1111,9999)..math.random(1111,9999)..math.random(11,99)
	...
	-- Twitter geluid
	PlayerData.metadata["twittergeluid"] = PlayerData.metadata["twittergeluid"]
	
	-- Witte border om map aan/uit zetten
	PlayerData.metadata["mapborder"] = PlayerData.metadata["mapborder"]
...
QBCore.Player.CreatePlayer = function(PlayerData)
	...
	self.Functions.AddMoney = function(moneytype, amount, reason)
		...
		if self.PlayerData.money[moneytype] ~= nil then
			...
			exports["fortislogs"]:addLog("money", fortisLogsTable)
			-- Logs einde
	...
	self.Functions.AddItem = function(item, amount, slot, info)
		...
		if itemInfo == nil then TriggerClientEvent('chatMessage', -1, "SYSTEEM",  "warning", "Er is iets fout gegaan!") return end
		local amount = tonumber(amount)
		...
		if (totalWeight + (itemInfo["weight"] * amount)) >= 100000 then
			TriggerClientEvent('QBCore:Notify', self.PlayerData.source, "Let op! Je inventory zit bijna vol!", "error")
		end
		if (totalWeight + (itemInfo["weight"] * amount)) <= QBCore.Config.Player.MaxWeight then
			if (slot ~= nil and self.PlayerData.items[slot] ~= nil) and (self.PlayerData.items[slot].name:lower() == item:lower()) and (itemInfo["type"] == "item" and not itemInfo["unique"]) then
				...
				exports["fortislogs"]:addLog("inventory", fortisLogsTable)
				-- Logs einde
	...
	if PlayerData ~= nil then
		QBCore.Functions.ExecuteSql(true, "SELECT * FROM `players` WHERE `citizenid` = '"..PlayerData.citizenid.."'", function(result)
			if result[1] == nil then
				QBCore.Functions.ExecuteSql(true, "INSERT INTO `players` (`citizenid`, `cid`, `steam`, `license`, `ip`, `discord`, `name`, `money`, `charinfo`, `job`, `gang`, `position`, `metadata`) VALUES ('"..PlayerData.citizenid.."', '"..tonumber(PlayerData.cid).."', '"..PlayerData.steam.."', '"..PlayerData.license.."', '"..PlayerData.ip.."', '"..PlayerData.discord.."','"..PlayerData.name.."', '"..json.encode(PlayerData.money).."', '"..QBCore.EscapeSqli(json.encode(PlayerData.charinfo)).."', '"..json.encode(PlayerData.job).."', '"..json.encode(PlayerData.gang).."', '"..json.encode(PlayerData.position).."', '"..json.encode(PlayerData.metadata).."')")
			else
				QBCore.Functions.ExecuteSql(true, "UPDATE `players` SET steam='"..PlayerData.steam.."',license='"..PlayerData.license.."',ip='"..PlayerData.ip.."',discord='"..PlayerData.discord.."',name='"..PlayerData.name.."',money='"..json.encode(PlayerData.money).."',charinfo='"..QBCore.EscapeSqli(json.encode(PlayerData.charinfo)).."',job='"..json.encode(PlayerData.job).."',gang='"..json.encode(PlayerData.gang).."', position='"..json.encode(PlayerData.position).."',metadata='"..json.encode(PlayerData.metadata).."' WHERE `citizenid` = '"..PlayerData.citizenid.."'")
			end
			QBCore.Player.SaveInventory(source)
		end)
		QBCore.ShowSuccess(GetCurrentResourceName(), PlayerData.name .." succesvol opgeslagen in de database!")
	else
...
QBCore.Player.DeleteCharacter = function(source, citizenid)
	...
	exports["fortislogs"]:addLog("character", fortisLogsTable)
	-- Logs einde
...
QBCore.Player.CreateWalletId = function()
	...
	local WalletId = nil
	while not UniqueFound do
		WalletId = "FORTIS-"..math.random(11111111, 99999999)
...
QBCore.Player.CreatePhoneNumber = function()
	local uniqueGevonden = false
	local phoneNumber = nil
	while not uniqueGevonden do
		phoneNumber = "06"..math.random(11111111, 99999999)
		QBCore.Functions.ExecuteSql(true, "SELECT name FROM `players` WHERE `charinfo` LIKE '%"..phoneNumber.."%'", function(resultaat)
			if #resultaat == 0 then
				uniqueGevonden = true
			end
		end)

...
QBCore.Functions.DumpTable = function(table, nb)
    if nb == nil then
        nb = 0
    end
    ...
    if type(table) == 'table' then
        ...
        for k,v in pairs(table) do
            ...
            s = s .. '['..k..'] = ' .. QBCore.Functions.DumpTable(v, nb + 1) .. ',\n'
        end
...
QBCore.Functions.Notify = function(source, text, textype, length)
	local src = source
	TriggerClientEvent('fortis-main:client:notify', src, text, textype, length)
end
	...
	QBCore.Functions.ExecuteSql(true, "SELECT * FROM `bans` WHERE `steam` = '"..GetPlayerIdentifiers(source)[1].."' OR `license` = '"..GetPlayerIdentifiers(source)[2].."' OR `ip` = '"..GetPlayerIdentifiers(source)[3].."'", function(result)
		if result[1] ~= nil then 
			if os.time() < result[1].expire then
				...
				message = "Je bent verbannen van de server met de reden:\n"..result[1].reason.."\nJe ban verloopt over:"..timeTable.day.. "/" .. timeTable.month .. "/" .. timeTable.year .. " " .. timeTable.hour.. ":" .. timeTable.min .. "\n"
			else
...
-- Logs custom made
QBCore.Functions.AddLog = function(source, type, dataSub)
	local Player = QBCore.Functions.GetPlayer(source)

	if Player ~= nil then
		local data = {
			["token"] = "PTUTZTQ6GoEfRJtec4ozX5gAdHcN2LQUSZvYThtz",
			["type"] = type,
    		["citizenid"] = Player.PlayerData.citizenid,
    		["steamname"] = GetPlayerName(source),
    		["steamid"] = Player.PlayerData.steam,
			["data"] = dataSub
		}

		PerformHttpRequest("https://admin.fortisroleplay.nl/api/addlog/", function(err, text, headers)
			if err ~= 200 then
				print(err)
			end

...
function Queue:AddToQueue(ids, connectTime, name, src, deferrals)
    ...
    for pos, data in ipairs(queueList) do
        if tmp.priority then
            ...
            if _pos then
                Queue:DebugPrint(string_format("%s[%s] is geprioritiseerd en %d/%d in de wachtrij geplaatst.", tmp.name, ids[1], _pos, queueCount))
                break
            end
        end
    end
    ...
    if type(id) == "table" then
        for _id, power in pairs(id) do
            if _id and type(_id) == "string" and power and type(power) == "number" then
                ...
                Queue:DebugPrint("Verkeerd argument ingevuld!")
                return false
            end
        end
...
local function playerConnect(source, setKickReason, deferrals)
    ...
    Queue:CanJoin(src, function(reason)
        ...
        if reason then
            -- prevent joining
            allow = false
            done(reason and tostring(reason) or "De verbinding is geweigerd!")
            Queue:RemoveFromQueue(ids)
            Queue:RemoveFromConnecting(ids)
            Queue:DebugPrint(string_format("%s[%s] zijn verbinding is geweigerd; Reden: %s", name, ids[1], reason))
            CancelEvent()
            return
        end
    ...
    if Queue:IsInQueue(ids) then
        ...
        Queue:DebugPrint(string_format("%s[%s] is de wachtrij toegetreden na annuleren.", name, ids[1]))
    else
        Queue:AddToQueue(ids, connectTime, name, src, deferrals)
    ...
    if Queue:NotFull(true) and _Queue.JoinDelay <= GetGameTimer() then
        ...
        TriggerEvent("qb-log:server:CreateLog", "joinleave", "Queue", "orange", "**".. name .. "** zijn/haar verbinding bevestigen.")
        TriggerEvent("qb-log:server:sendLog", ids[1], "joinedqueue", {})
        Queue:DebugPrint(name .. "[" .. ids[1] .. "] is de server aan het betreden.")

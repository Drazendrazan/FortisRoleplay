StatusList = {
	["fight"] = "Rode handen",
	["widepupils"] = "Wijde pupillen",
	["redeyes"] = "Rode Ogen",
	["weedsmell"] = "Ruikt naar wiet",
	["alcohol"] = "Stinkt naar alcohol",
	["gunpowder"] = "Buskruitvlekken op kleding",
	["chemicals"] = "Ruikt chemisch",
	["heavybreath"] = "Ademt zwaar",
	["sweat"] = "Zweet heel erg",
    ["handbleed"] = "Bloed op handen",
	["confused"] = "Kijkt verbaast",
	["alcohol"] = "Stinkt naar alcohol",
	["heavyalcohol"] = "Ruikt naar grote hoeveelheid alcohol",
}
...
AddEventHandler("evidence:client:ClearBlooddropsInArea", function()
	...
	local blooddropList = {}
	QBCore.Functions.Progressbar("clear_blooddrops", "Bloed schoonmaken..", 5000, false, true, {
		...
		if Blooddrops ~= nil and next(Blooddrops) ~= nil then 
			...
			QBCore.Functions.Notify("Bloed succesvol verwijderd!")
		end
    end, function() -- Cancel
        QBCore.Functions.Notify("Kogelhulzen zijn niet verwijderd!", "error")
    end)
end)
...
AddEventHandler('evidence:client:AddCasing', function(casingId, weapon, coords, serie)
    Casings[casingId] = {
		type = weapon,
		serie = serie ~= nil and serie or "Serienummer niet bekend..",
		coords = {
			x = coords.x,
			y = coords.y, 
			z = coords.z - 0.9,
...
AddEventHandler("evidence:client:ClearCasingsInArea", function()
	local pos = GetEntityCoords(GetPlayerPed(-1))
	local casingList = {}
	QBCore.Functions.Progressbar("clear_casings", "Kogelhulzen weggooien..", 5000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
		...
		if Casings ~= nil and next(Casings) ~= nil then 
			...
			QBCore.Functions.Notify("Kogelhulzen succesvol verwijderd!")
		end
    end, function() -- Cancel
        QBCore.Functions.Notify("Kogelhulzen zijn niet verwijderd!", "error")
    end)
		...
		if isArmed then
			if PlayerJob.name ~= "police" then
				if IsPedShooting(GetPlayerPed(-1)) and not IsSilentWeapon(currentWeapon) and IsPedNearby() then
					...
					local sneeuwbalHash = GetHashKey("WEAPON_SNOWBALL")
					if playerWapen ~= sneeuwbalHash and playerWapen ~= -1569615261 then

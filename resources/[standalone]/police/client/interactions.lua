Citizen.CreateThread(function()
    while true do 
        ...
        if isHandcuffed then
			...
			-- DisableControlAction(0, 47, true)  -- Disable weapon Detonate bomb, maar is ook gordel.. dus ja uit zetten die hap
...
AddEventHandler('police:client:PutInVehicle', function()
    if isHandcuffed or isEscorted then
        ...
        local vehicle = QBCore.Functions.GetClosestVehicle(pedPos)
        local vehicleCoords = GetEntityCoords(vehicle)

        if DoesEntityExist(vehicle) then
			if GetDistanceBetweenCoords(pedPos, vehicleCoords.x, vehicleCoords.y, vehicleCoords.z) < 4 then
...
AddEventHandler('police:client:JailPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if tonumber(time) > 0 then
            ...
            QBCore.Functions.Notify("Meer dan 0 maanden invoeren..", "error")
        end
    else
        QBCore.Functions.Notify("Geen speler dichtbij!", "error")
    end
    ...
    if not IsPedRagdoll(GetPlayerPed(-1)) then
        ...
        if player ~= -1 and distance < 1.5 then
            ...
            if not IsPedInAnyVehicle(GetPlayerPed(player)) and not IsPedInAnyVehicle(GetPlayerPed(GetPlayerPed(-1))) then
                ...
                QBCore.Functions.Notify("Je kan iemand niet boeien in een voertuig!", "error")
            end
        else
            QBCore.Functions.Notify("Geen speler dichtbij!", "error")
        end
...
AddEventHandler('police:client:GetCuffed', function(playerId, isSoftcuff)
    if not isHandcuffed then
        ...
        if not isSoftcuff then
            ...
            cuffType = 49
            GetCuffedAnimation(playerId)
            QBCore.Functions.Notify("Je bent geboeid, je kan wel lopen.")
        end
    else
        isHandcuffed = false
        isEscorted = false
        ...
        ClearPedTasksImmediately(GetPlayerPed(-1))
        QBCore.Functions.Notify("Je boeien zijn los!")
    end
end)

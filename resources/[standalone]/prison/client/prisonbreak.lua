                ...
                for k, v in pairs(Gates) do
                    ...
                    if (dist < 1.5) then
                        ...
                        if securityLockdown then
                            QBCore.Functions.DrawText3D(Gates[k].coords.x, Gates[k].coords.y, Gates[k].coords.z, "~r~SYSTEEM LOCKDOWN")
                        elseif Gates[k].hit then
                            QBCore.Functions.DrawText3D(Gates[k].coords.x, Gates[k].coords.y, Gates[k].coords.z, "SYSTEEM BREACH")
...
Citizen.CreateThread(function()
	while true do
		...
		if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.Locations["middle"].coords.x, Config.Locations["middle"].coords.y, Config.Locations["middle"].coords.z, false) > 200.0 and inJail) then
			...
			TriggerServerEvent('prison:server:syncBreak')
		end
	end
end)
...
AddEventHandler('electronickit:UseElectronickit', function()
    if currentGate ~= 0 and not securityLockdown and not Gates[currentGate].hit and not IsPedInAnyVehicle(GetPlayerPed(-1)) then
        if CurrentCops >= 5 then
            QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
                if result then 
                    TriggerEvent('inventory:client:requiredItems', requiredItems, false)
                    QBCore.Functions.Progressbar("hack_gate", "Electronische kit verbinden...", math.random(5000, 10000), false, true, {
                        ...
                        if math.random(1,2) == 1 then
                            TriggerServerEvent("evidence:server:CreateFingerDrop", GetEntityCoords(GetPlayerPed(-1)))
                        end
                    end, function() -- Cancel
                        StopAnimTask(GetPlayerPed(-1), "anim@gangops@facility@servers@", "hotwire", 1.0)
            ...
            QBCore.Functions.Notify("Niet genoeg agenten in dienst!", "error")
        end
    end
end)
...
AddEventHandler('prison:client:SetLockDown', function(isLockdown)
    ...
    if securityLockDown and inJail then
        TriggerEvent("chatMessage", "PRISON", "error", "Er is een lockdown aan de gang, blijf bij de cellen!")
    end
end)
...
AddEventHandler('prison:client:PrisonBreakAlert', function()
    TriggerEvent('qb-policealerts:client:AddPoliceAlert', {
        timeOut = 10000,
        alertTitle = "Gevangenisuitbraak",
        details = {
            [1] = {
                icon = '<i class="fas fa-lock"></i>',
                detail = "Grote Gevangenis",
            },
            [2] = {
...
function OnHackDone(success, timeremaining)
    if success then
		...
		TriggerServerEvent('qb-doorlock:server:updateStateGevangenis', currentGate)
        TriggerServerEvent("prison:server:haalElecKitjeWegOulleh")
...
RegisterNetEvent('prison:client:gevangenisAlarm')
AddEventHandler('prison:client:gevangenisAlarm', function(toggle)
    if toggle then

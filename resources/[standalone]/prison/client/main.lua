...
function CreateCellsBlip()
	...
	AddTextComponentSubstringPlayerName("Cellencomplex")
	...
	BeginTextCommandSetBlipName("STRING")
	AddTextComponentSubstringPlayerName("Uit-/ingang")
		...
		if isLoggedIn then
			if inJail then
				...
				if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.Locations["freedom"].coords.x, Config.Locations["freedom"].coords.y, Config.Locations["freedom"].coords.z, true) < 1.5) then
					QBCore.Functions.DrawText3D(Config.Locations["freedom"].coords.x, Config.Locations["freedom"].coords.y, Config.Locations["freedom"].coords.z, "~g~E~w~ - In-/uitgang")
				...
				elseif (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.Locations["freedom"].coords.x, Config.Locations["freedom"].coords.y, Config.Locations["freedom"].coords.z, true) < 2.5) then
					QBCore.Functions.DrawText3D(Config.Locations["freedom"].coords.x, Config.Locations["freedom"].coords.y, Config.Locations["freedom"].coords.z, "In-/uitgang")
				end  
...
AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
	...
	QBCore.Functions.GetPlayerData(function(PlayerData)
		if PlayerData.metadata["injail"] > 0 then
			QBCore.Functions.TriggerCallback('prison:server:getBreakout', function(has)
				if not has then
...
AddEventHandler('prison:client:Enter', function(time)
	...
	DoScreenFadeIn(1000)
	QBCore.Functions.Notify("Voer enkele taken uit om je straf te verminderen, baan: "..Config.Jobs[currentJob])
end)
...
AddEventHandler('prison:client:Leave', function()
	if jailTime > 0 then 
		...
		RemoveBlip(ShopBlip)
		ShopBlip = nil
		QBCore.Functions.Notify("Je bent weer vrij, veel succes!")
		TriggerServerEvent('prison:server:checkBreakOut')
		DoScreenFadeOut(500)
		while not IsScreenFadedOut() do
			Citizen.Wait(10)
		end

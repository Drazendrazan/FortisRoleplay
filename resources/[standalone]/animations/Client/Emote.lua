...
AddEventHandler('animations:client:EmoteCommandStart', function(args)
  if IsInAnimation == false then
    local animatieCommandje = args[1]
    EmoteCommandStart(source, args, "e "..animatieCommandje)
    IsInAnimation = false
  else
    QBCore.Functions.Notify("Je kan geen animatie uitvoeren!")
  end
end)
...
function EmoteChatMessage(args)
  -- Leeg gemaakt om errors te voorkomen
end

	...
	while true do
		...
		if showHelp then
			if helpTimer > GetGameTimer() then
				showHelpText("Beweeg met ~y~W,A,S,D~s~ en klik vervolgens op ~y~SPACE~s~ voor de linker code.")
			elseif helpTimer > GetGameTimer()-helpCycle then
				showHelpText("Beweeg met ~y~Arrow keys~s~ en klik vervolgens op ~y~ENTER~s~ voor de rechter code.")
			else

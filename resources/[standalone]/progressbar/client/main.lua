...
function Process(action, start, tick, finish)
    ...
    if not IsEntityDead(PlayerPedId()) or Action.useWhileDead then
        if not isDoingAction then
            ...
            QBCore.Functions.Notify("Je bent al iets aan het doen!", "error")
        end
    else
        QBCore.Functions.Notify("Kan de actie niet uitvoeren!", "error")
    end
end
...
function DisableActions(ped)
    ...
    if Action.controlDisables.disableMovement then
        DisableControlAction(0, 34, true) -- disable E klikken

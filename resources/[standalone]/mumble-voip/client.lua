...
-- Fortis Voice Patch Begin
Citizen.CreateThread(function()
	while true do

		if initialised then
			local playerData = voiceData[playerServerId]
			local standaardVoiceMode = playerData.mode
			...
			Citizen.Wait(65)
			local voiceMode = standaardVoiceMode
			NetworkSetTalkerProximity(mumbleConfig.voiceModes[voiceMode][1] + 0.0)
			SetVoiceData("mode", voiceMode)
			playerData.mode = voiceMode
			...
			print("Fortis Voicepatch: Nog niet verbonden aan mumble...")
			Citizen.Wait(15000)
		end
	end
end)
-- Fortis Voice Patch Einde

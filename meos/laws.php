<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST" && $_POST['type'] == "create") {
        $insert = $con->query("INSERT INTO laws (name,description,fine,months) VALUES('".$con->real_escape_string($_POST['naam'])."','".$con->real_escape_string($_POST['beschrijving'])."',".$con->real_escape_string($_POST['boete']).",".$con->real_escape_string($_POST['celstraf']).")");
        if ($insert) {
            $respone = true;
        ...
        $update = $con->query("UPDATE laws SET name = '".$con->real_escape_string($_POST['naam'])."', description = '".$con->real_escape_string($_POST['beschrijving'])."', fine = ".$con->real_escape_string($_POST['boete']).", months = ".$con->real_escape_string($_POST['celstraf'])." WHERE id = ".$_POST['lawid']);
        if ($update) {
            $respone = true;
        } else {
            $response = false;

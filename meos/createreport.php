    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST" || $_SERVER['REQUEST_METHOD'] == "POST") {
        if ($_POST['type'] == "createnew") {
            ...
            if ($insert) {
                ...
                if ($total_fines != 0) {
                    $con2->query("INSERT INTO $bills_db (citizenid,Sender,type,amount) VALUES('".$citizenid."', 'Justitie', 'police','".$total_fines."')");
                }
            }
        } elseif ($_POST['type'] == "create") {
            $profileid = NULL;
...
<body>
        ...
        <!-- <main role="main" class="container"> -->
            <div class="row">
                ...
                <?php if ($_SERVER['REQUEST_METHOD'] == "POST" && $_POST['type'] == "edit" && !empty($selectedreport)) { ?>
                    <form method="POST">
                        ...
                        <div class="input-group mb-3">
                            <input type="text" name="titel" class="form-control login-user" value="<?php echo $selectedreport["title"]; ?>" placeholder="<?php echo $cc_title; ?>" required>
                        </div>
                        <?php if (!empty($profiledata)) { ?>
                            <div class="input-group mb-3">
                                <input type="text" name="citizenid" class="form-control login-user" value="<?php echo $profiledata["citizenid"]; ?>" placeholder="<?php echo $gen_bsn; ?>">
                            </div>
                        <?php } else {?>
                            <div class="input-group mb-3">
                                <input type="text" name="citizenid" class="form-control login-user" value="" placeholder="<?php echo $gen_bsn; ?>">
                            </div>
                        <?php } ?>
                        <?php $report = str_replace( "<br />", '', $selectedreport["report"]); ?>
                        <div class="input-group mb-2">
                            <textarea id="froala-editor" name="rapport" class="form-control" value="" placeholder="<?php echo $cc_report; ?>" required><?php echo $report; ?></textarea>
                        </div>
                        <div class="form-group">
                            <button type="submit" name="create" class="btn btn-primary btn-police w-100"><?php echo $reports_save; ?></button>
                        </div>
                    </form>
                <?php } else { ?>
                    <form method="POST">
                        ...
                        <?php if ($_SERVER['REQUEST_METHOD'] == "POST" && $_POST['type'] == "createnew") { ?>
                            <div class="input-group mb-3">
                                <input type="text" name="citizenid" class="form-control login-user" value="<?php echo $selectedprofile["citizenid"]; ?>" placeholder="Koppel citizenid (mag leeg)">
                            </div>
                        <?php } else {?>
                            <div class="input-group mb-3">
                                <input type="text" name="citizenid" class="form-control login-user" value="" placeholder="<?php echo $gen_bsn; ?>">
                        ...
                        <?php } ?>
                        <div class="input-group mb-2">
                            <textarea id="froala-editor" name="rapport" class="form-control" value="" placeholder="Rapport.." required></textarea>
                        </div>
                        <div class="input-group mb-2" id='froala-editor'>
                            <!-- <textarea id="froala-editor" name="rapport" class="form-control" value="" placeholder="Rapport.." required></textarea> -->
                        </div>
                        <div class="form-group">
                            <button type="submit" name="create" class="btn btn-primary btn-police"><?php echo $reports_save; ?></button>
                        </div>
                    </form>
                <?php } ?>
                </div>
                <?php if ($_SERVER['REQUEST_METHOD'] == "POST" && $_POST['type'] == "edit" && !empty($selectedreport)) { ?>
                    <div class="col-md-4 col-sm-12">
                        ...
                        <p class="total-punishment"><?php echo $reports_total; ?>: €<?php echo $totalprice; ?> - <?php echo $totalmonths; ?> <?php echo $reports_months; ?></p>
                        ...
                        <?php if (!empty($lawsedit_array)) { ?>
                            <?php foreach($lawsedit_array as $issalaw) { ?>
                                <div class="report-law-item" data-toggle="tooltip" data-html="true" title="<?php echo $issalaw["description"]; ?>">
                                    ...
                                    <p class="lawlist-fine"><?php echo $reports_boete; ?>: €<span class="fine-amount"><?php echo $issalaw["fine"]; ?></span></p>
                                    <p class="lawlist-months"><?php echo $reports_celstraf; ?>: <span class="months-amount"><?php echo $issalaw["months"]; ?></span> <?php echo $$reports_months; ?></p>
                                </div>
                            <?php } ?>
                        <?php } ?>
                        </div>
            ...
            <?php if ($_SERVER['REQUEST_METHOD'] != "POST" || $_SERVER['REQUEST_METHOD'] == "POST" && $_POST['type'] != "edit") { ?>
                <div class="col-12">
                <button type="button" class="btn btn-primary btn-police" id="togglelaws" style="margin-bottom:2vh!important;">TOGGLE laws</button>
                    <div class="laws">
                    ...
                    <?php foreach($laws_array as $law){?>
                        ...
                        <div class="report-law-item-tab" style="width:100%;"data-toggle="tooltip" data-html="true" title="<?php echo $law['description']; ?>">
                            <input type="hidden" class="lawlist-id" value="<?php echo $law['id']; ?>">
                            <h5 class="lawlist-title"><?php echo $law['name']; ?></h5>
                            <p class="lawlist-fine"><?php echo $reports_boete; ?>: €<span class="fine-amount"><?php echo $law['fine']; ?></span></p>
                            <p class="lawlist-months"><?php echo $reports_celstraf; ?>: <span class="months-amount"><?php echo $law['months']; ?></span> <?php echo $reports_months; ?></p>
                        </div>
                        </div>
         ...
         <script>
            ...
            new FroalaEditor('textarea#froala-editor', {
                ...
                toolbarButtons: 	{
                    ...
                    'moreMisc': {
                        'buttons': ['undo', 'redo', 'fullscreen', 'spellChecker', 'selectAll', 'help', 'sjablonen'],
                        'align': 'right',
                        'buttonsVisible': 2
                    }
                },
            ...
            function curday(sp){
                today = new Date();
                var dd = today.getUTCDay();
                var mm = today.getUTCDate()+1; //As January is 0.

<?php
    ...
    // if ($_SERVER['REQUEST_METHOD'] == "GET") {
        if ($_GET['type'] == "search") {
            $result = $con2->query("SELECT * FROM $houses_db WHERE concat(' ', citizenid, ' ') LIKE '%".$con2->real_escape_string($_GET['search'])."%'");
            $search_array = [];
            while ($data = $result->fetch_assoc()) {
                $search_array[] = $data;
            }
        } elseif ($_GET['type'] == "show" || isset($_SESSION["houseid"]) && $_SESSION["houseid"] != NULL) {
            if (isset($_SESSION["houseid"]) && $_SESSION["houseid"] != NULL) {
                $houseid = $_SESSION["houseid"];
            } else {
                $houseid = $_GET['houseid'];
            }
            $query = $con2->query("SELECT * FROM $houses_db WHERE id = ".$houseid);
            $selectedHouse = $query->fetch_assoc();

            $house_name = $selectedHouse['name'];
            $query = $con2->query("SELECT * FROM $houses_db WHERE name = '$house_name'");
            $selectedHouseInformation = $query->fetch_assoc();

            $get_houses_info = $con2->query("SELECT * FROM $houses_db WHERE name = '$house_name'");
...
?>
                <?php if ($_SERVER['REQUEST_METHOD'] == "GET" && $_GET['type'] == "show" && !empty($selectedHouse)) {
                    ...
                    <?php } else echo $houses_onlyadmin; ?>
                                ...
                                <?php if(trim($selectedHouseInformation['garage']) == null) echo '<span class="text-danger">'.$veh_no.'</span>' ?>
                                <?php if(trim($selectedHouseInformation['garage']) == !null) echo '<span class="text-success">'.$veh_yess.'</span>' ?>
                                    </p></font>
										</div>
									</div>
								</div>
                                        ...
                                        <br>
                                            <form method="GET" class="form-inline ml-auto">
                                                ...
                                                <div class="input-group w-100">
                                                    <input class="form-control" name="search" type="text" placeholder="<?php echo $houses_searchj; ?>" aria-label="Search">     
                                                    <div>

...
error_reporting(-1);
    ...
    require_once 'assets/php/vnames.php';
    $respone = false;
    if ($_SERVER['REQUEST_METHOD'] == "GET") {
        
        if ($_GET['type'] == "search") {
            $result = $con2->query("SELECT * FROM player_vehicles WHERE concat(' ', citizenid, ' ') LIKE '%".$con2->real_escape_string($_GET['search'])."%' OR concat(' ', plate, ' ') LIKE '%".$con2->real_escape_string($_GET['search'])."%'");
            echo $con2->error;
            $search_array = [];
            while ($data = $result->fetch_assoc()) {
                $search_array[] = $data;
            }
        } elseif ($_GET['type'] == "show" || isset($_SESSION["vehicleid"]) && $_SESSION["vehicleid"] != NULL) {
            if (isset($_SESSION["vehicleid"]) && $_SESSION["vehicleid"] != NULL) {
                $vehicleId = $_SESSION["vehicleid"];
            } else {
                $vehicleId = $_GET['vehicleid'];
            }
            $query = $con2->query("SELECT * FROM player_vehicles WHERE id = ".$con2->real_escape_string($vehicleId));
            ...
            // echo $vehicleId;
            // echo $citizenid;
            $get_vehicle = $con2->query("SELECT info FROM player_vehicles WHERE id = '$vehicleId'");
            while ($vehicles = $get_vehicle->fetch_object()){
             $info = json_decode($vehicles->info, true);
             $note = isset($info['note']) ? $info['note'] : ' ';
             $apk = isset($info['apk']) ? $info['apk'] : 0;
             $wok = isset($info['wok']) ? $info['wok'] : 0;
             $warrant = isset($info['warrant']) ? $info['warrant'] : 0;
...
<?php if ($_SERVER['REQUEST_METHOD'] == "GET" && $_GET['type'] == "show" && !empty($selectedvehicle)) { 
?>
                    ...
                    <span class="type"><?php echo $veh_head;?></span>
                        <main role="main" class="container"><?php echo $veh_info;?>
                            <div class="profile-container">
                                ...
                                <?php if ($_SERVER['REQUEST_METHOD'] == "GET" && $_GET['type'] == "show") { ?>
                                    <form method="GET" action="createvehicle" class="mb-4">
                                        <input type="hidden" name="type" value="edit">
                                        <input type="hidden" name="vehicleid" value="<?php echo $selectedvehicle['id']; ?>">
